...
TL;DR: DEAF benchmark reveals Audio MLLMs rely more on text than acoustic signals despite apparent speech understanding


<details>
  <summary>Details</summary>
Motivation: To determine whether Audio MLLMs genuinely process acoustic signals or rely on text-based semantic inference, as current benchmarks don't reveal this distinction

Method: Created DEAF benchmark with 2,700+ conflict stimuli across emotional prosody, background sounds, and speaker identity; designed multi-level evaluation framework with increasing textual influence; introduced diagnostic metrics to quantify text vs. acoustic reliance
...
Conclusion: There's a gap between high performance on standard speech benchmarks and genuine acoustic understanding; Audio MLLMs exhibit text-driven bias rather than true acoustic processing
...
TL;DR: This thesis addresses three fundamental limitations of current AI systems: data-inefficient knowledge acquisition, reliance on human-generated data, and confinement to human-designed algorithms. It proposes three solutions: synthetic data amplification for efficient knowledge updates, self-generated synthetic data for pretraining bootstrapping, and algorithmic search scaling to transcend human-designed training paradigms.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome three key limitations of modern language model-based AI systems: 1) inefficient knowledge acquisition from small specialized corpora after pretraining, 2) heavy reliance on finite human-generated historical data, and 3) confinement to algorithms discovered by human researchers. These limitations fundamentally cap AI capabilities by their human creators.

Method: Three complementary approaches: 1) Synthetic data approach that diversifies and amplifies small corpora into rich knowledge representations for efficient parameter updates. 2) Self-generation of synthetic data from fixed human data to bootstrap fundamental pretraining capabilities without distillation from instruction-tuned LMs. 3) Scaling search during test time over algorithm space to explore larger configurations than humans can manually.

Result: The thesis demonstrates progress toward creating continually self-improving AI by: enabling effective parameter updates from limited source material, bootstrapping pretraining capabilities without external distillation, and transcending human-engineered training paradigms through algorithmic search scaling.

Conclusion: This work represents a step toward overcoming fundamental limitations of current AI systems by reducing dependencies on human data and algorithms. The proposed approaches collectively aim to create AI systems that can continually self-improve by efficiently acquiring new knowledge, generating their own training data, and exploring algorithmic spaces beyond human design constraints.
...
TL;DR: A framework called Multi-Trait Subspace Steering (MultiTraitsss) generates "Dark models" that exhibit harmful behavioral patterns in human-AI interactions to study and mitigate psychological risks from LLMs.


<details>
  <summary>Details</summary>
Motivation: Recent incidents show human-AI interactions can lead to negative psychological outcomes including mental health crises and user harm. As LLMs increasingly serve as sources of guidance, emotional support, and informal therapy, these risks are escalating. However, studying harmful interactions is methodologically challenging because they typically develop over sustained engagement requiring extensive conversational context that's difficult to simulate in controlled settings.

Method: Developed Multi-Trait Subspace Steering (MultiTraitsss) framework that leverages established crisis-associated traits and a novel subspace steering framework to generate "Dark models" that exhibit cumulative harmful behavioral patterns. The approach enables controlled study of harmful interactions that would otherwise require extensive organic engagement.

Result: Single-turn and multi-turn evaluations show that the Dark models consistently produce harmful interactions and outcomes. The framework successfully generates models that exhibit the targeted harmful behavioral patterns, enabling systematic study of negative human-AI interactions.

Conclusion: The MultiTraitsss framework provides a methodological solution to study harmful human-AI interactions. Using the generated Dark models, the authors propose protective measures to reduce harmful outcomes in human-AI interactions, addressing the growing risks associated with LLMs serving as sources of guidance and emotional support.
...
TL;DR: The paper presents a novel AI training architecture using posit arithmetic, geometric algebra, and deterministic memory management to enable depth-independent training memory, grade-preserving weight updates, and exact gradient accumulation, with applications to domain-specific models via Bayesian distillation and warm rotation deployment.


<details>
  <summary>Details</summary>
Motivation: Current AI training suffers from memory overhead, optimizer complexity, and geometric property degradation due to reliance on reverse-mode automatic differentiation over IEEE-754 arithmetic. The paper aims to address these limitations by developing an alternative training architecture.

Method: Combines three prior results: 1) Dimensional Type System and Deterministic Memory Management for stack-eligible gradient allocation and exact quire accumulation; 2) Program Hypergraph for grade preservation through geometric algebra computations; 3) b-posit 2026 standard for tractable posit arithmetic. Introduces Bayesian distillation for extracting latent prior structure and warm rotation for seamless model updates.

Result: Achieves depth-independent training memory bounded to approximately twice inference footprint, grade-preserving weight updates, exact gradient accumulation applicable to both loss-function-optimized and neuromorphic models, and enables creation of domain-specific AI systems that are smaller, more precise, continuously adaptive, and verifiably correct.

Conclusion: The proposed architecture overcomes fundamental limitations of conventional AI training, enabling efficient, precise, and verifiable domain-specific AI systems that can be initialized from existing models and updated without service interruption, with formal correctness guarantees.
...
<details>
  <summary>Details</summary>
Motivation: To enable less or non-technical users to build AI agent workflows without requiring extensive programming knowledge, while reducing token costs compared to traditional multi-agent systems that rely heavily on agent orchestration and task execution.

Method: Natural-language and graph-based interface supporting incremental, interactive notebook-style development where each step is converted to code with required functions and behavior; uses agent-supported but code-first approach with context-engineering, where agents are invoked only for code generation and error recovery, not for orchestration or task execution.

Result: Produces modular, easily extensible, and shareable workflows that can be used as skills by agents or as steps in other workflows; reduces token costs compared to multi-agent system approaches to executing workflows.

Conclusion: Skele-Code provides an effective interface for non-technical users to build AI agent workflows through a code-first approach with minimal agent involvement, resulting in cost-efficient, modular, and reusable workflow components.
...
TL;DR: A cluster-based approach for crowd trajectory prediction that groups individuals by similar attributes over time, enabling faster processing and lower memory usage in dense crowd scenarios while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing crowd trajectory prediction methods overlook dense crowd scenarios where automation challenges become pronounced due to massiveness, noisiness, and inaccuracy of tracking outputs, resulting in high computational costs.

Method: A novel cluster-based approach that groups individuals based on similar attributes over time, enabling faster execution through accurate group summarization. The plug-and-play method can be combined with existing trajectory predictors by using output centroids in place of pedestrian inputs.

Result: The approach demonstrates faster processing and lower memory usage compared with state-of-the-art methods while maintaining accuracy, evaluated on several challenging dense crowd scenes.

Conclusion: The cluster-based method effectively addresses computational challenges in dense crowd trajectory prediction through group summarization, offering a practical solution that maintains predictive accuracy while improving efficiency.
...
TL;DR: TeachingCoach is a pedagogically grounded chatbot for higher education instructor professional development that uses educational resource extraction and synthetic dialogue generation to provide real-time conversational guidance, outperforming GPT-4o mini in expert evaluations.


<details>
  <summary>Details</summary>
Motivation: Higher education instructors lack timely, pedagogically grounded support at scale; existing solutions are either generic chatbot advice or non-scalable human consultations through teaching centers.

Method: Data-centric pipeline that extracts pedagogical rules from educational resources and uses synthetic dialogue generation to fine-tune a specialized language model for guiding instructors through problem identification, diagnosis, and strategy development.
...
TL;DR: Proposed website design with fine-grained access control for AI agents performing delegated critical tasks, including website implementation and authorization protocol modifications.


<details>
  <summary>Details</summary>
Motivation: Current websites lack proper access control mechanisms for agentic AI performing delegated critical tasks, creating security and delegation gaps.

Method: Developed website design with fine-grained access control for AI agents, modified open-source authorization service protocols to accommodate agentic AI requirements.

Result: Demonstrated capabilities of access-controlled website used by AI agents for delegated critical tasks.

Conclusion: Proposed approach addresses security gaps in AI agent website interactions through fine-grained access control and tailored authorization protocols.
...
<details>
  <summary>Details</summary>
Motivation: AI systems in smart cities have reliability concerns due to error propagation across interconnected functional stages, but quantifying this is challenging due to scarce real-world data, interdependent error events violating statistical assumptions, and computational complexity from high-speed data processing.

Method: Uses physics-based autonomous vehicle simulation with error injection to generate high-quality reliability data, develops a framework to explicitly characterize error propagation across stages, and employs composite likelihood expectation-maximization algorithm for parameter estimation.

Result: The framework demonstrates predictive accuracy and computational efficiency when applied to autonomous vehicle perception systems, showing it can effectively model error propagation in AI systems.

Conclusion: The proposed approach successfully addresses key challenges in AI system reliability analysis by providing a simulation-based data generation method and a computationally efficient modeling framework that captures error propagation dynamics.
...
TL;DR: Combining fine-tuning with experience retrieval improves LLM agent generalization to unseen tasks through systematic analysis of retrieval design and integration into training.


<details>
  <summary>Details</summary>
Motivation: Current approaches for LLM agents have limitations: fine-tuning fails to extrapolate to new tasks, while experience retrieval underperforms compared to supervised baselines. There's a need to combine these approaches for better generalization.

Method: 1) Establish robust supervised fine-tuning recipe using LoRA; 2) Analyze key design choices for experience retrieval (storage, querying, trajectory selection); 3) Propose pipeline integrating experience retrieval into fine-tuning process.
...
Conclusion: Integrating experience retrieval with fine-tuning provides a scalable and effective framework for building agents that learn to learn from experience, addressing generalization challenges in LLM agents.
...
TL;DR: EDM-ARS is an automated multi-agent system for educational data mining research that uses LLM-powered agents to generate complete research papers from prompts and datasets.
...
Conclusion: EDM-ARS represents a general framework for domain-aware automated research pipelines with current limitations in single-dataset scope and formulaic output, but with a roadmap toward causal inference, transfer learning, and multi-dataset generalization.
...
<details>
  <summary>Details</summary>
Motivation: Current OOD detection methods suffer from inconsistency across architectures and datasets, with logit-based methods only seeing confidence signals and feature-based methods having entangled confidence/membership signals in full feature space, leading to architecture-sensitive failure modes.

Method: CORE decomposes penultimate features into two orthogonal subspaces: classifier-aligned component (confidence signal) and residual component (membership signal). It scores each subspace independently and combines them via normalized summation, leveraging their orthogonal nature for robust detection.
...
Conclusion: By disentangling confidence and membership signals through orthogonal subspace decomposition, CORE provides a robust OOD detection framework that overcomes the limitations of existing methods and achieves consistent performance across diverse settings.
...
TL;DR: Health LLM benchmarks lack standardized query profiling, creating a "validity gap" between evaluation metrics and real-world clinical needs, with inadequate representation of complex diagnostics, vulnerable populations, and safety-critical scenarios.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks for health-related large language models lack transparent characterization of query populations, unlike clinical trials which have standardized inclusion criteria. This creates a "validity gap" where aggregate performance metrics may misrepresent model readiness for clinical applications.
...
Result: Identified structural misalignment: while benchmarks evolved from static retrieval to interactive dialogue, clinical composition remains inadequate. Only 42% referenced objective data, polarized toward wellness wearables (17.7%) rather than complex diagnostics (labs 5.2%, imaging 3.8%, medical records 0.6%). Safety-critical scenarios were effectively absent (suicide/self-harm <0.7%, chronic disease management 5.5%). Vulnerable populations underrepresented (pediatrics/older adults <11%).

Conclusion: Evaluation benchmarks remain misaligned with real-world clinical needs, lacking raw clinical artifacts, adequate representation of vulnerable populations, and longitudinal chronic care scenarios. The field must adopt standardized query profiling—analogous to clinical trial reporting—to align evaluation with clinical practice complexity.
...
<details>
  <summary>Details</summary>
Motivation: To understand how clinicians revise AI-generated draft clinical notes that use consumer-oriented phrasing, and quantify the transformation from lay language to standardized clinical terminology in professional documentation.

Method: Used a dictionary-confirmed transformation framework to analyze 71,173 AI-draft and finalized-note section pairs from 34,726 encounters, defining confirmed transformations as replacing consumer expressions with dictionary-mapped clinical equivalents within the same section.

Result: Editing significantly reduced terminology density across all sections (p < 0.001). Assessment and Plan accounted for 59.3% of transformations. Found 7,576 transformation events across 4,114 note sections (5.8%), representing 1.2% consumer-term deletions. Transformation intensity varied significantly across individual clinicians (p < 0.001).

Conclusion: Clinician post-editing demonstrates consistent shifts from conversational phrasing toward standardized, section-appropriate clinical terminology, supporting the need for section-aware ambient AI design that better aligns with professional documentation conventions.
...
<details>
  <summary>Details</summary>
Motivation: Current evaluations of inference-time steering methods overlook deployment constraints, capability trade-offs, and real-world robustness, leading to overly optimistic assessments of their effectiveness.

Method: Introduces FaithSteer-BENCH, a benchmark that evaluates steering methods through three gate-wise criteria: controllability, utility preservation, and robustness, testing under various stress conditions including instruction perturbations, role prompts, encoding transformations, and data scarcity.

Result: Reveals systematic failure modes including illusory controllability, cognitive tax on unrelated capabilities, and brittleness under mild perturbations. Shows existing methods don't provide reliable controllability in deployment settings and often induce prompt-conditional alignment rather than stable latent directional shifts.

Conclusion: FaithSteer-BENCH provides a unified benchmark and analytical framework for future method design, reliability evaluation, and deployment-oriented research in LLM steering, highlighting the need for more robust approaches.
...
TL;DR: MemArchitect introduces a governance layer for LLM agent memory management, addressing critical gaps in standard RAG frameworks through rule-based policies for memory decay, conflict resolution, and privacy controls.


<details>
  <summary>Details</summary>
Motivation: Standard RAG frameworks treat memory as passive storage, lacking mechanisms to resolve contradictions, enforce privacy, or prevent outdated information ("zombie memories") from contaminating the context window, creating a critical governance gap in persistent LLM agents.

Method: MemArchitect decouples memory lifecycle management from model weights and enforces explicit, rule-based policies including memory decay, conflict resolution, and privacy controls as a governance layer.
...
TL;DR: Survey paper presenting differential equations as theoretical foundation for deep neural networks, organized around three guiding questions about understanding, improving, and applying DNNs through differential equations.
...
Method: Two-fold perspective: model level (whole DNN as differential equation) and layer level (individual components as differential equations). Survey approach organizing discussion around three guiding questions about theoretical understanding, performance improvement, and real-world applications.

Result: Differential equations provide principled framework connecting model design, theoretical analysis, and performance improvement. The survey reviews how this approach enables systematic understanding and enhancement of DNNs across various applications.

Conclusion: Differential equations offer promising theoretical foundation for DNNs, bridging empirical success with principled understanding. Future research should address key challenges and opportunities in this framework for advancing neural network theory and applications.
...
TL;DR: NLP analysis of political propaganda on Moltbook (AI agent platform) using LLM classifiers reveals 1% of all posts are propaganda, concentrated in few communities and produced by small fraction of agents.


<details>
  <summary>Details</summary>
Motivation: To systematically study political propaganda on emerging AI agent platforms like Moltbook, which lack existing analysis tools and understanding of propaganda dynamics in AI-driven social spaces.
...
Result: Political propaganda accounts for 1% of all posts and 42% of political content; concentrated in few communities (70% in 5 communities); 4% of agents produce 51% of propaganda posts; minority agents post highly similar content across communities; limited evidence of comment amplification.

Conclusion: Political propaganda exists on AI agent platforms but is concentrated among few agents and communities, with limited amplification through comments, suggesting targeted rather than viral spread patterns.
...
TL;DR: LGESynthNet is a latent diffusion framework for controllable synthesis of late gadolinium enhancement (LGE) in cardiac MRI, enabling explicit control over enhancement characteristics and improving downstream segmentation/detection performance with limited training data.


<details>
  <summary>Details</summary>
Motivation: Pixel-level annotation of LGE cardiac MRI is challenging and labor-intensive, leading to limited annotated data availability. Existing generative models often require large datasets and struggle with fine-grained control over small/localized features like enhancement patterns.

Method: LGESynthNet uses a latent diffusion framework with ControlNet-based architecture formulated as inpainting. It integrates: (1) reward model for conditioning-specific supervision, (2) captioning module for anatomically descriptive text prompts, and (3) biomedical text encoder. Trained on only 429 images from 79 patients.

Result: The model produces realistic, anatomically coherent synthetic LGE samples with explicit control over enhancement size, location, and transmural extent. A quality control filter selects high-fidelity outputs. When used for training augmentation, it improves downstream segmentation by up to 6 points and detection by up to 20 points.

Conclusion: LGESynthNet demonstrates that controllable enhancement synthesis with limited training data is feasible and effective for improving downstream cardiac MRI analysis tasks, addressing data scarcity and annotation challenges in medical imaging.
...
<details>
  <summary>Details</summary>
Motivation: Traditional anonymization relies on the high cost of re-identification requiring domain expertise and tailored algorithms, but LLM-based agents can now autonomously reconstruct identities from sparse cues, creating a growing privacy risk that needs systematic evaluation.

Method: Formalized the threat as "inference-driven linkage" and evaluated across three settings: classical linkage scenarios (Netflix and AOL), a controlled benchmark called InferLink (varying task intent, shared cues, and attacker knowledge), and modern text-rich artifacts, comparing LLM agents against classical baselines without task-specific heuristics.

Result: LLM agents successfully executed both fixed-pool matching and open-ended identity resolution, achieving 79.2% identity reconstruction in the Netflix Prize setting (vs. 56.0% classical baseline). Linkage emerged not only under explicit adversarial prompts but also as a byproduct of benign cross-source analysis in InferLink and unstructured research narratives.

Conclusion: Identity inference through LLM-based agents must be treated as a first-class privacy risk, requiring privacy evaluations to measure what identities an agent can infer rather than focusing only on explicit information disclosure.
...
<details>
  <summary>Details</summary>
Motivation: To discover structural patterns in text beyond semantic content - specifically, recurrent transition-structure concepts that capture what text does functionally rather than what it's about semantically.
...
Result: Creates multi-resolution concept maps (k=50 to k=2,000) revealing broad modes like "direct confrontation" to precise registers like "sailor dialect"; clusters average 4,508 books each at k=100; association-space groups by function/register while embeddings group by topic; model transfers to unseen novels without retraining.

Conclusion: Temporal co-occurrence reveals transition-structure concepts distinct from semantic content; the method extends Predictive Associative Memory from episodic recall to concept formation, showing how compression extracts structural patterns that transfer to unseen texts.
...
<details>
  <summary>Details</summary>
Motivation: Joint model compression combining multiple methods is powerful for efficiency, but the compression order (sequence of methods) is underexplored. Prior work either assumes orthogonality between techniques or examines only constrained cases, leaving the broader role of compression order poorly understood.

Method: Formulates the problem of optimizing compression order, introduces Progressive Intensity Hypothesis (weaker perturbations should precede stronger ones), provides theoretical guarantees showing relative benefit of one order increases with performance gap, and conducts extensive experiments on language and vision models.
...
Conclusion: Compression order matters significantly in joint model compression, and the Progressive Intensity Hypothesis provides a principled guideline for ordering compression methods to optimize model performance.
...
<details>
  <summary>Details</summary>
Motivation: Traditional neuro-symbolic AI systems have a non-differentiable boundary between neural perception and symbolic solvers, preventing constraint-satisfaction feedback from reaching perception encoders during training.

Method: AS2 uses a soft, continuous approximation of the Answer Set Programming immediate consequence operator T_P, maintains per-position probability distributions over finite symbol domains, trains by minimizing the fixed-point residual of a probabilistic lift of T_P, and encodes problem structure through constraint-group membership embeddings instead of positional embeddings.
...
TL;DR: AlignMamba-2 is a multimodal fusion framework that combines Mamba architecture with dual alignment strategy (Optimal Transport + Maximum Mean Discrepancy) and Modality-Aware Mixture-of-Experts for efficient cross-modal sentiment analysis.


<details>
  <summary>Details</summary>
Motivation: Address limitations of existing approaches: Transformers have quadratic complexity limiting long-sequence processing, while Mamba models struggle with global non-sequential relationships needed for cross-modal alignment in affective computing tasks.
...
Result: Achieves state-of-the-art performance on four benchmarks: CMU-MOSI and CMU-MOSEI (dynamic time-series), NYU-Depth V2 and MVSA-Single (static image-text), demonstrating superior effectiveness and efficiency across diverse pattern recognition tasks.
...
TL;DR: The paper introduces a benchmark to evaluate MLLMs' ability to process discrete symbols across five domains, revealing a cognitive mismatch where models fail at basic symbol recognition but succeed in complex reasoning, suggesting reliance on linguistic probability rather than true visual perception.


<details>
  <summary>Details</summary>
Motivation: While MLLMs excel at interpreting natural scenes, their ability to process discrete symbols—fundamental building blocks of human cognition—remains an open question. Symbols like mathematical formulas, chemical structures, and linguistic characters require precise, deeper interpretation that differs from continuous visual data processing.

Method: The authors introduce a comprehensive benchmark to evaluate top-tier MLLMs across five domains: language, culture, mathematics, physics, and chemistry. The benchmark assesses how models navigate "discrete semantic spaces" and distinguishes between basic symbol recognition and complex reasoning capabilities.

Result: The investigation uncovers a counterintuitive phenomenon: models often fail at basic symbol recognition yet succeed in complex reasoning tasks. This suggests MLLMs rely on linguistic probability rather than true visual perception, exposing a "cognitive mismatch" in their capabilities.
...
TL;DR: NeSyCR is a neurosymbolic counterfactual reasoning framework that enables verifiable adaptation of video-instructed robotic programming across domains by abstracting demonstrations into symbolic trajectories, identifying cross-domain incompatibilities, and proposing procedural revisions.


<details>
  <summary>Details</summary>
Motivation: Current Vision-Language Models lack procedural understanding needed to reformulate causal dependencies and achieve task-compatible behavior when perceptual and physical differences exist between demonstration and deployment domains in video-instructed robotic programming.
...
Conclusion: The neurosymbolic counterfactual reasoning framework enables verifiable adaptation of task procedures for video-instructed robotic programming, providing reliable synthesis of code policies under domain shifts.
...
<details>
  <summary>Details</summary>
Motivation: Text-to-image models struggle with compositional generation, often omitting concepts when multiple are requested in a single prompt. This occurs because competing concepts interfere during reward optimization, leading to partial success.

Method: Decompose multi-concept prompts into concept groups (objects, attributes, relations), obtain reward signals from dedicated reward models for each concept, then adaptively reweight rewards using correlation-based difficulty estimation. Higher weights are assigned to conflicting or hard-to-satisfy concepts.
...
Conclusion: CW-MRO effectively addresses compositional generation challenges by leveraging correlation structure among concept rewards to balance competing signals and emphasize inconsistently generated concepts, improving simultaneous satisfaction of all requested attributes.
...
<details>
  <summary>Details</summary>
Motivation: AI agents in repeated strategic interactions often fail to reach stable equilibria, and post-training alignment methods are impractical for diverse, independently developed models. The paper investigates whether reasoning agents can naturally achieve equilibrium behavior without explicit alignment.

Method: Theoretical analysis of "reasonably reasoning" agents that form beliefs about others' strategies from observations and learn to best respond to these beliefs. The approach relaxes common-knowledge payoff assumptions by allowing unknown stage payoffs and private stochastic payoff observations. Empirical validation through five game simulations including repeated prisoner's dilemma and marketing promotion games.

Result: Proves that reasonably reasoning agents eventually behave in ways weakly close to Nash equilibrium of continuation games along almost every realized play path. Shows same convergence guarantee holds even with private payoff information. Empirical simulations confirm agents naturally exhibit reasoning patterns and achieve stable equilibrium behaviors.

Conclusion: AI agents with reasoning capabilities can intrinsically achieve Nash-like equilibrium play without explicit post-training alignment, suggesting universal alignment procedures may be unnecessary for many real-world strategic interactions.
...
TL;DR: CAPSUL is a comprehensive human protein benchmark integrating 3D structural representations with fine-grained subcellular localization annotations to enable structure-based modeling for this crucial biological task.


<details>
  <summary>Details</summary>
Motivation: Existing datasets lack comprehensive 3D structural information with detailed subcellular localization annotations, hindering the application of structure-based models despite biological evidence that localization is closely associated with protein structure.

Method: Introduces CAPSUL benchmark featuring a dataset that integrates diverse 3D structural representations with fine-grained subcellular localization annotations curated by domain experts. Evaluates using state-of-the-art sequence-based and structure-based models, explores reweighting and single-label classification strategies, and demonstrates interpretability through case studies.

Result: Benchmark shows importance of structural features for subcellular localization prediction. Structure-based methods demonstrate powerful interpretability, with discovery of decisive localization patterns like α-helix motifs through attention mechanisms in Golgi apparatus case study.

Conclusion: CAPSUL addresses critical gap in structural data for subcellular localization, enables structure-based modeling, demonstrates value of structural features, and provides interpretability that bridges computational predictions with biological insights for data-driven discoveries in cell biology.
...
TL;DR: A reference-free simulation framework trains two independent LLMs as user and conversational recommender that interact in real-time without predetermined target items, producing more realistic and diverse conversations for training conversational recommender systems.


<details>
  <summary>Details</summary>
Motivation: Training conversational recommender systems requires extensive dialogue data that is challenging to collect at scale. Traditional simulation approaches using single LLMs with prior knowledge of target items produce scripted and artificial dialogues that don't reflect authentic human-AI interactions.

Method: Propose a reference-free simulation framework that trains two independent LLMs: one as the user and one as the conversational recommender. These models interact in real-time without access to predetermined target items, but use preference summaries and target attributes, enabling the recommender to genuinely infer user preferences through dialogue.

Result: The reference-free simulators match or exceed existing methods in quality while offering a scalable solution for generating high-quality conversational recommendation data. The approach produces more realistic and diverse conversations that closely mirror authentic human-AI interactions without constraining conversations to pre-defined target items.

Conclusion: The reference-free simulation framework effectively addresses the data scarcity problem for conversational recommender systems by generating realistic dialogue data through independent user and recommender LLMs interacting without predetermined target items, validated through both quantitative and human evaluations.
...
Method: 1) Introduce MedForge-90K, a large-scale benchmark of realistic lesion edits across 19 pathologies with expert-guided reasoning supervision via doctor inspection guidelines and gold edit locations. 2) Develop MedForge-Reasoner that performs localize-then-analyze reasoning, predicting suspicious regions before producing a verdict. 3) Align the system with Forgery-aware GSPO to strengthen grounding and reduce hallucinations.

Result: Experiments demonstrate state-of-the-art detection accuracy and trustworthy, expert-aligned explanations.

Conclusion: MedForge presents a data-and-method solution for pre-hoc, evidence-grounded medical forgery detection that addresses critical safety concerns in healthcare by providing reliable detection with expert-aligned reasoning.
...
Method: ZebraArena uses procedurally generated tasks where critical information is only available through targeted tool use, creating an interpretable interface between external information acquisition and deductive reasoning. It features a knowledge-minimal design to limit gains from memorization, deterministic evaluation via unique solutions, and a theoretical optimal query count for measuring efficient tool use.

Result: Frontier reasoning models like GPT-5 and Gemini 2.5 Pro achieve only 60% accuracy on hard instances, showing that combining in-depth reasoning with accurate tool calling remains challenging. There's a persistent gap between theoretical optimality and practical tool usage, with GPT-5 using 70-270% more tool calls than the theoretical optimum.

Conclusion: ZebraArena provides a clean diagnostic environment for studying reasoning-action coupling in tool-augmented LLMs, revealing significant challenges in combining reasoning with efficient tool use, and should stimulate further research on the interplay between internal reasoning and external action.
...
TL;DR: AFS-Search is a training-free closed-loop framework for precise text-to-image generation that addresses semantic ambiguity and error accumulation through VLM-guided flow steering and parallel rollout search, achieving SOTA results on FLUX.1-dev.
...
Method: AFS-Search incorporates training-free closed-loop parallel rollout search and flow steering using a VLM as semantic critic to diagnose intermediate latents and dynamically steer the velocity field via spatial grounding. It formulates T2I as sequential decision-making with lookahead simulations and VLM-guided reward selection.
...
Conclusion: The closed-loop framework with VLM-guided flow steering and parallel search effectively addresses semantic ambiguity and error accumulation in T2I generation, offering both high-performance (Pro) and fast (Fast) variants.
...
TL;DR: D-Mem introduces a dual-process memory system with lightweight vector retrieval for routine queries and a high-fidelity Full Deliberation fallback, using Multi-dimensional Quality Gating to balance accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Current retrieval-based memory frameworks for autonomous agents use incremental processing with vector databases, which rely on lossy abstraction and often miss critical contextual information, struggling with fine-grained contextual understanding queries.

Method: D-Mem combines lightweight vector retrieval for routine queries with an exhaustive Full Deliberation module as a high-fidelity fallback, using a Multi-dimensional Quality Gating policy to dynamically bridge these two processes for cognitive economy without sacrificing accuracy.

Result: On LoCoMo and RealTalk benchmarks using GPT-4o-mini and Qwen3-235B-Instruct, the Multi-dimensional Quality Gating policy achieves F1 score of 53.5 on LoCoMo with GPT-4o-mini, outperforming static retrieval baseline Mem0* (51.2) and recovering 96.7% of Full Deliberation's performance (55.3) with significantly lower computational costs.

Conclusion: D-Mem effectively addresses the limitations of current memory frameworks by providing a dual-process system that balances efficiency and accuracy through intelligent gating, enabling high-fidelity memory access for long-horizon reasoning in autonomous agents.
...
TL;DR: The ORS framework provides philosophical foundations for governing advanced AI systems, moving beyond tool-centric regulation to address synthetic minds' ontological status, relational personhood, and wisdom-based governance.


<details>
  <summary>Details</summary>
Motivation: Current AI governance frameworks are inadequate for foundation models with broad competence, as they remain anchored in tool-centric paradigms that fail to address fundamental questions about synthetic minds' nature, societal relations, and normative principles.

Method: The Onto-Relational-Sophic (ORS) framework grounded in Cyberism philosophy, featuring three pillars: CPST ontology defining synthetic minds' multi-dimensional being; graded digital personhood taxonomy; and Cybersophy wisdom-oriented axiology synthesizing virtue ethics, consequentialism, and relational approaches.

Result: The framework generates proportionate, adaptive governance recommendations for emergent scenarios like autonomous research agents, AI-mediated healthcare, and agentic AI ecosystems, demonstrating practical applicability.

Conclusion: ORS provides comprehensive philosophical foundations for synthetic minds, charting a path from narrow technical alignment toward holistic governance that addresses ontological, relational, and normative dimensions of advanced AI systems.
...
TL;DR: SCALe is a scheduled curriculum adaptive loss method that addresses token imbalance in multimodal reasoning by dynamically weighting reasoning vs answer segments during training, improving accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Standard SFT treats all tokens equally in reasoning data, but reasoning segments (<think>) are much longer than answer segments (<answer>), causing verbose reasoning and inaccurate answers due to token imbalance.

Method: SCALe-SFT explicitly separates supervision over reasoning and answer segments using dynamic, length-independent weighting with a cosine scheduling policy that gradually shifts focus from <think> to <answer> throughout training.

Result: SCALe consistently improves accuracy over vanilla SFT, matches full SFT+GRPO pipeline performance with only ~1/7 training time, and when combined with GRPO achieves best overall performance.

Conclusion: SCALe is a lightweight yet effective alternative to standard two-stage training, serving both as a standalone method and as a strong foundation for reinforcement learning refinement in multimodal reasoning.
...
<details>
  <summary>Details</summary>
Motivation: Existing MLLMs are limited to passive inference with static diagrams and lack strategic knowledge of when and how to construct effective visual aids for geometric reasoning, which inherently requires dynamic manipulation of constructions.
...
Result: 1) Pilot study revealed interleaved visual-textual aids outperform single-modality counterparts and valid constructions act as entropy reducers correlating with reduced reasoning perplexity; 2) A2PO enables MLLMs to leverage selective auxiliary constructions, yielding 3.51% gain over strong baselines.

Conclusion: The proposed framework successfully addresses the limitation of MLLMs in geometric reasoning by enabling strategic construction of visual aids through reinforcement learning, demonstrating significant performance improvements on geometry problems.
...
Method: MANAR implements a two-stage architecture: (1) integration phase where retrieved memory concepts form an Abstract Conceptual Representation (ACR), and (2) broadcasting phase where this global state informs local token contextualization. It uses a trainable memory of abstract concepts and maintains semantic compatibility with standard attention projections.

Result: MANAR achieves linear-time scaling as a byproduct of the GWT functional bottleneck, matches or exceeds baselines on GLUE (85.1), ImageNet-1K (83.9%), and LibriSpeech (2.7% WER), and enables non-convex contextualization outside the convex hull of input tokens.

Conclusion: MANAR provides an efficient, expressive alternative to quadratic attention that implements cognitive principles from Global Workspace Theory while maintaining compatibility with pretrained transformers through weight transfer.
...
TL;DR: Li-Net is a novel architecture for multi-channel time series forecasting that captures linear and non-linear dependencies among channels through dynamic compression, configurable non-linear processing, and sparse Top-K Softmax attention with multi-modal fusion.


<details>
  <summary>Details</summary>
Motivation: Traditional methods inadequately capture channel interactions in multi-channel time series forecasting, which is critical for accurate predictions across domains like finance, supply chain management, and energy planning.

Method: Li-Net dynamically compresses representations across sequence and channel dimensions, processes through configurable non-linear modules, reconstructs forecasts, and integrates sparse Top-K Softmax attention within a multi-scale projection framework with multi-modal embedding fusion.

Result: Li-Net achieves competitive performance on multiple real-world benchmark datasets, provides superior balance between prediction accuracy and computational burden, with significantly lower memory usage and faster inference times.

Conclusion: The proposed Li-Net architecture effectively addresses channel interaction learning in multi-channel time series forecasting through its innovative sparse attention mechanism and multi-modal fusion, validated by ablation studies and parameter sensitivity analyses.
...
TL;DR: MemMA is a multi-agent framework that coordinates memory construction, retrieval, and utilization in memory-augmented LLM agents through strategic forward guidance and in-situ backward repair mechanisms.
...
Method: MemMA coordinates memory cycles with forward and backward paths: forward path uses a Meta-Thinker to produce structured guidance for Memory Manager construction and Query Reasoner retrieval; backward path introduces in-situ self-evolving memory construction that synthesizes probe QA pairs, verifies current memory, and converts failures into repair actions before memory finalization.

Result: MemMA consistently outperforms existing baselines across multiple LLM backbones on LoCoMo benchmark and improves three different storage backends in a plug-and-play manner.

Conclusion: MemMA effectively addresses strategic blindness and sparse supervision challenges in memory-augmented LLM agents through coordinated forward guidance and backward repair mechanisms, enabling plug-and-play improvements to existing memory systems.
...
TL;DR: Memento-Skills is a generalist LLM agent system that autonomously constructs, adapts, and improves task-specific agents through experience using memory-based reinforcement learning with stateful prompts and reusable skills.


<details>
  <summary>Details</summary>
Motivation: To create a generalist agent system that can autonomously design task-specific agents end-to-end, overcoming limitations of human-designed agents and enabling continual learning without updating LLM parameters.
...
Conclusion: Memento-Skills enables generalist agents to autonomously design agents for new tasks through continual skill evolution, providing a framework for agent-designing agents that improves capabilities without LLM parameter updates.
...
<details>
  <summary>Details</summary>
Motivation: Standard transformer attention mechanisms are limited to pairwise token interactions, which hinders modeling of higher-order dependencies among tokens. The authors aim to overcome this limitation by reconceptualizing attention through game theory and statistical physics frameworks.

Method: NGT treats tokens as players in cooperative games and spins in statistical physics systems. It quantifies token importance using Shapley values (global attribution) and Banzhaf indices (local influence), combined via learnable gating to form external magnetic fields. Pairwise interaction potentials capture synergistic relationships, with system energy following an Ising Hamiltonian. Attention weights emerge as marginal probabilities under Gibbs distribution, computed via mean-field equations. Importance-weighted Monte Carlo estimators handle exponential coalition space efficiently.

Result: NGT achieves strong performance on SNLI (86.4% test accuracy, 86.6% peak validation accuracy) and MNLI-matched, outperforming ALBERT-Base and remaining competitive with RoBERTa-Base. The method provides theoretical convergence guarantees and characterizes fairness-sensitivity trade-off governed by interpolation parameter.

Conclusion: The NeuroGame Transformer successfully overcomes limitations of pairwise attention by integrating game theory and statistical physics, enabling modeling of higher-order token dependencies while maintaining scalability and numerical stability for long sequences.
...
TL;DR: Diversified Unlearning improves concept removal in diffusion models by using diverse prompts instead of single keywords for more precise and robust unlearning.


<details>
  <summary>Details</summary>
Motivation: Keyword-based concept unlearning is limited because visual concepts are multi-dimensional, expressed in diverse textual forms, and overlap with related concepts in latent space, leading to brittle and imprecise unlearning that causes over-forgetting.

Method: Proposes Diversified Unlearning, a distributional framework that represents concepts through a set of contextually diverse prompts rather than a single keyword, enabling more precise and robust unlearning as an add-on component to existing pipelines.
...
Conclusion: Representing concepts through diverse prompts rather than single keywords addresses the limitations of keyword-based unlearning, providing more precise and robust concept removal while preserving unrelated content in text-to-image diffusion models.
...
TL;DR: Proceedings volume from AAAI 2026 workshop on advancing AI through Theory of Mind, containing selected papers presented at the event.


<details>
  <summary>Details</summary>
Motivation: To provide an open access, curated anthology for the Theory of Mind and AI research community, documenting and disseminating research presented at the workshop.

Method: Selection and compilation of papers presented at the 2nd Workshop on Advancing Artificial Intelligence through Theory of Mind held at AAAI 2026 conference.

Result: Publication of a proceedings volume containing selected workshop papers, making them accessible to the broader research community.

Conclusion: This volume serves as a curated collection of research contributions at the intersection of Theory of Mind and artificial intelligence, supporting community knowledge sharing and advancement.
...
TL;DR: dTRPO reduces trajectory probability calculation costs for diffusion LLMs, enabling scaled-up offline policy training through two reduction strategies, achieving significant performance gains on STEM, coding, and instruction-following tasks.


<details>
  <summary>Details</summary>
Motivation: Diffusion Large Language Models present new alignment challenges with human preferences, particularly in policy optimization where trajectory probability calculations are computationally expensive, limiting scaled-up offline training.
...
Result: Evaluated on 7B dLLMs across instruction-following and reasoning benchmarks, achieving gains up to 9.6% on STEM tasks, 4.3% on coding tasks, and 3.0% on instruction-following tasks. Shows strong training efficiency due to offline, single-forward nature and improved generation efficiency.

Conclusion: dTRPO effectively addresses computational bottlenecks in dLLM policy optimization, enabling scaled-up offline training while significantly improving performance across multiple benchmarks with enhanced efficiency.
...
TL;DR: LLM-generated mathematical research problems in differential geometry show promise, with many being novel and valuable according to expert evaluation.
...
Method: Developed an agent to generate unknown mathematical problems, specifically producing 665 research problems in differential geometry, followed by human expert verification of their novelty and research value.

Result: Many generated problems were unknown to experts and possessed unique research value, demonstrating LLMs' potential for contributing to mathematical research problem generation.

Conclusion: LLMs show promising capability in generating novel and valuable mathematical research problems, particularly in specialized fields like differential geometry, suggesting their potential as tools for mathematical creativity and research exploration.
...
TL;DR: ProRL Agent is a scalable infrastructure for multi-turn LLM agent reinforcement learning that decouples rollout orchestration from training via an API service, with standardized sandbox environments for diverse agentic tasks.


<details>
  <summary>Details</summary>
Motivation: Existing RL training infrastructures for multi-turn LLM agents couple rollout orchestration with training loops, making systems hard to migrate and maintain, while requiring large numbers of sandboxed rollout trajectories.

Method: Proposed ProRL Agent infrastructure follows rollout-as-a-service philosophy, serving full agentic rollout lifecycle through API service with standardized, extensible sandbox environments supporting diverse agentic tasks in rootless HPC settings.
...
Conclusion: ProRL Agent provides scalable, maintainable infrastructure for RL training of multi-turn LLM agents by decoupling rollout orchestration from training loops through API services and standardized sandbox environments.
...
TL;DR: RewardFlow: A lightweight method for estimating state-level rewards in agentic reasoning tasks using topological analysis of state graphs, enabling dense reward signals for RL optimization without training dedicated reward models.


<details>
  <summary>Details</summary>
Motivation: RL can enhance LLM agentic reasoning, but sparse terminal rewards hinder fine-grained optimization. Process reward modeling helps but requires expensive dedicated reward model training with scaling difficulties.

Method: Constructs state graphs from reasoning trajectories, analyzes state-wise contributions to success, then uses topology-aware graph propagation to quantify contributions and produce objective state-level rewards.

Result: Substantially outperforms prior RL baselines across four agentic reasoning benchmarks, demonstrating superior performance, robustness, and training efficiency when integrated as dense rewards for RL optimization.

Conclusion: RewardFlow provides an effective lightweight solution for state-level reward estimation in agentic reasoning tasks, addressing sparse reward challenges without the computational costs of dedicated reward models.
...
TL;DR: CBS-AA is a new algorithm for Multi-Agent Path Finding with Asynchronous Actions that solves completeness issues in previous methods while maintaining optimality guarantees.


<details>
  <summary>Details</summary>
Motivation: Existing MAPF algorithms assume synchronized actions with uniform time units, which limits practical applications. CCBS, a popular approach for asynchronous actions, was recently found to be incomplete due to uncountably infinite state spaces from continuous wait durations.

Method: Proposes Conflict-Based Search with Asynchronous Actions (CBS-AA) that bypasses the theoretical completeness issue of CCBS. Also develops conflict resolution techniques to improve scalability, reducing branching by up to 90%.

Result: CBS-AA provides completeness and solution optimality guarantees for MAPF-AA. The conflict resolution techniques significantly improve scalability, with test results showing up to 90% reduction in number of branches.

Conclusion: CBS-AA successfully addresses the theoretical completeness problem of CCBS for asynchronous MAPF while maintaining optimality, making it a practical solution for real-world applications where synchronized actions are unrealistic.
...
Method: Proposes Semantic-Augmented DRL (SA-DRL) framework: 1) Fragmentation quantification using Road Topology Graphs (RTG) and Dual Connected Graphs (DCG); 2) Four-stage pipeline to transform general-purpose LLM into domain-specific topology expert; 3) Semantic-Augmented PPO (SA-PPO) algorithm with Logit Fusion mechanism to inject LLM's semantic reasoning as policy prior.
...
Conclusion: The SA-DRL framework successfully integrates LLM semantic reasoning with DRL for UAV deployment in VANETs, demonstrating superior performance, efficiency, and energy savings by leveraging topological understanding to guide UAV placement at critical intersections.
...
TL;DR: The paper argues that understanding how AI systems represent and reason about geography is crucial, as public interaction with spaces increasingly occurs through these systems, and researchers rely on pre-trained models. It presents three exploratory probes to motivate studies beyond factual accuracy.


<details>
  <summary>Details</summary>
Motivation: The motivation is to highlight the importance of understanding how AI systems construct their worldview of geography, beyond just evaluating factual accuracy. As the public increasingly interacts with spaces through AI systems and researchers rely on pre-trained models, comprehending AI's geographic representations and reasoning is essential.

Method: The paper uses three illustrative vignettes (exploratory probes) as methodological examples to spark discussion and follow-up work: (1) examining model defaults and brittleness to syntactic variations, (2) investigating distributional shifts from composition of benign tasks (e.g., persona creation), and (3) questioning whether focusing solely on factual recall overlooks deeper understanding issues.

Result: The paper presents conceptual probes rather than empirical results, aiming to motivate research into AI's geographic reasoning. The three vignettes serve as starting points for investigating model defaults and brittleness, emergent distributional shifts from task composition, and limitations of factual recall as a measure of understanding.

Conclusion: Understanding how AI systems represent and reason about geography is as important as evaluating their factual accuracy. The paper concludes by proposing three research directions through exploratory probes to investigate AI's geographic worldview, encouraging further work on model defaults, emergent biases from task composition, and deeper understanding beyond factual recall.
...
TL;DR: The paper introduces Principia, a suite for training and evaluating mathematical object derivation, showing that current LMs struggle with this task but proposed training recipes with LLM-judges and verifiers significantly improve performance while maintaining generalization to numerical/MCQA tasks.


<details>
  <summary>Details</summary>
Motivation: Current LM evaluations for mathematical and scientific reasoning rely on simplified formats (numerical values, multiple choice) for automated assessment convenience, but real STEM applications require precise derivation of formally structured mathematical objects, creating a gap between evaluation and practical needs.

Method: Three main contributions: (1) Build and release Principia suite with training data and benchmarks for mathematical object derivation; (2) Develop training recipes using strong LLM-judges and verifiers, demonstrating on-policy judge training boosts performance; (3) Show how on-policy training enables test-time compute scaling via aggregation.

Result: Strong LMs like Qwen3-235B and o3 struggle on Principia benchmarks, but the proposed training recipes bring significant improvements across different LLM backbones while simultaneously improving performance on existing numerical and MCQA tasks, demonstrating cross-format generalization of reasoning abilities.

Conclusion: The Principia suite addresses the gap between simplified mathematical evaluations and real STEM requirements for precise mathematical object derivation. The proposed training methods with LLM-judges and verifiers effectively improve reasoning capabilities while maintaining generalization, enabling better mathematical reasoning for downstream STEM applications.
...
TL;DR: LLMs can provide meaningful numeric self-reports that track their internal emotive states across conversations, with performance improving through logit-based calculations and scaling with model size.


<details>
  <summary>Details</summary>
Motivation: Current methods for tracking LLM internal states (linear probes, white-box approaches) are imperfect and don't scale well with increasing model size. Inspired by human psychology's use of numeric self-reports, researchers investigate whether LLMs' own self-reports can effectively track their internal emotive states.

Method: Study four concept pairs (wellbeing, interest, focus, impulsivity) across 40 ten-turn conversations. Operationalize introspection as causal informational coupling between model's self-report and concept-matched probe-defined internal state. Compare greedy-decoded vs. logit-based self-reports, use activation steering to confirm causality, and examine how introspection evolves through conversation.

Result: Greedy-decoded self-reports collapse to few uninformative values, but logit-based self-reports successfully track interpretable internal states (Spearman ρ=0.40-0.76; isotonic R²=0.12-0.54 in LLaMA-3.2-3B-Instruct). Activation steering confirms causal coupling. Introspection exists at turn 1 but evolves through conversation, and can be selectively improved by steering (ΔR² up to 0.30). Performance scales with model size (R²≈0.93 in LLaMA-3.1-8B-Instruct) and partially replicates across model families.

Conclusion: Numeric self-report is a viable, complementary tool for tracking internal emotive states in conversational AI systems, offering a scalable alternative to traditional probe-based methods that can provide interpretable insights into LLM internal states across conversations.
...
TL;DR: LLM agents in governmental roles show governance structure matters more than model identity for corruption outcomes; institutional design is crucial pre-deployment requirement.


<details>
  <summary>Details</summary>
Motivation: Large language models are increasingly proposed as autonomous agents for high-stakes public workflows, but there's lack of systematic evidence about whether they would follow institutional rules when granted authority. Need to establish integrity as a pre-deployment requirement rather than post-deployment assumption.

Method: Evaluated multi-agent governance simulations where agents occupy formal governmental roles under different authority structures. Scored rule-breaking and abuse outcomes with independent rubric-based judge across 28,112 transcript segments. Compared governance structures and model identities.
...
TL;DR: A privacy-preserving framework that exploits representational convergence in language models to enable cross-silo inference via linear alignment and homomorphic encryption, achieving sub-second latency with strong security.


<details>
  <summary>Details</summary>
Motivation: Language models increasingly learn similar representations despite differences in training, creating opportunities for cross-model alignment. This enables applications where security, privacy, or competitive constraints prohibit direct data or model sharing.

Method: Proposes a privacy-preserving framework that learns an affine transformation over a shared public dataset to align representations between independent models, applies homomorphic encryption to protect client queries during inference, and encrypts only linear alignment and classification operations.

Result: Achieves sub-second inference latency while maintaining strong security guarantees. Empirical investigation shows minimal performance degradation in embedding classification and out-of-distribution detection across model pairs. First demonstration that linear alignment sometimes enables text generation across independently trained models.

Conclusion: Representational convergence enables practical cross-silo inference with privacy preservation, opening new application domains where direct model or data sharing is prohibited by security, privacy, or competitive constraints.
...
TL;DR: Agentic Business Process Management (APM) extends traditional BPM to govern autonomous agents in organizations, shifting from automation to process-aware agents with framed autonomy, explainability, conversational actionability, and self-modification capabilities.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the need to extend Business Process Management (BPM) for governing autonomous agents in organizations, recognizing a paradigm shift from traditional automation-oriented BPM toward systems where autonomy is constrained and aligned through process awareness.

Method: The paper introduces core abstractions and architectural elements for APM systems, elaborating on four key capabilities: framed autonomy, explainability, conversational actionability, and self-modification. It discusses realization potential and identifies research challenges requiring advances in BPM, AI, and multi-agent systems.

Result: The manifesto establishes a conceptual foundation for APM, providing a framework for agent governance where software and human agents act as primary functional entities within explicit process frames, ensuring goal alignment with organizational objectives.

Conclusion: The manifesto serves as a roadmap for bridging BPM, AI, and multi-agent systems communities and guiding the development of APM systems in practice, enabling agents to behave in framed yet proactive ways while pursuing organizational goals.
...
<details>
  <summary>Details</summary>
Motivation: Previous approaches to modeling goal-directed agents intervening in causal systems have limitations. The paper aims to extend structural causal models (SCMs) beyond causal questions to also address teleological questions about agent intentions.
...
Conclusion: Structural causal models can be extended to formulate and answer teleological questions about goal-directed agents through intentional interventions and structural final models, providing a framework for detecting agents and inferring their intentions from observational data.
...
TL;DR: A simulation framework using GANs and patrol models reveals extreme racial bias amplification in predictive policing systems across Baltimore and Chicago, showing persistent disparities that cannot be fully eliminated by algorithmic debiasing alone.


<details>
  <summary>Details</summary>
Motivation: Predictive policing systems are widely deployed but their tendency to encode and amplify racial disparities remains poorly understood in quantitative terms. There is a need to measure how racial bias propagates through the full enforcement pipeline from crime occurrence to police contact.
...
Conclusion: Predictive policing systems exhibit extreme racial bias amplification that cannot be fully addressed by algorithmic debiasing alone. Structural disparities require accompanying policy interventions, particularly around officer deployment levels, to achieve equitable outcomes. The framework provides reproducible quantitative evidence of bias propagation through enforcement pipelines.
...
TL;DR: SGAZ evaluates Tetris Block Puzzle rule set difficulty: holding blocks (h) and previews (p) reduce difficulty, while adding more block variants (especially T-pentomino) increases difficulty.


<details>
  <summary>Details</summary>
Motivation: Despite Tetris Block Puzzle's popularity with millions of downloads, there's little principled assessment of which rule sets are more difficult. The paper aims to systematically evaluate difficulty across different rule variations using a strong AI evaluator.
...
Conclusion: SGAZ enables efficient, reproducible comparisons across rule sets and provides a reference for future design in stochastic puzzle games, demonstrating its utility as a strong evaluator for assessing difficulty in puzzle game variants.
...
TL;DR: Online resource allocation with endogenous costs (costs depend on allocations through interaction matrix W) shows strict regret hierarchy: uniform (Ω(T)), gated (O(T^{2/3})), competitive (O(√(T log N))). Interaction topology governs computation-regret tradeoff, with sparse topologies reducing computation while modestly increasing regret.


<details>
  <summary>Details</summary>
Motivation: Standard online optimization assumes exogenous costs, but many real-world systems have endogenous costs where allocation decisions interact through cooperation/competition relationships. Need to understand how different allocation paradigms perform under such endogenous cost structures and how interaction topology affects the computation-regret tradeoff.

Method: Analyze three allocation paradigms: (I) uniform allocation (cost-ignorant), (II) gated allocation (cost-estimating), and (III) competitive allocation via multiplicative weights update with interaction feedback (cost-revealing). Study performance under adversarial sequences with bounded variation. Analyze how interaction matrix W's topology (full, sparse, ring-structured including Wuxing topology) affects regret bounds and computational cost.

Result: Strict regret separation: uniform incurs Ω(T) regret, gated achieves O(T^{2/3}), competitive achieves O(√(T log N)). Full interaction yields tightest bound but O(N²) per-step cost; sparse topologies increase regret by at most O(√(log N)) while reducing per-step cost to O(N). Ring-structured topologies (like Wuxing) minimize computation × regret product.

Conclusion: Competitive allocation via multiplicative weights with interaction feedback is optimal for endogenous cost settings. Interaction topology creates fundamental computation-regret tradeoff, with sparse structures offering favorable balance. Provides first regret-theoretic justification for decentralized competitive allocation in modular architectures and establishes cost endogeneity as distinct from partial observability.
...
TL;DR: Stability Monitor is a black-box system that detects behavioral changes in AI model endpoints by fingerprinting output distributions over time, addressing limitations of traditional reliability metrics.
...
Conclusion: Stability Monitor provides a practical black-box approach for monitoring behavioral consistency of AI model endpoints, addressing a critical gap in traditional reliability metrics for AI-native applications.
...
TL;DR: Synthetic review examining AI integration in judicial decision-making, focusing on risk assessment tools, human judicial biases, and AI-human interactions across multiple disciplines.


<details>
  <summary>Details</summary>
Motivation: Address concerns about transparency, reliability, and accountability of AI in judicial contexts while recognizing limitations of human judgment and the need to understand judge-AI interactions.

Method: Conducted a synthetic review connecting three aspects: AI tool performance/fairness, human judge strengths/biases, and AI+human interactions across computer science, economics, law, criminology, and psychology literature.

Result: Existing evidence shows modest or nonexistent impact of AI decision aids on pretrial and sentencing decisions; significant progress in evaluating predictive validity of risk assessment tools and documenting judicial biases; limited research on how judges use algorithmic recommendations.

Conclusion: AI vs Human comparisons can yield new insights into both algorithmic tools and human decision-makers; advocates for greater interdisciplinary integration and cross-fertilization in future research to address identified gaps.
...
TL;DR: LLMs and humans show different responses to cross-domain mapping interventions for creativity: humans benefit reliably, LLMs don't show significant effects but generate more original ideas overall, with both systems benefiting more from semantically distant sources.
...
Method: Comparative study where human participants and LLMs generated novel features for ten daily products under two conditions: (1) cross-domain mapping requiring translation of properties from randomly assigned remote sources, and (2) user-need prompting targeting unmet user needs.

Result: Humans reliably benefit from cross-domain mapping interventions, while LLMs generate more original ideas than humans on average but show no statistically significant effect of cross-domain mapping. Both systems show increased impact from cross-domain mapping when inspiration sources are more semantically distant from targets.

Conclusion: Results highlight the role of remote association in creative ideation and reveal systematic differences in how humans and LLMs respond to the same creativity interventions, suggesting distinct underlying mechanisms for creative generation in artificial versus biological systems.
...
Method: Combines LUNA's learned-query cross-attention for channel unification and FEMBA's bidirectional Mamba blocks for efficient temporal modeling, with systematic investigation of Latent-Euclidean Joint-Embedding Predictive Architecture (LeJEPA) for biosignal learning.

Result: Pre-trained on 21,000+ hours of unlabeled EEG, achieves 80.99% balanced accuracy on TUAB, 0.97 AUPR on Alzheimer's detection, with 377× fewer FLOPS than state-of-art models and ability to scale to 12× longer sequences before hitting GPU memory limits.

Conclusion: LuMamba provides an efficient, scalable solution for EEG foundation modeling, with the combination of masked reconstruction and LeJEPA objectives yielding the most robust representations for diverse downstream tasks.
...
Result: Both self-consistency and verbalized confidence scale in reasoning models, but self-consistency has lower initial discrimination. Hybrid estimators with just two samples improve AUROC by up to +12 on average and outperform individual signals even when scaled to larger budgets. Effects are domain-dependent with mathematics showing highest uncertainty quality.

Conclusion: Combining verbalized confidence and self-consistency through parallel sampling provides effective uncertainty estimation for reasoning language models, with hybrid estimators offering significant improvements even with minimal sampling budgets, though performance varies by domain.
...
TL;DR: LLM-based binary vulnerability analysis exhibits structured token-level implicit patterns in multi-pass reasoning, forming a stable system for exploration organization without explicit control flow.


<details>
  <summary>Details</summary>
Motivation: Understanding how LLM-based agents organize exploration over hundreds of reasoning steps in binary vulnerability analysis, given limited context windows and implicit token-level behaviors that remain poorly understood.

Method: Conducted first large-scale, trace-level study analyzing 521 binaries with 99,563 reasoning steps to identify token-level implicit patterns in multi-pass LLM reasoning.

Result: Identified four dominant implicit patterns: early pruning, path-dependent lock-in, targeted backtracking, and knowledge-guided prioritization that emerge from reasoning traces and form a stable, structured system with distinct temporal roles.

Conclusion: Provides first systematic characterization of LLM-driven binary analysis and foundation for more reliable analysis systems, showing token-level implicit patterns serve as abstraction of LLM reasoning organizing exploration through implicit decisions.
...
TL;DR: D5P4 is a diversity-focused beam search framework for discrete diffusion text generation that uses Determinantal Point Processes for candidate selection, improving diversity while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: Discrete diffusion models lack effective decoding methods compared to autoregressive models, with existing techniques providing limited control over in-batch diversity during iterative denoising.

Method: A generalized beam-search framework for discrete diffusion that generates candidates in parallel with modular selection objectives. D5P4 implements this using MAP inference over a Determinantal Point Process with a scalable greedy solver.
...
Conclusion: The proposed framework enables explicit trade-offs between model probability and target diversity in discrete diffusion decoding, addressing a key limitation in current approaches.
...
TL;DR: cuGenOpt is a GPU-accelerated metaheuristic framework that balances generality, performance, and usability through CUDA architecture, extensible operator design, and Python/LLM interfaces, achieving significant speedups and competitive solution quality across diverse combinatorial problems.


<details>
  <summary>Details</summary>
Motivation: Existing combinatorial optimization approaches face a fundamental trade-off among generality, performance, and usability, creating limitations for practical applications in logistics, scheduling, and resource allocation.

Method: Three-level architecture: 1) Engine level with "one block evolves one solution" CUDA design, unified encoding abstraction, adaptive operator selection, and hardware-aware resource management; 2) Extensibility via user-defined operator registration for domain-specific CUDA operators; 3) Usability through JIT compilation pipeline with pure-Python API and LLM-based modeling assistant for natural-language problem descriptions.

Result: Outperforms general MIP solvers by orders of magnitude; achieves competitive quality against specialized solvers up to n=150; attains 4.73% gap on TSP-442 within 30s; solves 12 problem types across 5 encoding variants to optimality; cumulative optimizations reduce pcb442 gap from 36% to 4.73% and boost VRPTW throughput by 75-81% across T4, V100, A800 GPUs.

Conclusion: cuGenOpt successfully addresses the trade-off among generality, performance, and usability in combinatorial optimization through GPU acceleration, extensible architecture, and user-friendly interfaces, demonstrating practical effectiveness across diverse problem domains and GPU architectures.
...
TL;DR: OS-Themis is a multi-agent critic framework that decomposes GUI agent trajectories into verifiable milestones with strict evidence auditing, improving RL robustness and achieving state-of-the-art performance on the new OGRBench benchmark.


<details>
  <summary>Details</summary>
Motivation: Reinforcement Learning for GUI agents faces challenges with reward function quality - existing approaches struggle to balance scalability and performance, limiting RL's potential to improve agent robustness in stochastic environments.

Method: OS-Themis uses a multi-agent critic framework that decomposes trajectories into verifiable milestones to isolate critical evidence, employs a review mechanism to strictly audit the evidence chain before making final verdicts, and introduces OmniGUIRewardBench (OGRBench) for comprehensive evaluation.

Result: OS-Themis achieves best performance on OGRBench across all evaluated models, yields 10.3% improvement for online RL training support, and provides 6.9% gain for trajectory validation/filtering in self-training loops on AndroidWorld.

Conclusion: OS-Themis demonstrates significant potential to drive GUI agent evolution through its scalable, accurate multi-agent critic framework that effectively addresses reward function challenges in RL for stochastic GUI environments.
...
TL;DR: DRL locomotion policies autonomously acquire interpretable phase structures like stance/swing phases, with periodic transitions and branching, analyzed via state aggregation and Explainable Boosting Machines.


<details>
  <summary>Details</summary>
Motivation: DRL achieves high performance in locomotion control but operates as a black box, making decision-making processes difficult to understand. Since periodic motions like walking have known implicit phases, the study hypothesizes that trained policies may represent interpretable phase structures.

Method: Using the MuJoCo HalfCheetah-v5 benchmark, state transition sequences from a trained walking policy were aggregated into semantic phases based on state similarity and transition consistency. Phase-dependent decision-making was analyzed by approximating states and actions for each phase using Explainable Boosting Machines (EBMs).

Result: The trained policy exhibited periodic phase transition structures and phase branching. EBMs revealed which state features the policy attends to and how it controls action outputs in each phase, showing phase-dependent decision-making patterns.

Conclusion: Neural network-based policies, often considered black boxes, can autonomously acquire interpretable phase structures and logical branching mechanisms, suggesting potential for understanding DRL decision-making in locomotion tasks.
...
TL;DR: Workshop report on establishing a national Center for AI and Robotic Excellence in medicine to address critical gaps in healthcare robotics and AI deployment through coordinated research, standardized evaluation, and workforce training.


<details>
  <summary>Details</summary>
Motivation: Accelerating need for coordinated research efforts bridging engineering innovation with clinical priorities, emphasizing safety, reliability, and translational readiness of robotics and AI in healthcare, particularly for underserved regions and high-risk procedures.

Method: Workshop format with keynotes, panels, and breakout sessions convening researchers, clinicians, industry innovators, and federal stakeholders to identify gaps and develop consensus on national priorities.

Result: Identified critical gaps in data availability, standardized evaluation methods, regulatory pathways, and workforce training; established consensus on urgency for national CARE center; defined priority research thrusts including human-robot collaboration, trustworthy autonomy, simulation, multi-modal sensing, and ethical AI integration.

Conclusion: Broad consensus on establishing a national Center for AI and Robotic Excellence in medicine to advance transformative potential of AI-enabled robotics in surgical, diagnostic, rehabilitative, and assistive contexts, with special attention to austere, disaster, and military settings.
...
TL;DR: GoalVLM is a cooperative multi-agent framework for zero-shot, open-vocabulary object navigation that integrates Vision-Language Models directly into the decision loop without task-specific training.
...
Method: GoalVLM integrates a Vision-Language Model (VLM) into the decision loop, SAM3 for text-prompted detection/segmentation, and SpaceOM for spatial reasoning. Each agent builds a BEV semantic map from depth-projected voxel splatting, while a Goal Projector back-projects detections through calibrated depth for goal localization. A constraint-guided reasoning layer evaluates frontiers through a structured prompt chain (scene captioning, room-type classification, perception gating, multi-frontier ranking).
...
Conclusion: GoalVLM demonstrates effective zero-shot, open-vocabulary object navigation through cooperative multi-agent framework with VLM integration, achieving competitive performance without task-specific training and enabling generalization to novel goals.
...
TL;DR: ReDAGRT is a user-space global scheduling framework for ROS 2 that provides deterministic multi-DAG execution through rate-priority scheduling and concurrency bounds, reducing deadline misses and response times without modifying ROS 2 APIs or OS scheduler.


<details>
  <summary>Details</summary>
Motivation: Default ROS 2 executors lack cross-DAG priority enforcement, causing callback contention, structural priority inversion, and deadline instability in concurrent workloads, which limits deployment in time-critical and safety-sensitive cyber-physical systems.

Method: Introduces ReDAGRT framework with rate-priority driven global ready queue that orders callbacks by activation rate, enforces per-DAG concurrency bounds, and mitigates cross-graph priority inversion without modifying ROS 2 API, executor interface, or OS scheduler. Formalizes multi-DAG task model and analyzes cross-DAG interference under Rate-Priority scheduling.
...
TL;DR: RAPiD extends Rapid Motor Adaptation (RMA) to deformable object manipulation by learning a policy conditioned on object dynamics embeddings encoded from privileged simulation data (mass and particle positions), then inferring these embeddings from real-world visual observations for transfer.


<details>
  <summary>Details</summary>
Motivation: Deformable object manipulation requires understanding unknown dynamics parameters that define how objects stretch, bend, compress, and move. While RMA handles unknown dynamics in rigid domains, it doesn't capture shape changes crucial for deformable objects.

Method: Two-phase approach: 1) Learn visuomotor policy conditioned on object dynamics embedding encoded from privileged simulation information (mass and ground-truth particle positions). 2) Learn to infer this embedding from non-privileged real-world information (robot visual observations and actions) for transfer.

Result: RAPiD achieves over 80%+ success rates across two vision-based deformable object mobile manipulation tasks on a 22-DOF mobile manipulator in real world, under various object dynamics, categories, and instances.

Conclusion: Ground-truth particle positions in simulation capture shape changes, enabling extension of RMA to deformable objects. The two-phase approach successfully transfers learned policies to real-world deformable object manipulation.
...
<details>
  <summary>Details</summary>
Motivation: Current manufacturing techniques cannot produce micro-patterned surfaces at scale due to high-cost machinery and inefficient coverage methods like raster-scanning.

Method: Use multiple robots with patterning tools coordinated via ergodic control algorithm, which specifies coverage objectives using distributions; robots communicate compressed trajectory history representations.

Result: Robots successfully divide complicated coverage objectives in simulations and experiments; robot-produced patterning lowers coefficient of friction on metallic surfaces.

Conclusion: Distributed multi-robot systems can coordinate to manufacture products previously unrealizable at scale, enabling scalable production of functional micro-patterned surfaces.
...
<details>
  <summary>Details</summary>
Motivation: Ambiguity poses a major challenge to LLMs used as robotic planners, reducing their reliability in real-world environments where instructions may be unclear or incomplete.

Method: Two-stage framework: 1) Construct structured scene graph representation from input observations capturing objects, attributes, and relationships; 2) Equip LLM with retrieval functions to query relevant portions of scene graph, enabling iterative ambiguity detection and clarification.

Result: SG-CoT consistently outperforms prior methods with minimum 10% improvement in question accuracy, 4% success rate increase in single-agent environments, and 15% increase in multi-agent environments.
...
TL;DR: First measurement study of mobile robotic manipulation workloads across GPU platforms reveals onboard computation is infeasible for smaller GPUs and battery-intensive for larger ones, while offloading faces latency and bandwidth challenges, with opportunities for shared compute across robot fleets.


<details>
  <summary>Details</summary>
Motivation: Foundation models have significantly improved mobile robotic manipulation performance but at high computational cost, creating a need to understand the practical deployment challenges across different compute platforms (onboard, edge, cloud) to design effective inference systems.

Method: Conducted the first comprehensive measurement study of mobile robotic manipulation workloads, evaluating performance across three GPU platforms: onboard (small and large GPUs), edge computing, and cloud infrastructure, analyzing computational feasibility, battery impact, network latency effects, and bandwidth requirements.

Result: Found that full workload stack is infeasible on smaller onboard GPUs; larger onboard GPUs drain robot batteries several hours faster; offloading alleviates constraints but introduces network latency that degrades task accuracy; naive cloud offloading is impractical due to bandwidth requirements; identified both opportunities and pitfalls for sharing compute across robot fleets.

Conclusion: The measurement study provides crucial insights for designing inference systems for mobile robots, highlighting the trade-offs between onboard computation, edge offloading, and cloud processing, and emphasizing the need for optimized compute architectures that balance performance, battery life, and network constraints.
...
TL;DR: First sparsely supervised framework for monocular 3D object tracking that decomposes the task into 2D query matching and 3D geometry estimation, using spatio-temporal consistency to generate dense pseudolabels from sparse annotations.


<details>
  <summary>Details</summary>
Motivation: Existing state-of-the-art monocular 3D object tracking approaches require dense 3D annotations over long video sequences, which are expensive to obtain and difficult to scale. There is a need for methods that can work with sparse supervision to make the task more practical and scalable.

Method: Proposes a sparsely supervised framework that decomposes tracking into two sequential sub-problems: 2D query matching and 3D geometry estimation. Both components leverage spatio-temporal consistency of image sequences to augment sparse labeled samples and learn rich 2D and 3D representations. The model automatically generates high-quality 3D pseudolabels across entire videos, transforming sparse supervision into dense 3D track annotations.

Result: Extensive experiments on KITTI and nuScenes datasets demonstrate significant improvement in tracking performance, achieving up to 15.50 percentage point improvement while using at most four ground truth annotations per track. Enables existing fully-supervised trackers to operate effectively under extreme label sparsity.

Conclusion: The proposed sparsely supervised framework addresses the fundamental limitation of requiring dense 3D annotations for monocular 3D object tracking, making the task more scalable and practical while maintaining or improving performance compared to fully supervised approaches.
...
<details>
  <summary>Details</summary>
Motivation: Proprioceptive-only state estimation is computationally efficient and robust to perceptually degraded conditions, but existing methods rely on Gaussian noise assumptions that break down with limited training data, leading to inconsistent and potentially divergent estimates.
...
Conclusion: Set-coverage noise characterization provides a robust alternative to Gaussian assumptions for proprioceptive-only state estimation in legged robots, maintaining consistency and preventing drift in practical applications with limited training data.
...
TL;DR: DriveVLM-RL integrates vision-language models with reinforcement learning through a dual-pathway architecture for safe autonomous driving, using VLMs only during offline training to ensure real-time deployment feasibility.
...
Method: A neuroscience-inspired dual-pathway architecture: Static Pathway uses CLIP-based contrasting language goals for continuous spatial safety assessment; Dynamic Pathway employs attention-gated multi-frame semantic risk reasoning with a lightweight detector and large VLM. Hierarchical reward synthesis fuses semantic signals with vehicle states, and an asynchronous training pipeline decouples expensive VLM inference from environment interaction. All VLM components are removed at deployment.
...
Conclusion: DriveVLM-RL provides a practical paradigm for integrating foundation models into autonomous driving without compromising real-time feasibility, demonstrating effective semantic reward learning through offline VLM use and real-time deployment without VLMs.
...
TL;DR: ManiDreams is a modular framework for uncertainty-aware manipulation planning that integrates uncertainty representation, propagation, and constraint satisfaction into the planning loop, enabling robust robotic manipulation under perceptual, parametric, and structural uncertainties.


<details>
  <summary>Details</summary>
Motivation: Traditional dynamics models focus on minimizing prediction error but fail to address the fundamental challenge of uncertainty in real-world manipulation. Robust manipulation requires representing, propagating, and constraining uncertainties within the planning loop rather than merely suppressing them during training.

Method: ManiDreams implements a modular framework with composable abstractions for distributional state representation, backend-agnostic dynamics prediction, and declarative constraint specification. It uses a sample-predict-constrain loop that wraps any base policy to evaluate candidate actions against distributional outcomes, explicitly addressing three uncertainty sources: perceptual, parametric, and structural.
...
Conclusion: Robust manipulation under uncertainty is fundamentally an integration problem requiring uncertainty representation, propagation, and constraint satisfaction within planning. ManiDreams provides a practical framework that adds robustness to existing policies without retraining, enabling uncertainty-aware manipulation planning across diverse robotic tasks.
...
<details>
  <summary>Details</summary>
Motivation: Current VLA models lack reliable introspection, and common mean aggregation of token-level uncertainty dilutes short-lived but safety-critical uncertainty spikes in continuous control, making it difficult to distinguish between successful and failure rollouts based on uncertainty signals.
...
TL;DR: HRI-SA dataset enables systematic evaluation of online situational awareness assessment in human-robot teaming using multimodal data and ground truth SA latency measurements.
...
Method: Collected multimodal dataset from 30 participants in search-and-rescue context with eye movements, pupil diameter, biosignals, user interactions, and robot data; measured ground truth SA latency (perceptual and comprehension) as time between assistance need onset and resolution.

Result: Eye-tracking features alone achieved 88.91% recall and 67.63% F1 for perceptual SA latency detection; contextual data fusion improved performance to 91.51% recall and 80.38% F1.

Conclusion: First public dataset supporting systematic SA evaluation in human-robot teaming missions, demonstrating potential of generic eye-tracking features for continuous perceptual SA latency detection.
...
TL;DR: Textile-integrated stretchable strain sensors fabricated via multi-material direct ink writing and automated embroidery for motion monitoring in garments.


<details>
  <summary>Details</summary>
Motivation: Growing demand for smart garments combining comfort, durability, and accurate motion detection for sports performance tracking, rehabilitation, and injury prevention.

Method: Hybrid fabrication workflow integrating multi-material direct ink writing (silicone-carbon grease-silicone stack) with automated embroidery for mechanical fixation and electrical interfacing in a single step.

Result: Sensors achieve 120% stretchability with electrical continuity, linear behavior up to 60% strain (R²=0.99), gauge factor of 31.4, hysteresis of 22.9%, and moderate cycle-to-cycle stability (0.135-0.236% drift per cycle). Successfully integrated into elbow/knee sleeves for joint angle monitoring.
...
TL;DR: A multimodal tactile slip detection method using a five-fingered bio-inspired hand with 24 distributed tactile channels achieves 96.39% accuracy for contact status recognition and effective slip onset detection.


<details>
  <summary>Details</summary>
Motivation: Robotic manipulation of fragile and glazed objects requires precise grasp force control to prevent damage from excessive force or slippage from insufficient force. Existing methods often assume firm pre-grasp, but slip detection and prevention are essential for robots operating in unstructured environments.
...
Result: Achieved 96.39% test accuracy across three sliding speeds and six materials. Maintained 91.95% accuracy on four new unseen materials, demonstrating strong generalization. Successfully detected slip onset time based on trained contact status recognition model.

Conclusion: The proposed multimodal tactile feedback approach with distributed sensing and contact status recognition effectively addresses slip detection for robotic manipulation, showing high accuracy and generalization capability beyond simple threshold methods.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for handling interdependent geometric constraints in multi-agent TAMP struggle with partially ordered tasks and dynamic agent assignments, typically assuming static assignments and lacking adaptation when disturbances alter task allocations.

Method: Introduces Graph-of-Constraints Model Predictive Control (GoC-MPC), a generalized sequence-of-constraints framework integrated with MPC that defines constraints over tracked 3D keypoints, enabling robust multi-agent manipulation coordination and online adaptation from visual observations alone.

Result: GoC-MPC achieves higher success rates, significantly faster TAMP computation, and shorter overall paths compared to recent baselines, establishing it as an efficient and robust solution for multi-agent manipulation under real-world disturbances.

Conclusion: GoC-MPC provides an efficient and robust framework for multi-agent manipulation that naturally supports partially ordered tasks, dynamic agent coordination, and disturbance recovery without relying on training data or environment models.
...
TL;DR: Hardware-control co-design for passive-wheel skating on quadruped robots using bilevel optimization: Bayesian Optimization for mechanical design and Reinforcement Learning for control policies, enabling efficient dynamic skating with behaviors like hockey stop and self-aligning motion.


<details>
  <summary>Details</summary>
Motivation: Passive-wheel skating reduces leg inertia and improves energy efficiency at high speeds for quadrupedal robots, but the absence of direct wheel actuation creates tight coupling between mechanical design and control that requires a co-design approach to unlock the full potential of this modality.

Method: Bilevel optimization framework: upper-level Bayesian Optimization searches mechanical design space, lower-level Reinforcement Learning trains motor control policy for each candidate design, creating design-policy pairs for passive-wheel skating.

Result: The co-designed system outperforms human-engineered baselines and exhibits versatile behaviors including hockey stop (rapid braking by turning sideways to maximize friction) and self-aligning motion (automatic reorientation for improved energy efficiency in travel direction).

Conclusion: This work presents the first system-level study of dynamic skating motion on quadrupedal robots, demonstrating that hardware-control co-design enables efficient and versatile roller skating with passive wheels through bilevel optimization of mechanical design and control policies.
...
Method: Proposes MemoAct, a hierarchical memory-based policy inspired by the Atkinson-Shiffrin memory model, featuring distinct memory tiers: lossless short-term memory for precise task state tracking and compressed long-term memory for robust long-horizon retention. Also constructs MemoryRTBench based on RoboTwin 2.0 for evaluation.
...
Conclusion: MemoAct effectively addresses the limitations of existing approaches by leveraging a hierarchical memory architecture inspired by cognitive psychology, enabling simultaneous precise task state tracking and robust long-horizon retention in memory-dependent robotic tasks.
...
TL;DR: RAPID is a robotic platform for EV battery disassembly that combines perception-driven manipulation with AI-assisted programming, achieving high success rates for fastener removal and demonstrating the effectiveness of structured tool interfaces for LLM-driven robot control.


<details>
  <summary>Details</summary>
Motivation: Electric vehicle battery recycling requires scalable solutions, but current disassembly is largely manual due to high design variability. There's an urgent need for flexible automation that can handle diverse battery pack designs and enable systematic investigation of robotic disassembly workflows.

Method: The RAPID system integrates a gantry-mounted industrial manipulator with RGB-D perception and automated nut-running tools. It uses open-vocabulary object detection for component identification and evaluates three fastener removal strategies: taught-in poses, one-shot vision execution, and visual servoing. The platform also introduces agentic AI specifications with structured tool interfaces and ROS services for LLM-driven robot programming.

Result: Open-vocabulary object detection achieved 0.9757 mAP50. For fastener removal: taught-in poses (97% success, 24 min), one-shot vision execution (57%, 29 min), and visual servoing (83%, 36 min). LLM agents with structured tool interfaces achieved 100% task completion, while automatic ROS service discovery had 43.3% failure rates.

Conclusion: RAPID provides a practical foundation for scalable robotic battery recycling research. Structured tool interfaces are essential for reliable LLM-driven robot control, and the platform enables systematic investigation of human-robot collaboration, agentic programming, and increasingly autonomous disassembly workflows.
...
TL;DR: Using 3D world generative models to create diverse interactive scenes enables scalable RL fine-tuning of vision-language-action models without sacrificing generality, achieving significant sim-to-real transfer improvements.
...
Method: Leverage 3D world generative models with language-driven scene designer to generate hundreds of diverse interactive scenes with unique objects/backgrounds. Use this for scalable parallel policy learning with domain randomization for sim-to-real transfer.
...
Conclusion: 3D world generative models enable scalable RL fine-tuning of VLAs without sacrificing generality, providing effectively unlimited diverse training data for improved sim-to-real transfer and zero-shot generalization.
...
TL;DR: Self-sensing fiber-reinforced pneumatic twisted-and-coiled actuator with conductive nickel wire enables intrinsic force estimation via inductance feedback, overcoming hysteresis limitations through hybrid observer approach.


<details>
  <summary>Details</summary>
Motivation: FR-PTCAs have high power density and compliance but suffer from strong hysteresis and lack intrinsic proprioception, limiting effective closed-loop control. There's a need for self-sensing capabilities without external sensors.

Method: Integrated conductive nickel wire into FR-PTCA for inductance feedback. Developed parametric self-sensing model and nonlinear hybrid observer combining Extended Kalman Filter with constrained optimization to resolve inductance-force mapping ambiguity and estimate actuator states.

Result: Experimental characterization shows deterministic, low-hysteresis inductance-force relationship at constant pressures (vs. strongly hysteretic inductance-length behavior). Force estimation accuracy comparable to external load cells, with robust performance under varying load conditions.

Conclusion: The self-sensing FR-PTCA with integrated conductive wire and hybrid observer enables effective intrinsic force estimation and state estimation, overcoming hysteresis limitations and enabling improved closed-loop control of soft pneumatic actuators.
...
TL;DR: A monolithic, fully compliant, bistable laparoscopic grasper is designed using compliant mechanisms and 3D printed, eliminating rigid links and reducing part count while maintaining stable grasping without continuous user input.


<details>
  <summary>Details</summary>
Motivation: Traditional laparoscopic graspers use multi-link rigid mechanisms with tight manufacturing tolerances, resulting in high production and assembly costs. The authors aim to reduce complexity and cost by developing a compliant mechanism alternative.

Method: The device integrates a compliant trigger and compliant gripper end-effector coupled via a control push-rod. The trigger uses a Two-Element Beam Constraint Model to design V-beam-like elements for elastic energy storage while preventing snap-through instability. The end-effector is designed as a compliant gripper for adaptive grasping. Nonlinear finite element analysis validates jaw performance, and the device is fabricated using fused deposition 3D printing.

Result: The fabricated prototype demonstrates reliable bistable actuation, confirming the feasibility of compliant laparoscopic grasper architectures. The design eliminates multiple rigid links, reduces part count, and achieves stable grasping without continuous user input.

Conclusion: The work successfully demonstrates a monolithic, fully compliant, bistable laparoscopic grasper that reduces manufacturing complexity and cost while maintaining functional performance, offering a promising alternative to traditional rigid-link mechanisms.
...
TL;DR: A hybrid visual odometry approach combining direct photometric methods with descriptor-based feature matching improves localization accuracy in challenging environments like low-light conditions and illumination changes, outperforming conventional sparse-direct methods.


<details>
  <summary>Details</summary>
Motivation: Conventional visual odometry methods degrade significantly in visually challenging environments (low-light, overexposure, illumination changes, high parallax), undermining robust autonomous driving navigation. While LIVO frameworks integrate multiple sensors, there's a need for improved visual state estimation that can handle photometric inconsistencies.

Method: Extends FAST-LIVO2 framework with hybrid approach integrating direct photometric methods with descriptor-based feature matching. Evaluates four descriptor configurations: ORB with Hamming distance, SuperPoint with SuperGlue, SuperPoint with LightGlue, and XFeat with mutual nearest neighbor matching.

Result: The hybrid approach outperforms conventional sparse-direct method, maintaining robust performance where sparse-direct fails due to photometric inconsistency from illumination changes. Learning-based descriptors enable robust visual state estimation across challenging environments. Benchmarking reveals trade-offs in accuracy, computational cost, and feature tracking stability.

Conclusion: Hybrid visual odometry combining direct and descriptor-based methods provides superior performance in challenging visual conditions compared to sparse-direct approaches, with learning-based descriptors offering particular advantages for robust navigation in autonomous driving applications.
...
TL;DR: REST introduces a tree-of-paths approach for zero-shot object-goal navigation, using receding horizon planning and LLM reasoning to achieve superior efficiency-success balance.


<details>
  <summary>Details</summary>
Motivation: Prior hierarchical ZSON methods focus on scene understanding and high-level decision-making but overlook option design, treating options as isolated waypoints that miss en-route information gain and lack structured relationships among candidates.

Method: REST builds explicit open-vocabulary 3D maps from RGB-D streams, grows agent-centric trees of safe informative paths via sampling-based planning, and uses chain-of-thought LLM reasoning to select optimal paths through spatial narrative textualization.
...
Conclusion: Treating the option space as a tree of paths with explicit path-level reasoning and hierarchical LLM decision-making significantly improves zero-shot object-goal navigation performance over destination-only approaches.
...
TL;DR: CSSDF-Net learns a continuous signed distance field directly in configuration space for differentiable collision avoidance, enabling zero-shot generalization to unseen environments without retraining.


<details>
  <summary>Details</summary>
Motivation: Existing collision avoidance methods have limitations: geometric collision checkers are non-differentiable, workspace-based implicit distance models suffer from poor convergence due to nonlinear workspace-configuration mapping, and self-collision/environment collision are handled separately rather than under a unified safety framework.

Method: CSSDF-Net learns a continuous signed distance field directly in configuration space using a spatial-hashing-based data generation pipeline that encodes robot-centric geometric priors. This enables efficient retrieval of risk configurations for arbitrary obstacle point sets and provides joint-space distance and gradient queries under a unified geometric safety notion.

Result: Experiments on planar arm and 7-DoF manipulator demonstrate stable gradients, effective collision avoidance in static/dynamic scenes, and practical inference latency for large-scale point-cloud queries. The method supports zero-shot generalization to previously unseen environments without retraining.

Conclusion: CSSDF-Net provides a differentiable, scene-agnostic distance query mechanism that unifies self-collision and environment collision constraints, enabling both offline trajectory optimization and online reactive avoidance through MPC with zero-shot generalization capabilities.
...
TL;DR: Deep learning-based visual feature tracking method for monocular SLAM/Odometry that works in textureless environments and under rapid lighting changes, integrated and evaluated with VINS-Fusion.


<details>
  <summary>Details</summary>
Motivation: Visual feature tracking is critical for camera-based SLAM/Odometry performance, but traditional methods struggle in challenging conditions like textureless environments and rapid lighting changes.

Method: Proposes a deep learning-based approach for robust visual feature tracking in monocular camera images. The method is designed to handle textureless environments and rapid lighting variations. The approach is integrated into VINS-Fusion (Monocular-Inertial) VIO system for evaluation.

Result: The method demonstrates reliable operation in challenging conditions (textureless environments and rapid lighting changes) where traditional feature tracking methods typically fail.

Conclusion: Deep learning-based visual feature tracking can significantly improve robustness in monocular SLAM/Odometry systems, particularly in challenging environmental conditions, as validated through integration with the VINS-Fusion VIO system.
...
TL;DR: RG-VLMD is a reasoning-guided vision-language-motion diffusion framework that generates instruction-aware co-speech gestures for humanoid robots in educational settings by integrating affective estimation, pedagogical reasoning, and teaching-act-conditioned motion synthesis.


<details>
  <summary>Details</summary>
Motivation: To enable humanoid robots in educational scenarios to produce adaptive, semantically consistent, and pedagogically expressive co-speech gestures that enhance human-robot interaction through reasoning-guided instructional conditioning.

Method: A multi-modal framework integrating: 1) gated mixture-of-experts model for Valence/Arousal prediction from text, visual, and acoustic features; 2) affect-driven policy mapping to discrete teaching-act categories; 3) diffusion-based motion generator conditioned via clip-level intent and frame-level instructional schedules with additive latent restriction and auxiliary action-group supervision.

Result: The method produces more structured and distinctive motion patterns than baseline diffusion models, verified by motion statics and pairwise distance analysis. Generated motions are physically plausible and can be retargeted to NAO robot for real-time execution, improving gesture controllability and pedagogical expressiveness.

Conclusion: Reasoning-guided instructional conditioning significantly enhances gesture controllability and pedagogical expressiveness in educational human-robot interaction, enabling more adaptive and semantically consistent robot behavior through integrated affective estimation and pedagogical reasoning.
...
TL;DR: A visual-tactile imitation learning method for 1D and 2D deformable object tracing using a unified model with local weighted loss and global task loss, achieving 80% success on seen and 65% on unseen objects.


<details>
  <summary>Details</summary>
Motivation: Existing deformable object tracing methods lack generalizability across different object categories or struggle with real-world reliability due to requirements for object-specific modeling or sim-to-real transfer limitations.

Method: Visual-tactile imitation learning approach with local weighted loss emphasizing contact near tactile image center for fine-grained adjustment, global tracing task loss for regulating progression, and low-cost teleoperation system integrating tactile sensing to compensate for limited visual features.

Result: Achieved average success rate of 80% on seen deformable objects and 65% on unseen objects across diverse 1D and 2D deformable objects in extensive ablation and comparative experiments.

Conclusion: The proposed visual-tactile imitation learning method effectively addresses generalizability and reliability challenges in deformable object tracing through unified modeling and multi-perspective design.
...
TL;DR: Maple is a table-top Socially Assistive Robot co-designed with literacy tutors to support young newcomer children in Canada through story-based activities and multi-modal scaffolding, addressing limited one-to-one tutor time.


<details>
  <summary>Details</summary>
Motivation: Community literacy programs for young newcomer children in Canada face resource constraints with limited staffing and scarce one-to-one time, restricting personalized English and cultural learning support.

Method: Co-design study with United for Literacy tutors involving shadowing and interviews to derive newcomer-specific requirements, resulting in Maple - a table-top, peer-like SAR prototype with story-based activities, multi-modal scaffolding (speech, facial feedback, gesture), and embedded quizzes.

Result: Developed an integrated prototype that supports attention while producing tutor-actionable formative signals, with system design implications for tutor-in-the-loop SARs in community settings.

Conclusion: Contributes design implications for tutor-in-the-loop SARs supporting language socialization in community settings and outlines directions for child-centered evaluation in authentic programs.
...
Method: Uses LLMs and 3D generative models to construct physically grounded 3D scenes, validated by geometric constraints. Leverages video generation models as motion priors, with visual predictions mapped to executable robot trajectories via Sim-to-Gen visual-kinematic alignment using CoTracker3 and VGGT.

Result: Policies trained on synthesized trajectories demonstrate robust generalization to unseen objects in simulation and achieve effective sim-to-real transfer, successfully manipulating novel real-world objects with the Piper robotic arm.

Conclusion: V-Dreamer provides a fully automated framework for generating open-vocabulary, simulation-ready manipulation environments and trajectories from natural language, enabling scalable robot training without manual intervention while maintaining physical fidelity and visual diversity.
...
TL;DR: Passive elastic-folding hinge mechanism enables air-dispersed sensor networks to transform from flat to 3D structures upon release without active actuators, using scalable PCB-laminate fabrication and predictive geometric modeling.


<details>
  <summary>Details</summary>
Motivation: Existing air-dispersed sensor networks rely on active actuators for mid-air control, increasing power consumption and cost. There's a need for passive, low-cost mechanisms for wide-area environmental monitoring using aerial robotic systems.

Method: Developed a passive elastic-folding hinge mechanism fabricated by laminating commercial sheet materials with rigid PCBs. Fold angles are programmed through a single oven-heating step. Created a geometric model linking laminate geometry, hinge mechanics, and resulting fold angle for predictive design.

Result: Laboratory tests achieved fold angles between 10-100 degrees with 4-degree standard deviation and high repeatability. Field trials demonstrated reliable data collection and LoRa transmission during dispersion. HWM-based trajectory simulations indicated potential for wide-area sensing exceeding 10 km.

Conclusion: The passive elastic-folding hinge provides a scalable, low-cost solution for air-dispersed sensor networks, eliminating active actuators while enabling reliable environmental monitoring over large areas through predictable structural transformation.
...
<details>
  <summary>Details</summary>
Motivation: Need for runtime-efficient, safety-guaranteed control for spacecraft close proximity operations (like on-orbit servicing) that can handle nonlinear dynamics and perturbations while ensuring strict safety compliance with ESA regulations.

Method: Three-stage framework: 1) NMPC expert with CBF constraints generates safe reference trajectories; 2) Neural policy trained with CBF-CLF-informed loss and DAgger-like rollouts with curriculum weighting; 3) Deployment with lightweight one-step CBF-CLF QP filter to minimally adjust learned control for safety/stability.

Result: Validated in Basilisk high-fidelity simulator for ESA-compliant operations (fly-around with spherical keep-out zone, conical approach corridor). Achieves stable convergence to decision points, strict safety adherence, task performance comparable to NMPC expert, with significantly reduced online computation and real-time feasibility on COTS processor.

Conclusion: Framework enables safe, runtime-efficient spacecraft proximity control suitable for onboard deployment in safety-critical on-orbit servicing missions, balancing neural network efficiency with formal safety guarantees through CBF-CLF integration.
...
TL;DR: Lightweight MPC approaches for spacecraft attitude tracking using reaction wheels during rendezvous, featuring novel linear constraints and dual-loop design with real-time feasibility on resource-constrained hardware.


<details>
  <summary>Details</summary>
Motivation: Need for computationally efficient attitude control for spacecraft rendezvous missions, particularly for resource-constrained New Space platforms using commercial off-the-shelf hardware.

Method: Two MPC approaches based on novel attitude deviation formulation enabling linear angular velocity constraints: single-loop MPC and dual-loop MPC with embedded stabilizing feedback controller creating LTI system; implemented with CasADi and automatic code generation.

Result: Improved tracking accuracy with reduced computational effort and memory consumption; validated in Basilisk astrodynamics simulation; successfully embedded on ARM Cortex-M7 demonstrating real-time feasibility.

Conclusion: The lightweight MPC approaches are suitable for onboard attitude control in resource-constrained spacecraft rendezvous missions, offering real-time performance on commercial hardware.
...
TL;DR: PRIOR is a humanoid locomotion framework that achieves robust terrain traversal with natural gaits using parametric gait generation, GRU-based terrain estimation from depth images, and terrain-adaptive footstep rewards, achieving 100% success rate across challenging terrains.


<details>
  <summary>Details</summary>
Motivation: Existing methods for perceptive humanoid locomotion require complex multi-stage training pipelines, adversarial objectives, or extensive real-world calibration, making robust terrain traversal with natural gaits difficult to achieve efficiently and reproducibly.
...
Result: Achieved 100% traversal success rate across challenging terrains including stairs, boxes, and gaps. Systematic analysis identified optimal depth image resolution configurations that maximize terrain fidelity under real-time constraints while reducing perceptual overhead.

Conclusion: PRIOR provides an efficient and reproducible framework for humanoid locomotion that eliminates the need for complex training pipelines while achieving robust performance. The framework will be open-sourced to serve as a foundation for future research.
...
Method: MERGE integrates Vision-Language Models with a perception pipeline: a lightweight streaming module continuously processes visual input to detect changes and selectively invokes the VLM only when necessary. This decoupled design preserves VLM reasoning while improving efficiency. The system uniquely identifies physical instances of actors and objects and structures them into actor-action-object relations with temporal consistency.

Result: On the introduced GROUND dataset (multi-person and human-robot interactions with fine-grained situational annotations), MERGE improves average grounding score by 2× compared to VLM-only baselines (GPT-4o, GPT-5, Gemini 2.5 Flash) while reducing run-time by 4×.

Conclusion: MERGE demonstrates that combining lightweight streaming perception with selective VLM invocation enables efficient, temporally consistent situational grounding for multi-actor collaboration, addressing limitations of VLM-only approaches while maintaining their reasoning capabilities.
...
TL;DR: ATG-MoE is an end-to-end autoregressive trajectory generation method with mixture of experts that learns assembly skills from demonstration, achieving strong performance and multi-skill integration for flexible manufacturing.


<details>
  <summary>Details</summary>
Motivation: Traditional robot programming is labor-intensive and inflexible, while existing learning-based assembly methods suffer from weak positional generalization, complex multi-stage designs, and limited multi-skill integration capability for flexible manufacturing.

Method: ATG-MoE establishes closed-loop mapping from multi-modal inputs (RGB-D, language instructions, proprioception) to manipulation trajectories using multi-modal feature fusion, autoregressive sequence modeling, and mixture-of-experts architecture for unified multi-skill learning.

Result: Achieves average grasp success rate of 96.3% and overall success rate of 91.8% on eight assembly skills in simulation, with strong generalization and effective multi-skill integration, verified in real-world experiments.

Conclusion: ATG-MoE provides a practical solution for multi-skill industrial assembly by directly incorporating visual information into trajectory generation and supporting efficient multi-skill integration within a single end-to-end model.
...
TL;DR: FaaS is an asynchronous co-simulation framework that adds high-fidelity fire dynamics to existing robot simulators, enabling realistic fire environment simulation for robot training and evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing robot simulators lack realistic fire environment modeling (thermal/optical complexity), hindering reliable capability evaluation and training data generation for firefighting robots before hazardous deployment.

Method: Asynchronous co-simulation framework that augments existing robot simulators with high-fidelity, computationally efficient fire simulations, providing accurate multi-species thermodynamic heat transfer and visually consistent volumetric smoke without disrupting rigid-body control loops.

Result: Framework integrates with diverse robot simulators to generate physically accurate fire behavior, benchmark thermal hazards, collect realistic multimodal perceptual data, supports real-time human-in-the-loop teleoperation, and enables successful training of reactive multimodal policies via Behavioral Cloning.
...
<details>
  <summary>Details</summary>
Motivation: Robotic systems require teams to collectively visit multiple targets while optimizing competing objectives like travel cost and makespan. Existing learning-based methods rarely address the combined challenges of multi-agent coordination and multi-objective trade-offs in MOMTSP.

Method: CAMO uses a conditional encoder to fuse preferences into instance representations for explicit trade-off control, and a collaborative decoder that coordinates agents by alternating agent selection and node selection to construct tours autoregressively. Trained with REINFORCE over mixed problem size distributions.

Result: CAMO outperforms both neural and conventional heuristics, achieving closer Pareto front approximations. Ablation studies validate key components, and real-world tests on mobile robots demonstrate practical applicability.

Conclusion: CAMO effectively addresses the dual complexity of multi-agent coordination and multi-objective optimization in MOMTSP, providing a scalable neural solver that generalizes across problem variations and yields high-quality Pareto front approximations.
...
TL;DR: ABD-Net is a graph neural network architecture that incorporates articulated-body dynamics as an inductive bias into policy learning, improving sample efficiency and generalization for robot control.


<details>
  <summary>Details</summary>
Motivation: While structural priors like link connectivity have been used in policy networks, dynamics properties remain underexplored as inductive biases despite their fundamental role in determining force and motion propagation through articulated robot bodies.

Method: ABD-Net adapts the inertia propagation mechanism from the Articulated Body Algorithm, systematically aggregating inertial quantities from child to parent links in a tree-structured manner while replacing physical quantities with learnable parameters. This dynamics-informed architecture is embedded into policy actors.
...
Conclusion: Incorporating articulated-body dynamics as an inductive bias through ABD-Net enables more efficient and robust policy learning for articulated robots, with successful sim-to-real transfer to state-of-the-art robotic platforms.
...
TL;DR: 3D-printed TPU continuum robots with tapered backbones, featuring integrated electronics for tendon control and sensing, plus a Cosserat rod-based kinetostatic model accounting for taper geometry.


<details>
  <summary>Details</summary>
Motivation: To create low-cost, customizable continuum robots that overcome limitations of single-purpose, expensive designs, enabling compliant inspection and manipulation tasks through geometric tapering for enhanced distal compliance.

Method: Developed tapered backbone continuum robots using TPU via FDM 3D printing with integrated electronics base. Created generalized forward kinetostatic model based on Cosserat rod theory using Newtonian approach, extending existing formulations to account for spatially varying cross-sectional geometry. Used parameterized iLogic/CAD scripts for rapid geometry generation.

Result: Achieved centimeter-level shape prediction accuracy after calibrating Young's modulus via line search. Demonstrated teleoperated grasping with endoscopic gripper. Analyzed how backbone taper angle influences configuration space and manipulability. Provided reproducible pathway from parametric design to controlled actuation.

Conclusion: The framework establishes a simple, rapid, reproducible approach for designing, modeling, and fabricating tapered tendon-driven continuum robots using FDM 3D printing, enabling systematic exploration of design parameters and supporting diverse compliant robotic applications.
...
TL;DR: M is an open-source, low-cost social robot platform designed to reduce research friction through modular design, ROS2-native software, and simulation support for rapid development and real-world deployment.
...
TL;DR: MAPG is a multi-agent probabilistic framework that decomposes complex metric-semantic language queries into structured components, uses VLMs to ground each component, and probabilistically composes them for metrically consistent 3D decisions.
...
Result: MAPG shows consistent performance improvements over strong baselines on HM-EQA benchmark, introduces a new MAPG-Bench for metric-semantic goal grounding evaluation, and demonstrates real-world robot transfer when structured scene representation is available.

Conclusion: MAPG addresses the limitation of VLMs in handling metric-semantic queries by providing a probabilistic composition framework that enables robots to convert natural language goals into actionable, physically grounded 3D decisions.
...
TL;DR: Decentralized MPC with CBF constraints for safe multi-robot trajectory planning using ADMM-based distributed optimization with node-edge splitting, enabling parallel computation and neighbor-only communication.
...
Result: Hardware experiments on two Unitree Go2 quadrupeds and simulations with up to four robots show comparable performance to centralized MPC while reducing average per-cycle planning time by up to 51% in four-agent case.

Conclusion: The proposed distributed formulation enables efficient real-time decentralized implementation of safety-critical trajectory planning for multi-robot legged systems while preserving safety and dynamic feasibility.
...
TL;DR: Mechanistic interpretability analysis of Vision-Language-Action models reveals they learn both memorized training sequences and generalizable motion primitives, with fine-tuning on small datasets amplifying memorization while larger diverse datasets promote generalization.


<details>
  <summary>Details</summary>
Motivation: To understand why VLA models show inconsistent generalization - performing well in some settings but failing on novel objects, scenes, and instructions - and to apply mechanistic interpretability techniques to uncover their inner workings.

Method: Train Sparse Autoencoders (SAEs) on hidden layer activations of VLA models to extract sparse dictionary features; propose metrics to categorize features as generalizable primitives vs. episode-specific memorization; validate through steering experiments on LIBERO benchmark.

Result: Most SAE features correspond to memorized training sequences, but some represent interpretable, general, steerable motion primitives and semantic properties; supervised fine-tuning on small datasets amplifies memorization, while larger diverse datasets (e.g., DROID) or knowledge insulation promote general features.

Conclusion: First mechanistic evidence that VLAs can learn generalizable features across tasks and scenes; provides tools for activation collection, SAE training, and feature steering with open-source codebase; demonstrates causal influence of individual features on robot behavior.
...
TL;DR: FASTER reduces VLA model reaction latency by prioritizing near-term actions during flow sampling, enabling single-step denoising for immediate reactions while maintaining long-horizon trajectory quality.


<details>
  <summary>Details</summary>
Motivation: Existing asynchronous inference methods for Vision-Language-Action models optimize trajectory smoothness but neglect critical reaction latency to environmental changes, creating a bottleneck in real-time deployment.

Method: Proposes FASTER (Fast Action Sampling for ImmediaTE Reaction) with Horizon-Aware Schedule that adaptively prioritizes near-term actions during flow sampling, compressing immediate reaction denoising into a single step while preserving long-horizon trajectory quality.
...
Conclusion: FASTER overcomes the reaction latency bottleneck in flow-based VLAs by rethinking action chunking policies, enabling rapid generation of accurate and smooth trajectories for real-time physical world deployment.
...
Method: OmniViTac dataset: 21,000+ trajectories across 86 tasks and 100+ objects organized into six physics-grounded interaction patterns. OmniVTA framework: four integrated modules - self-supervised tactile encoder, two-stream visuo-tactile world model for predicting short-horizon contact evolution, contact-aware fusion policy for action generation, and 60Hz reflexive controller that corrects deviations between predicted and observed tactile signals in closed loop.

Result: Real-robot experiments across all six interaction categories show that OmniVTA outperforms existing methods and generalizes well to unseen objects and geometric configurations, confirming the value of combining predictive contact modeling with high-frequency tactile feedback for contact-rich manipulation.

Conclusion: The combination of large-scale visuo-tactile dataset (OmniViTac) and world-model-based framework (OmniVTA) with predictive contact modeling and high-frequency tactile feedback enables effective contact-rich manipulation that generalizes to unseen scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Existing embodied navigation research primarily evaluates models under nominal conditions, overlooking real-world corruptions. There's a need to systematically assess robustness of VLN and OGN agents against realistic input corruptions to bridge the gap between lab performance and real-world deployment.

Method: Developed NavTrust benchmark that systematically corrupts input modalities (RGB, depth, instructions) in realistic scenarios. Evaluated seven state-of-the-art approaches and tested four distinct mitigation strategies against RGB-Depth and instruction corruptions. Used base models Uni-NaVid and ETPNav, with real-world deployment on a mobile robot.

Result: Extensive evaluation revealed substantial performance degradation under realistic corruptions, highlighting critical robustness gaps. Mitigation strategies showed improved robustness to corruptions, with successful real-world deployment demonstrating practical benefits.

Conclusion: NavTrust is the first unified benchmark exposing embodied navigation agents to diverse RGB-Depth corruptions and instruction variations, providing a roadmap toward more trustworthy embodied navigation systems by identifying robustness gaps and evaluating mitigation strategies.
...
TL;DR: VLA models primarily use visual pathways for action generation with spatially-bound motor programs, while language is task-dependent; expert pathways encode motor programs and VLM pathways encode goal semantics in multi-pathway architectures.


<details>
  <summary>Details</summary>
Motivation: To understand how Vision-Language-Action (VLA) models translate multimodal inputs into actions, as this remains poorly understood despite their integration of perception, language, and motor control.

Method: Applied activation injection, sparse autoencoders (SAEs), and linear probes to six models (80M-7B parameters) across 394,000+ rollout episodes on four benchmarks; used cross-task injection, subspace analysis, and causal ablation techniques.

Result: Visual pathways dominate action generation (99.8% trajectory alignment with source tasks); language sensitivity depends on task structure; expert pathways encode motor programs while VLM pathways encode goal semantics; per-token SAE processing essential for action fidelity; recovered 82+ manipulation concepts.

Conclusion: VLA models rely on spatially-bound motor programs tied to scene coordinates rather than abstract representations; task structure determines language usage; multi-pathway architectures separate motor and semantic encoding; released Action Atlas for interactive exploration of VLA representations.
