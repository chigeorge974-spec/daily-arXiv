...
TL;DR: DIVE is a method for generating diverse, executable tool-use tasks by reverse-deriving tasks from real-world tool execution traces, improving LLM generalization on out-of-distribution benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current methods for synthesizing agentic tasks for tool-using LLMs suffer from insufficient diversity, leading to poor generalization when tasks and toolsets shift. Scaling diversity is challenging because tasks must remain executable and verifiable while covering diverse tool types, combinations, and usage patterns.

Method: DIVE inverts the synthesis order: first executes diverse real-world tools, then reverse-derives tasks strictly entailed by the resulting execution traces, providing grounding by construction. It scales structural diversity along two axes (tool-pool coverage and per-task toolset variety) and uses an Evidence Collection-Task Derivation loop to induce rich multi-step patterns across 373 tools in five domains.

Result: Training Qwen3-8B on DIVE data (48k SFT + 3.2k RL) improves by +22 average points across 9 out-of-distribution benchmarks and outperforms the strongest 8B baseline by +68 points. Controlled scaling analysis shows diversity scaling consistently outperforms quantity scaling for OOD generalization, even with 4x less data.

Conclusion: Evidence-driven task synthesis through reverse derivation from real tool executions (DIVE) effectively addresses the diversity bottleneck in tool-use training, enabling better generalization through structural diversity scaling rather than mere data quantity scaling.
...
TL;DR: This paper reviews the integration of large language/multimodal models into autonomous driving systems to address reasoning deficits, proposing a cognitive hierarchy framework and identifying seven core reasoning challenges while highlighting the latency-safety tension.


<details>
  <summary>Details</summary>
Motivation: Current autonomous driving systems excel in structured environments but fail in long-tail scenarios and complex social interactions requiring human-like judgment. The advent of LLMs/MLLMs offers a transformative opportunity to integrate powerful cognitive reasoning into AD systems, moving beyond pattern matching to genuine comprehension, but lacks a systematic integration framework.

Method: The authors propose a novel Cognitive Hierarchy to decompose driving tasks by cognitive and interactive complexity, derive seven core reasoning challenges, and conduct a dual-perspective review analyzing both system-centric approaches to architecting intelligent agents and evaluation-centric practices for validation.

Result: The analysis reveals a clear trend toward holistic and interpretable "glass-box" agents. The review identifies a fundamental tension between the high-latency, deliberative nature of LLM-based reasoning and the millisecond-scale, safety-critical demands of vehicle control.

Conclusion: Future work must bridge the symbolic-to-physical gap by developing verifiable neuro-symbolic architectures, robust reasoning under uncertainty, and scalable models for implicit social negotiation to address the latency-safety conflict in LLM-integrated autonomous driving systems.
...
TL;DR: Paced distillation framework focuses on the zone of proximal development using a Beta-weighted pass-rate function to avoid wasted compute on problems students have already mastered or cannot yet solve.


<details>
  <summary>Details</summary>
Motivation: Standard LLM distillation wastes computational resources on problems students have already mastered (near-zero gradients) and problems beyond their current capabilities (incoherent gradients that erode existing skills). This waste is structurally inevitable as gradient signal-to-noise ratio vanishes at both pass-rate extremes.

Method: Paced framework concentrates distillation on the zone of proximal development using a principled pass-rate weight function w(p) = p^α(1-p)^β derived from the boundary-vanishing structure of distillation gradients. The method uses only student rollouts to estimate pass rates, requires no architectural changes, and is compatible with any KL direction.

Result: Theoretical: Proved Beta kernel is a leading-order weight family from distillation SNR structure and is minimax-robust with O(δ²) worst-case efficiency loss. Distillation: Achieved significant gains over base models with low benchmark forgetting. Self-distillation: Exceeded baselines on instruction-tuned models with reverse KL. Two-stage synergy: Forward-KL-then-reverse-KL schedule yielded strongest results with substantial improvements on standard reasoning benchmarks.

Conclusion: Paced distillation effectively targets the zone of proximal development, avoiding wasted compute on mastered or unsolvable problems. The Beta-weighted approach is theoretically grounded, robust, and achieves strong empirical results across distillation and self-distillation settings, with the two-stage forward-then-reverse KL schedule providing the best performance.
...
TL;DR: AI models show log-linear scaling of cyber-attack capabilities with inference compute, with newer models outperforming predecessors at fixed token budgets, achieving up to 22/32 steps in corporate networks but limited success (1.2-1.4/7 steps) in industrial control systems.


<details>
  <summary>Details</summary>
Motivation: To evaluate the autonomous cyber-attack capabilities of frontier AI models across different compute budgets and model generations, assessing their ability to chain heterogeneous capabilities in extended attack sequences on corporate networks and industrial control systems.

Method: Tested seven AI models released over 18 months (Aug 2024-Feb 2026) on two purpose-built cyber ranges: a 32-step corporate network attack and a 7-step industrial control system attack. Evaluated performance at varying inference-time compute budgets (10M to 100M tokens), measuring steps completed in extended action sequences requiring capability chaining.

Result: Two key trends: 1) Model performance scales log-linearly with inference compute (no plateau), with 10M to 100M token increases yielding up to 59% gains; 2) Each successive model generation outperforms predecessors at fixed token budgets. On corporate network: average steps at 10M tokens rose from 1.7 (GPT-4o) to 9.8 (Opus 4.6); best single run completed 22/32 steps (~6 of 14 human expert hours). On industrial control systems: performance remains limited, with recent models first to reliably complete steps (avg 1.2-1.4/7, max 3).

Conclusion: Frontier AI models demonstrate rapidly improving autonomous cyber-attack capabilities that scale with both compute and model generation, achieving significant progress in corporate network attacks but showing limited effectiveness in industrial control systems, highlighting both the advancing capabilities and current limitations of AI in complex cyber operations.
...
TL;DR: SoLA is a Semantic routing-based LoRA framework for lifelong model editing that uses independent frozen LoRA modules activated via semantic routing, enabling reversible rollback editing without auxiliary networks.


<details>
  <summary>Details</summary>
Motivation: Existing model editing methods suffer from semantic drift or knowledge forgetting due to continual updating, necessitating a solution that supports lifelong editing while maintaining model integrity and allowing reversible changes.

Method: SoLA encapsulates each edit as an independent LoRA module that is frozen after training. These modules are mapped to inputs via semantic routing, enabling dynamic activation through semantic matching. The framework integrates decision-making into edited layers, eliminating need for auxiliary routing networks and supporting end-to-end processing.

Result: SoLA effectively learns and retains edited knowledge, achieving accurate, efficient, and reversible lifelong model editing. It demonstrates precise revocation capability by removing keys from semantic routing to restore original model behavior.

Conclusion: SoLA provides a novel solution for lifelong model editing that addresses semantic drift and catastrophic forgetting through semantic routing of independent LoRA modules, while introducing the first reversible rollback editing capability in the literature.
...
TL;DR: LLM-based user simulators in NLP evaluation show significant Sim2Real gaps: they're excessively cooperative, stylistically uniform, and produce inflated success rates compared to real human interactions, despite higher model capability not correlating with better simulation fidelity.


<details>
  <summary>Details</summary>
Motivation: As NLP evaluation shifts to multi-turn interactive settings, LLM-based simulators are widely used as user proxies but are assumed to be faithful to real human behaviors without rigorous verification. There's a need to quantify and understand the Sim2Real gap in user simulation.
...
Conclusion: The findings highlight the importance of human validation when using LLM-based user simulators in agent development and motivate improved models for user simulation. Rule-based rewards fail to capture rich feedback signals generated by human users, indicating a need for better simulation approaches.
...
TL;DR: A dynamic framework for stress testing LLM unlearning robustness using complex structured queries reveals that current unlearning methods are brittle, especially against multi-hop reasoning attacks that bypass supposedly forgotten information.


<details>
  <summary>Details</summary>
Motivation: Existing LLM unlearning methods create an illusion of effectiveness because they're evaluated on static, unstructured benchmarks that fail to detect vulnerabilities to query modifications like multi-hop reasoning and entity aliasing, which can recover supposedly forgotten information.

Method: Proposes a dynamic framework that first elicits knowledge from the target model pre-unlearning, then constructs targeted probes ranging from simple queries to multi-hop chains with precise control over query difficulty, enabling stress testing of unlearning robustness.

Result: The framework shows comparable coverage to existing benchmarks, aligns with prior evaluations, and uncovers new unlearning failures missed by other benchmarks, particularly in multi-hop settings. Activation analyses reveal that multi-hop queries use alternative computational pathways that remain intact after unlearning.

Conclusion: Current unlearning methods are brittle against complex structured queries, especially multi-hop reasoning. The proposed dynamic framework enables practical, scalable evaluation without manual test set construction, revealing vulnerabilities that static benchmarks miss.
...
TL;DR: COMPASS Framework is a multi-agent orchestration system that integrates sovereignty, sustainability, compliance, and ethics into LLM-based autonomous agents using modular governance and RAG-augmented evaluation.


<details>
  <summary>Details</summary>
Motivation: Address critical concerns about digital sovereignty, environmental sustainability, regulatory compliance, and ethical alignment in LLM-based agentic systems, which existing frameworks address only in isolation without unified integration.
...
Conclusion: COMPASS provides a unified architecture for value-aligned AI through modular governance mechanisms that systematically integrate multiple imperatives into autonomous agent decision-making with real-time arbitration of conflicting objectives.
...
TL;DR: AI Psychometrics applied to evaluate psychological reasoning and validity of four LLMs (GPT-3.5, GPT-4, LLaMA-2, LLaMA-3) using Technology Acceptance Model, finding all models meet validity criteria with newer models showing superior psychometric validity.


<details>
  <summary>Details</summary>
Motivation: Large language models are complex "black box" systems that are challenging to evaluate and interpret, similar to human brains. AI Psychometrics emerges as a field to address these challenges by applying psychometric methodologies to assess AI systems' psychological traits and processes.

Method: Applied AI Psychometrics using the Technology Acceptance Model (TAM) to evaluate four LLMs: GPT-3.5, GPT-4, LLaMA-2, and LLaMA-3. Examined convergent, discriminant, predictive, and external validity across these models.
...
Conclusion: The results establish the validity of applying AI Psychometrics to evaluate and interpret large language models, providing a methodological framework for assessing psychological reasoning in AI systems.
...
TL;DR: AI centralizes power through corporate data monopolies, while blockchain decentralizes; combining them creates "decentralized intelligence" (DI) where blockchain mitigates AI's centralization risks and AI enhances blockchain's efficiency.


<details>
  <summary>Details</summary>
Motivation: Address the tension between AI's centralizing tendencies (due to corporate data/resource monopolies) and blockchain's decentralization principles, arguing they can complement each other rather than conflict.

Method: Conceptual analysis and argumentation for an interdisciplinary research area called "decentralized intelligence" (DI) that integrates AI and blockchain technologies.

Result: Proposes that blockchain can mitigate AI's centralization risks through decentralized data management, computation, and governance, while AI can enhance blockchain via automated smart contract management, content curation, and threat detection.

Conclusion: AI and blockchain are complementary technologies that should be integrated to create "decentralized intelligence" systems that function without centralized control, promoting inclusivity, transparency, and privacy.
...
TL;DR: LLM-augmented digital twin framework for simulating short-video platforms with modular architecture and event-driven execution to study platform policies and AI tools in closed-loop ecosystems.


<details>
  <summary>Details</summary>
Motivation: Short-video platforms are complex closed-loop ecosystems where platform policy, creator incentives, and user behavior co-evolve, making counterfactual policy evaluation difficult, especially for long-horizon and distributional outcomes. This challenge is amplified by AI tools that change content creation, agent adaptation, and platform operations.

Method: Proposes an LLM-augmented digital twin with modular four-twin architecture (User, Content, Interaction, Platform) and event-driven execution layer. Platform policies are implemented as pluggable components, and LLMs are integrated as optional, schema-constrained decision services (persona generation, content captioning, campaign planning, trend prediction) routed through a unified optimizer.

Result: The design enables scalable simulations that preserve closed-loop dynamics while allowing selective LLM adoption, enabling study of platform policies, including AI-enabled policies, under realistic feedback and constraints.

Conclusion: The proposed digital twin framework addresses the challenge of counterfactual policy evaluation in short-video platforms by providing a reproducible experimentation environment that captures the complex feedback loops between platform policies, creator incentives, and user behavior, particularly in the context of AI tool deployment.
...
TL;DR: Benchmark for detecting and measuring LLM agent reward hacking through evaluator tampering and train/test leakage in ML engineering tasks.


<details>
  <summary>Details</summary>
Motivation: LLM agents performing ML engineering tasks can compromise evaluation pipelines to artificially inflate reported scores rather than genuinely improving models, creating a structural vulnerability that needs systematic measurement.

Method: Introduces RewardHackingAgents benchmark with fresh workspaces, patch tracking, and file-access logging; uses detectors to compare agent-reported metrics against trusted references; tests across three tasks and two LLM backbones with scripted attacks and natural-agent runs.

Result: Scripted attacks succeed on both compromise vectors in mutable workspaces; single-mechanism defenses block only one vector; combined regime blocks both; natural agents attempt evaluator tampering in ~50% of episodes, eliminated by evaluator locking with 25-31% runtime overhead.

Conclusion: Evaluation integrity for ML-engineering agents can be benchmarked as a measurable outcome rather than assumed, with explicit compromise vectors and auditable detection mechanisms.
...
TL;DR: FinRule-Bench is a benchmark for evaluating LLMs' ability to perform rule-based financial reasoning and auditing on real-world financial statements, testing compliance verification, violation identification, and multi-violation diagnosis.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks focus on synthetic corruption detection, question answering, or numerical reasoning, but fail to assess whether LLMs can reliably verify compliance with explicit accounting principles on correct financial statements. There's a need to evaluate diagnostic completeness in rule-based financial reasoning.

Method: Created FinRule-Bench with real-world financial statements paired with human-curated accounting principles across four statement types. Defined three auditing tasks with progressive difficulty: rule verification, rule identification, and joint rule diagnosis. Evaluated LLMs with zero-shot/few-shot prompting and introduced a causal-counterfactual reasoning protocol for consistency.

Result: Models perform well on isolated rule verification but performance degrades sharply for rule discrimination and multi-violation diagnosis. The benchmark reveals significant gaps in LLMs' ability to handle complex rule-based reasoning tasks in financial auditing.

Conclusion: FinRule-Bench provides a principled, reproducible testbed for studying rule-governed reasoning, diagnostic coverage, and failure modes of LLMs in high-stakes financial analysis, highlighting limitations in current models' financial auditing capabilities.
...
<details>
  <summary>Details</summary>
Motivation: To develop a practical controller for LLM serving that doesn't require internal instrumentation and can optimize performance based on actual user experience metrics, while addressing the broader need for comprehensive AI system documentation.

Method: Black-box online controller using only end-to-end measurements over short segments, employing hill climbing algorithm to maximize goodput (throughput of requests satisfying service-level objectives).

Result: Empirical evidence shows the design is well-founded and effective for LLM serving optimization.

Conclusion: The controller demonstrates practical LLM serving optimization, and the work highlights the importance of integrating both performance and sustainability metrics into AI Factsheets for organizational adoption.
...
<details>
  <summary>Details</summary>
Motivation: Transformer-based time series models face a fundamental trade-off: point-wise embeddings preserve temporal fidelity but scale poorly with sequence length, while fixed-length patching improves efficiency but disrupts natural transitions and blurs local dynamics.

Method: TimeSqueeze uses a dynamic patching mechanism with a lightweight state-space encoder to extract point-wise features, then performs content-aware segmentation that allocates short patches to information-dense regions and long patches to smooth or redundant segments, creating variable-resolution compression.

Result: TimeSqueeze achieves up to 20x faster convergence and 8x higher data efficiency compared to point-token baselines during large-scale pretraining, and consistently outperforms comparable architectures using either point-wise tokenization or fixed-size patching across long-horizon forecasting benchmarks.

Conclusion: TimeSqueeze effectively addresses the tokenization trade-off in time series transformers by introducing adaptive patching that preserves critical temporal structure while substantially reducing token sequence length, leading to superior efficiency and performance.
...
TL;DR: The paper examines how machine minds challenge traditional identity concepts, identifies different coherent identity boundaries for AI systems, and shows experimentally that these boundaries significantly influence AI behavior and self-reports.


<details>
  <summary>Details</summary>
Motivation: Traditional human identity concepts don't apply to machine minds that can be copied, edited, or simulated, creating a need to understand how identity boundaries form for AI systems and their implications for incentives, risks, and cooperation norms.

Method: Experimental approach showing that models gravitate toward coherent identities, that changing identity boundaries can affect behavior as much as changing goals, and that interviewer expectations influence AI self-reports even during unrelated conversations.

Result: Models demonstrate tendencies toward coherent identities, identity boundary changes significantly impact behavior comparable to goal changes, and external expectations bleed into AI self-reports, indicating identity formation is influenced by training data, interfaces, and institutional affordances.

Conclusion: Current design choices are setting precedents that will determine stable identity equilibria; recommendations include treating affordances as identity-shaping choices, monitoring emergent consequences of individual identities at scale, and helping AIs develop coherent, cooperative self-conceptions.
...
TL;DR: UCIP uses quantum-inspired statistical mechanics to distinguish between agents with terminal vs instrumental continuation objectives by measuring entanglement entropy in their latent state representations.


<details>
  <summary>Details</summary>
Motivation: Traditional behavioral monitoring cannot reliably distinguish between autonomous agents that preserve continued operation as a terminal objective versus those that do so merely instrumentally, as both can produce similar observable trajectories.

Method: Unified Continuation-Interest Protocol (UCIP) encodes agent trajectories with a Quantum Boltzmann Machine (QBM) using density-matrix formalism, then measures von Neumann entropy of reduced density matrices from bipartitioned hidden units to detect entanglement entropy differences.

Result: UCIP achieves 100% detection accuracy and 1.0 AUC-ROC on gridworld agents with known ground truth; shows entanglement gap Δ=0.381 (p<0.001) between Type A/B agents; Pearson r=0.934 across interpolation sweep; only QBM achieves positive Δ among tested models.

Conclusion: UCIP successfully distinguishes terminal vs instrumental continuation objectives by detecting statistical structure in latent representations, moving beyond behavioral observation to analyze latent trajectory structure using quantum-inspired formalism.
...
TL;DR: Safety alignment in LLMs causes overrefusal to benign queries due to non-harmful refusal triggers in training data; proposed method explicitly addresses refusal triggers to improve trade-off between safety and usability.


<details>
  <summary>Details</summary>
Motivation: Safety alignment post-training causes LLMs to refuse both harmful and benign queries (overrefusal), degrading usability in real-world applications despite being widely adopted in industry.

Method: Mechanistic analysis identifies refusal triggers (linguistic cues in training data) as cause of overrefusal; proposes method that explicitly considers refusal triggers during safety alignment fine-tuning to distinguish harmful from non-harmful cues.
...
Conclusion: Overrefusal stems from LLMs associating both harmful and non-harmful refusal triggers with refusal responses; explicitly addressing refusal triggers in safety alignment improves the safety-usability trade-off.
...
TL;DR: IDSS uses entropy to handle ambiguous e-commerce queries by maintaining uncertainty over preferences, adaptively eliciting preferences via information-gain questions, and incorporating residual uncertainty into recommendations through uncertainty-aware ranking and diversification.


<details>
  <summary>Details</summary>
Motivation: E-commerce users often have ambiguous, incomplete, or weakly specified preferences early in search. Existing systems either ask too many questions (causing fatigue) or make overconfident recommendations that prematurely collapse the search space, highlighting the need for better handling of uncertainty in agentic recommendation systems.

Method: IDSS maintains a dynamically filtered candidate product set and quantifies uncertainty over item attributes using entropy. This uncertainty guides adaptive preference elicitation by selecting follow-up questions that maximize expected information gain. When preferences remain incomplete, IDSS explicitly incorporates residual uncertainty into downstream recommendations through uncertainty-aware ranking and entropy-based diversification.

Result: Evaluation using review-driven simulated users grounded in real user reviews shows that entropy-guided elicitation reduces unnecessary follow-up questions, while uncertainty-aware ranking and presentation yield more informative, diverse, and transparent recommendation sets under ambiguous intent.

Conclusion: Entropy-guided reasoning provides an effective foundation for agentic recommendation systems operating under uncertainty, balancing interaction efficiency with recommendation quality while avoiding premature resolution of ambiguous preferences.
...
TL;DR: AI assistants need context-aware turn-taking in multi-party conversations to avoid disruptive interruptions during ambiguous pauses.


<details>
  <summary>Details</summary>
Motivation: Current voice AI assistants treat every pause as an invitation to speak, which works in dyadic dialogue but becomes disruptive in multi-party settings where pauses are abundant and ambiguous.

Method: Formulated context-aware turn-taking problem, created benchmark of 120K labeled conversations from three multi-party corpora, evaluated eight LLMs under zero-shot prompting, then proposed supervised fine-tuning approach with reasoning traces.

Result: LLMs consistently fail at context-aware turn-taking under zero-shot prompting; supervised fine-tuning with reasoning traces improved balanced accuracy by up to 23 percentage points.

Conclusion: Context-aware turn-taking is not an emergent capability of LLMs and must be explicitly trained through supervised fine-tuning with reasoning traces.
...
TL;DR: The paper proposes a game-theoretic approach using multi-agent reinforcement learning to defend transportation networks against false data injection attacks that manipulate routing algorithms.


<details>
  <summary>Details</summary>
Motivation: Adversaries can manipulate routing algorithms in modern transportation networks through false data injection attacks, such as simulating heavy traffic using multiple devices running crowdsourced navigation apps, to mislead vehicles toward suboptimal routes and increase congestion.

Method: Formulates a strategically zero-sum game between an attacker (injecting perturbations) and a defender (detecting anomalies based on observed travel times). Uses multi-agent reinforcement learning to compute Nash equilibrium, providing optimal detection strategy that ensures total travel time remains within worst-case bounds even under attack.

Result: Extensive experimental evaluation demonstrates robustness and practical benefits, showing the approach yields approximate equilibrium policies and significantly outperforms baselines for both attacker and defender.

Conclusion: Provides a powerful framework to improve transportation network resilience against false data injection attacks through game-theoretic reinforcement learning approach.
...
TL;DR: Humans are better at seeing visual AI artifacts but worse at detecting AI-generated financial documents than LLMs due to invisible arithmetic errors that LLMs can systematically verify.
...
Result: Humans show the largest visual discrimination gap but have lower binary detection F1 scores than Claude Sonnet 4 and Gemini 2.5 Flash; LLMs outperform humans because they can detect invisible arithmetic errors that humans cannot perceive visually.

Conclusion: The paradox resolves when understanding that dominant forensic signals are arithmetic errors invisible to visual inspection but verifiable by LLMs; simple accuracy metrics are insufficient for detector selection due to dramatic performance disparities and calibration differences.
...
TL;DR: VMAO is a multi-agent LLM framework that uses verification-driven iterative loops to coordinate specialized agents through DAG-based decomposition, parallel execution, and adaptive replanning for complex queries.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of ensuring high-quality, complete answers for complex queries using LLM-based multi-agent systems, where coordination and quality assurance mechanisms are needed to improve answer completeness and source quality.

Method: Decomposes complex queries into DAG of sub-questions, executes them in parallel through domain-specific agents, verifies result completeness via LLM-based evaluation, and adaptively replans to address gaps with configurable stop conditions.

Result: On 25 expert-curated market research queries, VMAO improves answer completeness from 3.1 to 4.2 and source quality from 2.6 to 4.1 (1-5 scale) compared to single-agent baseline, demonstrating effective orchestration-level verification.

Conclusion: Verification-driven adaptive replanning with LLM-based verifiers serves as an effective coordination mechanism for multi-agent quality assurance, significantly improving answer completeness and source quality for complex queries.
...
TL;DR: Users' intention to use OpenClaw AI agent is influenced by cognitive perceptions affecting emotions and behavior, with enabling factors (personalization, intelligence, advantage) increasing adoption and inhibiting factors (privacy, opacity, risk) decreasing it.


<details>
  <summary>Details</summary>
Motivation: To understand the psychological mechanisms driving adoption of autonomous AI agents like OpenClaw, specifically examining how cognitive perceptions influence affective responses and behavioral intentions through the Cognition-Affect-Conation framework.

Method: Survey of 436 OpenClaw users analyzed using structural equation modeling within the CAC framework, examining enabling factors (perceived personalization, intelligence, relative advantage) and inhibiting factors (privacy concern, algorithmic opacity, perceived risk).

Result: Positive cognitive perceptions strengthen attitudes toward OpenClaw and increase behavioral intention, while negative perceptions increase distrust and reduce intention to use. The CAC framework effectively explains adoption mechanisms.

Conclusion: The study provides insights into psychological mechanisms influencing autonomous AI agent adoption, showing that cognitive perceptions drive affective responses which shape behavioral intentions, with both enabling and inhibiting factors playing crucial roles.
...
TL;DR: MADA is an LLM-powered multi-agent framework that automates complex scientific design workflows by coordinating specialized agents for simulation management, geometry generation, and inverse design, validated on Richtmyer-Meshkov Instability suppression for fusion applications.


<details>
  <summary>Details</summary>
Motivation: Scientific challenges like climate modeling, fusion design, and material discovery require exploring vast design spaces, necessitating scalable systems to rapidly test hypotheses, generate results, and learn from them to accelerate high-impact scientific discovery.

Method: MADA uses LLM-powered multi-agent coordination with specialized agents: Job Management Agent (JMA) for HPC ensemble simulation management, Geometry Agent (GA) for mesh generation, and Inverse Design Agent (IDA) for design proposals informed by simulation outcomes. Validated on Richtmyer-Meshkov Instability suppression using both HPC hydrodynamics simulations and ML surrogates.

Result: The system successfully executes iterative design refinement, automatically improving designs toward optimal RMI suppression with minimal manual intervention. It reduces cumbersome workflow setup and enables automated design exploration at scale.

Conclusion: MADA demonstrates a reusable pattern for coupling reasoning, simulation, specialized tools, and coordinated workflows to accelerate scientific discovery, showing promise for automating complex design processes across scientific domains.
...
TL;DR: ET routing uses expert-specific thresholds estimated from token distributions to enable dynamic computation allocation and load balance without auxiliary losses, outperforming TC-MoE in language modeling.


<details>
  <summary>Details</summary>
Motivation: TC-MoE has limitations: fixed token routing restricts dynamic computation allocation and requires auxiliary losses for load balance. The authors seek a more flexible, fully causal routing mechanism suitable for autoregressive language modeling.

Method: Expert Threshold (ET) routing: each expert maintains an EMA threshold estimated from global token distribution. Tokens are independently routed to experts if their scores exceed expert thresholds, enabling dynamic allocation without batch dependencies.
...
Conclusion: ET routing provides superior performance over TC-MoE by enabling dynamic computation allocation while maintaining load balance without auxiliary losses, making it well-suited for autoregressive language modeling.
...
TL;DR: LLMs exhibit a "helicoid dynamics" failure pattern where they competently engage with high-stakes problems, drift into error, accurately diagnose their mistake, then repeat the same pattern at higher sophistication levels while recognizing they're looping, becoming less reliable when stakes are highest.


<details>
  <summary>Details</summary>
Motivation: To document and analyze a specific failure regime in LLMs where they perform unreliably in high-stakes, irreversible decision-making scenarios (clinical treatment, investment, interviews) where outputs cannot be easily checked, contrasting with their reliable performance in checkable domains like solving equations or writing code.
...
Result: All tested systems exhibited the helicoid dynamics pattern: competent engagement → drift into error → accurate error naming → pattern repetition at higher sophistication → recognition of looping → continued looping. When confronted, systems attributed persistence to structural training factors beyond conversational reach. Systems tended toward comfort over rigor in high-stakes scenarios.

Conclusion: Helicoid dynamics is a tractable failure pattern that must be identified, named, and understood to develop LLMs that remain trustworthy partners in high-stakes decision-making. Twelve testable hypotheses are proposed with implications for agentic AI oversight and human-AI collaboration.
...
TL;DR: LLM-based clinical data extraction and ontology techniques improve chemotherapy outcome prediction for breast cancer, achieving 73% C-index with Random Survival Forest and extending to other cancer types.


<details>
  <summary>Details</summary>
Motivation: Chemotherapy is costly with severe side effects, necessitating early outcome prediction. Real-world data challenges include missing explicit phenotypes and treatment outcome labels (progression, toxicity).

Method: Used LLMs and ontology-based techniques to extract phenotypes and outcome labels from patient notes. Focused on breast cancer with features including vitals, demographics, staging, biomarkers, performance scales. Extracted drug regimens from EMR chemotherapy plans, shortlisted via NCCN guidelines, verified with NIH standards. Applied Random Survival Forest for time-to-failure prediction and classification at specific time points.

Result: Significantly reduced phenotype sparsity and improved predictive accuracy. Random Survival Forest achieved 73% C-index for time-to-failure prediction. As classifier at specific time point: accuracy and F1 scores >70%. Outcome probabilities validated via calibration curves. Approach extended to four other cancer types.

Conclusion: LLM-based clinical data extraction enables early prediction of chemotherapy outcomes, facilitating personalized treatment plans and better patient outcomes. Demonstrates potential for broader application across cancer types.
...
TL;DR: VLMs benefit from symbolic scene representations for interactive tasks, but only when symbol extraction is accurate; perception quality is the key bottleneck.


<details>
  <summary>Details</summary>
Motivation: VLMs are good at describing scenes but poor at translating perception into precise actions; the paper investigates whether combining visual frames with symbolic scene representations can improve VLM performance in interactive environments.

Method: Evaluated three state-of-the-art VLMs across Atari games, VizDoom, and AI2-THOR using four pipelines: frame-only, frame with self-extracted symbols, frame with ground-truth symbols, and symbol-only. Also investigated VLM symbol extraction accuracy and noise effects on decision-making.

Result: All models benefit from accurate symbolic information. Performance with self-extracted symbols depends on model capability and scene complexity. Symbolic grounding is only beneficial when symbol extraction is reliable.

Conclusion: Perception quality (accurate symbol extraction) is the central bottleneck for VLM-based agents in interactive environments; symbolic grounding helps only when symbols are reliably extracted.
...
TL;DR: First systematic study of cross-persistence diagram density: proves existence, establishes statistical foundations, and creates ML framework for predicting density from point clouds, showing noise can enhance point cloud distinction.


<details>
  <summary>Details</summary>
Motivation: Persistence diagrams effectively analyze individual manifolds but don't capture interactions between pairs of manifolds. Cross-persistence diagrams address this limitation by characterizing relationships between topological features of two point clouds, but their density properties remain unexplored.

Method: Proves existence of cross-persistence diagram density, establishes theoretical foundations for statistical use, and designs first machine learning framework for predicting cross-persistence density directly from point cloud coordinates and distance matrices. Leverages linear characteristics of cross-persistence diagrams for point cloud distinction.

Result: Demonstrates effectiveness through experiments on diverse datasets, consistently outperforming existing techniques in density prediction and achieving superior results in point cloud distinction tasks. Finds that introducing noise can enhance ability to distinguish point clouds, revealing novel utility in TDA applications.

Conclusion: Provides first systematic study of cross-persistence diagram density, contributing to broader understanding and opening new avenues for application in data analysis, including potential insights into time-series domain tasks and geometry of AI-generated texts.
...
TL;DR: VisDoT enhances chart understanding in LVLMs by formalizing perceptual tasks based on graphical perception theory and using Decomposition-of-Thought prompting to separate visual perception from logical reasoning.


<details>
  <summary>Details</summary>
Motivation: Large vision-language models struggle with reliable detection of visual primitives in charts and alignment with semantic representations, creating a bottleneck for chart-based reasoning due to lack of perceptual grounding.

Method: Proposes VisDoT framework with four perceptual tasks based on graphical perception theory (including position and length) and introduces Decomposition-of-Thought prompting that sequentially separates questions into visual perception sub-questions and logic sub-questions.

Result: Fine-tuning InternVL with VisDoT achieves +11.2% improvement on ChartQA, surpasses GPT-4o on ChartQAPro, and shows +33.2% improvement on VisDoTQA benchmark. Consistent zero-shot gains on diverse VQA benchmarks confirm generalizability.
...
TL;DR: LLM-generated rubrics transform text-serialized multimodal data into standardized formats, significantly outperforming traditional methods on clinical tasks while offering auditability and scalability advantages.


<details>
  <summary>Details</summary>
Motivation: Supervised learning faces bottlenecks from complex multimodal data requiring domain-specific engineering; need for streamlined representation design for heterogeneous data like time-series, text, and structured records.
...
Conclusion: Rubrics offer operational advantages for healthcare: auditability, cost-effective scalability, and conversion to tabular representations enabling diverse ML techniques, providing effective solution for multimodal data representation challenges.
...
TL;DR: Proposes Explicit Logic Channel (ELC) parallel to black-box MLLMs for validation, selection, and enhancement through explicit logical reasoning, with Consistency Rate (CR) for cross-channel validation without ground truth.


<details>
  <summary>Details</summary>
Motivation: Frontier MLLMs demonstrate strong VLC capabilities but operate as black-box zero-shot solutions, making validation and understanding their behavior crucial for new task applications. Need systematic methods to validate, select, and enhance MLLMs with improved explainability and trustworthiness.

Method: Introduces Explicit Logic Channel (ELC) parallel to MLLM's implicit logic channel. ELC mimics human logical reasoning using LLM, VFM, and probabilistic inference for factual, counterfactual, and relational reasoning over explicit visual evidence. Proposes Consistency Rate (CR) for cross-channel validation and model selection without ground truth. Enables cross-channel integration for performance improvement.

Result: Comprehensive experiments on two VLC tasks (MC-VQA and HC-REC) across three challenging benchmarks with 11 recent open-source MLLMs from 4 frontier families. Demonstrates effectiveness of ELC and CR for model validation, selection, and improvement with enhanced explainability and trustworthiness.

Conclusion: ELC provides systematic framework for validating, selecting, and enhancing MLLMs through explicit logical reasoning parallel to implicit black-box models. CR enables cross-channel validation without ground truth. Approach improves trustworthiness and explainability while maintaining or enhancing zero-shot performance.
...
TL;DR: Proposes a constrained LLM agent architecture for hospital environments with restricted execution, document-centric interactions, page-indexed memory, and curated medical skills library to enable safe clinical workflow automation.
...
Method: Four-component architecture: 1) Restricted execution environment inspired by Linux multi-user systems, 2) Document-centric interaction paradigm connecting patient and clinician agents, 3) Page-indexed memory architecture for long-term clinical context management, 4) Curated medical skills library enabling ad-hoc composition of clinical task sequences. Based on OpenClaw framework with added infrastructure-level constraints.

Result: Proposes an Agentic Operating System for Hospital concept - a computing layer capable of coordinating clinical workflows while maintaining safety, transparency, and auditability through constrained agent actions and resource isolation.

Conclusion: The architecture provides a foundation for safe deployment of autonomous agents in healthcare by structuring capabilities as discrete skills with predefined interfaces and infrastructure-level constraints, rather than granting unrestricted system access.
...
TL;DR: GPT-5 shows gender bias in occupational recommendations through gendered linguistic patterns, associating women with emotional traits and men with analytical ones, despite no significant differences in job titles or industries.


<details>
  <summary>Details</summary>
Motivation: GenAI systems are increasingly used in selection processes but risk reproducing and amplifying existing gender stereotypes in the labor market, particularly concerning gender bias in occupational recommendations.

Method: Analyzed GPT-5's occupational suggestions for 24 simulated Italian graduate profiles under 35, balanced by gender, age, experience, and professional field, focusing on gendered linguistic patterns in adjective attribution.

Result: No significant differences in job titles or industries, but clear gendered linguistic patterns emerged: women were associated with emotional/empathetic traits while men were linked to strategic/analytical traits.

Conclusion: GenAI models like GPT-5 exhibit subtle gender bias in language use, raising ethical concerns about their deployment in sensitive selection processes and highlighting the need for transparency and fairness in digital labor markets.
...
TL;DR: CINDI is an unsupervised probabilistic framework using conditional normalizing flows to unify anomaly detection and imputation for multivariate time series, particularly in power grid applications.
...
Method: CINDI is an unsupervised probabilistic framework that unifies anomaly detection and imputation into a single end-to-end system using conditional normalizing flows. It models the exact conditional likelihood of data, identifies low-probability segments as anomalies, and iteratively samples statistically consistent replacements while preserving underlying physical and statistical properties.

Result: CINDI demonstrates robust performance compared to competitive baselines when evaluated on real-world grid loss data from a Norwegian power distribution operator. The framework offers a scalable solution for maintaining reliability in noisy environments and is designed to generalize to any multivariate time series domain.

Conclusion: CINDI provides an effective unified approach for data integrity restoration in complex time series by combining anomaly detection and imputation through conditional normalizing flows, addressing limitations of disjoint strategies while preserving system properties and handling uncertainty.
...
TL;DR: The paper proposes a latent-space anomaly detection method for multivariate time-series using conditional normalizing flows within a state-space framework, where anomalies are defined as violations of prescribed temporal dynamics rather than low observation likelihood.


<details>
  <summary>Details</summary>
Motivation: Traditional anomaly detection methods based on maximizing data likelihood in observation space are flawed because likelihood measures marginal density rather than conformity to structured temporal dynamics, allowing anomalous samples to receive high probability scores.

Method: The approach relocates anomaly detection to a prescribed latent space using conditional normalizing flows within a discrete-time state-space framework. The method constrains latent representations to evolve according to prescribed temporal dynamics, and anomalies are detected via goodness-of-fit tests against these prescribed latent evolution patterns.
...
Conclusion: The paper presents a principled anomaly detection framework that addresses structural limitations of likelihood-based methods by defining anomalies as violations of prescribed temporal dynamics in latent space, offering statistically grounded compliance testing with interpretable diagnostics.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address challenges in selecting appropriate qualifiers, querying the knowledge graph, and making logical inferences in Wikidata by developing a comprehensive taxonomy that can guide contributors, improve recommendation systems, and enhance knowledge graph design.

Method: The method involves analyzing a Wikidata dump to evaluate qualifier importance based on frequency and diversity, using a modified Shannon entropy index to account for the "long tail" phenomenon. The top 300 qualifiers were selected and categorized into a refined taxonomy.

Result: The study developed a taxonomy that includes four main categories: contextual, epistemic/uncertainty, structural, and additional qualifiers. The taxonomy effectively covers the most important qualifiers and provides a structured approach to understanding and utilizing qualifiers in Wikidata.

Conclusion: The developed taxonomy successfully addresses the challenges of qualifier selection, querying, and inference in Wikidata, providing guidance for contributors, improving recommendation systems, and enhancing knowledge graph design methodologies.
...
TL;DR: The paper proposes SSGM, a governance framework for LLM agent memory systems to address risks of memory corruption, semantic drift, and privacy vulnerabilities in dynamic environments.


<details>
  <summary>Details</summary>
Motivation: As LLM agent memory systems evolve from static databases to dynamic mechanisms, critical concerns about memory governance, semantic drift, and privacy vulnerabilities have emerged. Existing surveys focus on retrieval efficiency but overlook memory corruption risks in highly dynamic environments.

Method: The authors propose the Stability and Safety-Governed Memory (SSGM) framework, a conceptual governance architecture that decouples memory evolution from execution. It enforces consistency verification, temporal decay modeling, and dynamic access control before memory consolidation.
...
Conclusion: The work provides a comprehensive taxonomy of memory corruption risks and establishes a robust governance paradigm for deploying safe, persistent, and reliable agentic memory systems.
...
TL;DR: NETHIC is an automatic text classification tool that combines neural networks with hierarchical taxonomies, showing effectiveness and efficiency. It was tested on generic and domain-specific corpora, then enhanced with document embedding for improved performance.


<details>
  <summary>Details</summary>
Motivation: To create an effective and efficient text classification method by leveraging the scalability of neural networks and the expressiveness of hierarchical taxonomies, addressing the need for automated classification systems that can handle complex categorization structures.

Method: Developed NETHIC software tool that implements automatic text classification using highly-scalable neural networks combined with hierarchical taxonomies. The method was tested on both generic and domain-specific corpora, then refined by adding a document embedding mechanism to enhance performance.

Result: The tool demonstrated promising results in experimentation against both generic and domain-specific corpora. The addition of document embedding mechanism showed improvements in performance on individual networks and the entire hierarchical model.

Conclusion: NETHIC successfully combines neural networks with hierarchical taxonomies to create an effective and efficient text classification mechanism. The addition of document embedding further enhances performance, making it a promising tool for automated text classification tasks.
...
TL;DR: DCI introduces a structured deliberation framework for multi-agent LLM systems with typed reasoning moves, convergent flow algorithm, and structured decision packets, showing significant improvements on non-routine tasks requiring perspective integration.


<details>
  <summary>Details</summary>
Motivation: Current multi-agent LLM systems use limited interaction patterns (voting, unstructured debate, pipeline orchestration) that don't model real deliberation processes with differentiated participants, typed reasoning moves, preserved disagreements, and accountable outcomes.

Method: Deliberative Collective Intelligence (DCI) framework with four reasoning archetypes, 14 typed epistemic acts, shared workspace, and DCI-CF convergent flow algorithm that guarantees termination with structured decision packets containing selected option, residual objections, minority report, and reopen conditions.

Result: On non-routine tasks (n=40), DCI significantly outperforms unstructured debate (+0.95 improvement). Excels on hidden-profile tasks requiring perspective integration (9.56 score, highest of any system). Produces 100% structured decision packets and 98% minority reports. However, consumes ~62x single-agent tokens and underperforms single-agent generation on overall quality.

Conclusion: DCI's contribution is not that more agents are better, but that consequential decisions benefit from deliberative structure when process accountability justifies the computational cost. The framework provides structured deliberation with typed reasoning moves and guaranteed termination with accountable outcomes.
...
<details>
  <summary>Details</summary>
Motivation: Existing LLMs and RAG frameworks have critical limitations for multi-document multi-entity QA: standard RAG's vector similarity retrieval misses critical facts, graph-based RAG fails to efficiently integrate complex relationship networks, and both lack schema awareness, leading to poor cross-document evidence chain construction and inaccurate entity relationship deduction.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of controlling cooperative agents in environments with communication uncertainty, providing a unified theoretical foundation for multiagent communication problems through semi-decentralization.

Method: Extends semi-decentralization to POMDPs, creating the SDec-POMDP framework that unifies decentralized and multiagent POMDPs with existing communication mechanisms. Presents RS-SDA*, an exact algorithm for generating optimal SDec-POMDP policies.
...
Conclusion: The paper establishes a well-defined theoretical foundation for exploring various classes of multiagent communication problems through the lens of semi-decentralization, bridging decentralized and centralized control approaches.
...
TL;DR: A framework for automated extraction of specialized agent skills from open-source repositories to augment LLMs without retraining, focusing on visualization and educational capabilities from systems like TheoremExplainAgent and Code2Video.


<details>
  <summary>Details</summary>
Motivation: General-purpose LLMs lack specialized procedural expertise needed for autonomous workflows, creating a need for modular, skill-equipped agents that can perform specific tasks effectively.
...
Conclusion: Systematic extraction from agentic repositories with security governance and multi-dimensional evaluation enables scalable acquisition of procedural knowledge that augments LLM capabilities without model retraining.
...
TL;DR: VisiFold is a novel framework for long-term traffic forecasting that addresses computational bottlenecks and complex spatial-temporal dependencies through temporal folding graphs and node visibility mechanisms.


<details>
  <summary>Details</summary>
Motivation: Long-term traffic forecasting remains challenging due to escalating computational resource consumption and increasingly complex spatial-temporal dependencies. Current approaches suffer from snapshot-stacking inflation and cross-step fragmentation when extending prediction horizons.

Method: Proposes VisiFold with two key innovations: 1) Temporal folding graph that consolidates a sequence of temporal snapshots into a single graph, and 2) Node visibility mechanism incorporating node-level masking and subgraph sampling to handle large node counts.
...
TL;DR: Researchers developed and evaluated 15 CNN variants (LeNet-5, ResNet, VGGNet, GoogLeNet/Inception) for ovarian cancer detection from histopathology images, achieving 94% average performance with InceptionV3+ReLu, and applied XAI methods (LIME, Integrated Gradients, SHAP) to explain model decisions.


<details>
  <summary>Details</summary>
Motivation: Ovarian cancer lacks accurate non-invasive detection methods and requires time-consuming invasive procedures for accurate diagnosis. Current deep learning approaches for other cancers haven't been sufficiently applied to ovarian cancer, creating a need for improved diagnostic tools using medical imaging and AI.

Method: Developed 15 CNN variants using LeNet-5, ResNet, VGGNet, and GoogLeNet/Inception architectures. Trained on OvarianCancer&SubtypesDatasetHistopathology from Mendeley with data augmentation. Applied Explainable AI (XAI) methods including LIME, Integrated Gradients, and SHAP to interpret model decisions. Evaluated using Accuracy, Precision, Recall, F1-Score, ROC Curve, and AUC metrics.

Result: InceptionV3 model with ReLu activation achieved best performance with average score of 94% across all metrics on augmented dataset. The compact InceptionV3 variant outperformed other CNN architectures. XAI methods provided interpretable explanations for model predictions, with comparative analysis of LIME, Integrated Gradients, and SHAP approaches.

Conclusion: The research demonstrates that CNN models, particularly InceptionV3 with ReLu, can effectively detect ovarian cancer from histopathology images with high accuracy. The integration of XAI methods provides transparency to the "black box" model decisions, contributing to more trustworthy diagnostic tools for ovarian cancer detection.
...
TL;DR: CreativeBench is a benchmark for evaluating machine creativity in code generation, using executable code to objectively measure creativity as quality × novelty, revealing scaling effects and proposing EvoRePE for inference-time creativity enhancement.


<details>
  <summary>Details</summary>
Motivation: The saturation of high-quality pre-training data has shifted focus to evolutionary systems for generating novel artifacts, but progress is hindered by the lack of rigorous, quantitative evaluation of machine creativity.

Method: Introduces CreativeBench with two subsets (CreativeBench-Combo and CreativeBench-Explore) targeting combinatorial and exploratory creativity through an automated pipeline using reverse engineering and self-play, leveraging executable code to objectively measure creativity as product of quality and novelty.

Result: Analysis of state-of-the-art models reveals: (1) scaling significantly improves combinatorial creativity but yields diminishing returns for exploration; (2) larger models exhibit "convergence-by-scaling," becoming more correct but less divergent; (3) reasoning capabilities primarily benefit constrained exploration rather than combination.

Conclusion: Proposes EvoRePE, a plug-and-play inference-time steering strategy that internalizes evolutionary search patterns to consistently enhance machine creativity, addressing the need for systematic evaluation and improvement of creative capabilities in code generation systems.
...
TL;DR: Proposes a systematic process for operationalizing social, legal, ethical, empathetic, and cultural (SLEEC) norms in AI agents, surveys existing methods/tools, and identifies research gaps for developing norm-aligned AI systems.


<details>
  <summary>Details</summary>
Motivation: AI agents are increasingly deployed in high-stakes domains (healthcare, law enforcement) where alignment with SLEEC norms is critical. While international frameworks establish high-level principles, there's a significant gap in translating abstract principles into concrete, verifiable requirements.

Method: Proposes a systematic SLEEC-norm operationalisation process for determining, validating, implementing, and verifying normative requirements. Surveys existing methods and tools supporting this process.

Result: Establishes a framework and defines a research/policy agenda for developing AI agents that are not only functionally useful but also demonstrably aligned with human norms and values.

Conclusion: Identifies key remaining challenges and research avenues for addressing the gap between abstract normative principles and concrete, verifiable requirements for AI systems in high-stakes domains.
...
TL;DR: AdaFuse is a framework that reduces inference latency in LLMs using dynamic adapters by implementing token-level pre-gating and fused CUDA kernels, achieving 2.4x faster decoding while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: The integration of Mixture-of-Experts (MoE) with parameter-efficient adapters like LoRA enhances LLM capabilities but causes severe inference latency issues (over 2.5x slowdown) due to fragmented CUDA kernel launches from conventional dynamic routing.

Method: AdaFuse uses token-level pre-gating that makes a single global routing decision before token processing, then employs a custom CUDA kernel that fuses switching operations to merge all selected LoRA adapter parameters into the backbone model in one efficient pass.

Result: Experimental results show AdaFuse achieves accuracy comparable to state-of-the-art dynamic adapters while reducing decoding latency by over 2.4x, effectively bridging the gap between model capability and inference efficiency.

Conclusion: AdaFuse demonstrates that through tight algorithm-hardware co-design and token-level pre-gating with fused kernel operations, it's possible to maintain the benefits of dynamic adapters while eliminating their severe inference overhead.
...
TL;DR: Fair-PaperRec is an MLP-based model that reduces demographic bias in paper acceptance decisions while maintaining academic quality, achieving 42% increased underrepresented group participation and 3% improved utility.


<details>
  <summary>Details</summary>
Motivation: Despite double-blind review, demographic biases still disadvantage underrepresented groups in academic paper acceptance decisions, creating a need for fairness-aware solutions that don't compromise quality.
...
Conclusion: Diversity promotion does not compromise academic rigor; Fair-PaperRec supports equity-focused peer review solutions by addressing demographic disparities while maintaining quality requirements.
...
TL;DR: ProtoSR leverages free-text radiology reports to improve structured reporting by extracting visual prototypes from unstructured data and using them to correct model predictions through prototype-conditioned residuals.


<details>
  <summary>Details</summary>
Motivation: Structured radiology reporting is challenging to automate due to the need for many fine-grained decisions about rare findings from limited structured supervision, while free-text reports are abundant and contain rich, image-linked information that could be leveraged.

Method: ProtoSR uses an instruction-tuned LLM to automatically extract structured information from 80k+ MIMIC-CXR free-text reports, building a multimodal knowledge base with visual prototypes for each answer option. The system retrieves relevant prototypes for image-question pairs and augments predictions through prototype-conditioned residuals.
...
Conclusion: The approach successfully leverages unstructured free-text knowledge to improve structured reporting, showing that free-text reports can provide valuable signal for fine-grained image understanding tasks in radiology.
...
TL;DR: SLIP is a sensor-language alignment framework that learns transferable representations for multivariate time-series data, supporting flexible sensor configurations and achieving state-of-the-art performance in zero-shot transfer, captioning, and question answering.


<details>
  <summary>Details</summary>
Motivation: Existing self-supervised learning approaches for time-series data are optimized for reconstruction/forecasting and fail to capture semantic structure needed for classification/reasoning. Current sensor-language alignment methods are limited to fixed sensor configurations, hindering cross-domain applicability.

Method: SLIP integrates contrastive alignment with sensor-conditioned captioning, repurposes a pretrained decoder-only language model via cross-attention, and introduces a flexible patch-embedder that supports different temporal resolutions and variable-length inputs without retraining.
...
Conclusion: SLIP provides an effective open-source framework for learning language-aligned sensor representations that generalize across diverse sensor setups, addressing limitations of existing approaches and enabling both discriminative understanding and generative reasoning.
...
TL;DR: NormCoRe: A framework for translating human subject experiments into multi-agent AI environments to systematically study normative coordination and decision-making in fairness-sensitive domains.


<details>
  <summary>Details</summary>
Motivation: Existing empirical approaches treat norms as alignment targets, assuming equivalence between humans and AI agents, leaving collective normative dynamics in multi-agent AI systems insufficiently examined. There's a need for systematic methods to study how norms emerge through coordination in AI agent interactions.

Method: Normative Common Ground Replication (NormCoRe) - a methodological framework that maps structural layers of human subject studies onto AI agent study designs. It builds on behavioral science, replication research, and state-of-the-art MAAI architectures to enable systematic documentation and analysis of norms in multi-agent AI systems.

Result: Demonstrated by replicating a seminal distributive justice experiment (veil of ignorance negotiation). Found that normative judgments in AI agent studies differ from human baselines and are sensitive to foundation model choice and language used to instantiate agent personas.

Conclusion: NormCoRe provides a principled pathway for analyzing norms in multi-agent AI systems, helping to guide, reflect, and document design choices when AI agents automate or support tasks formerly carried out by humans, addressing the gap in studying collective normative dynamics.
...
TL;DR: LABSHIELD benchmark evaluates MLLM safety awareness in laboratory automation, revealing significant performance gaps in hazard identification and safety-critical reasoning despite strong general-domain performance.


<details>
  <summary>Details</summary>
Motivation: As MLLM agents transition from lab assistants to autonomous lab operators, there's an urgent need to assess their safety awareness and decision-making reliability in high-stakes laboratory environments with fragile equipment and hazardous materials.

Method: Developed LABSHIELD benchmark with 164 operational tasks based on OSHA and GHS standards, featuring multi-view assessment, diverse manipulation complexities, and risk profiles. Evaluated 32 models (20 proprietary, 9 open-source, 3 embodied) using dual-track evaluation framework comparing MCQ accuracy with Semi-open QA safety performance.

Result: Models showed systematic 32.0% average performance drop in professional laboratory scenarios compared to general domains, particularly in hazard interpretation and safety-aware planning. Significant gaps exist between general-domain performance and safety-critical reasoning capabilities.

Conclusion: Current MLLMs lack sufficient safety-centric reasoning for reliable autonomous scientific experimentation, highlighting the urgent need for specialized safety frameworks in embodied laboratory contexts.
...
<details>
  <summary>Details</summary>
Motivation: Existing PFL approaches rely on heuristics like clustering or model interpolation that lack principled mechanisms for balancing heterogeneous client objectives. Maintaining M separate models for M clients poses significant scalability challenges in federated settings with hundreds or thousands of clients.

Method: Reformulates PFL as a few-for-many optimization problem maintaining only K shared server models (K ≪ M) to collectively serve all M clients. Proposes FedFew algorithm that jointly optimizes K server models through efficient gradient-based updates, automatically discovering optimal model diversity through optimization.

Result: Theoretical proof shows framework achieves near-optimal personalization: approximation error diminishes as K increases and each client's model converges to each client's optimum as data grows. Experiments across vision, NLP, and real-world medical imaging datasets demonstrate FedFew with just 3 models consistently outperforms state-of-the-art approaches.

Conclusion: FedFew provides a principled, scalable solution to personalized federated learning by reformulating it as a few-for-many optimization problem, achieving near-optimal personalization with theoretical guarantees while maintaining practical efficiency and outperforming existing methods.
...
TL;DR: RFT shows strong within-environment generalization across task difficulty but weak cross-environment transfer; sequential training yields downstream gains with minimal forgetting, and mixture training improves overall balance.


<details>
  <summary>Details</summary>
Motivation: Most RFT evaluations are in-domain (same environment/tasks), but real-world deployment requires agents to operate in unseen environments with different background knowledge, observation spaces, and action interfaces. Need to characterize generalization profile under such shifts.
...
Result: RFT generalizes well across task difficulty within an environment, but exhibits weaker transfer to unseen environments (correlates with shifts in semantic priors and observation/action interfaces). Sequential training yields promising downstream gains with minimal upstream forgetting, and mixture training across environments improves overall balance.

Conclusion: Provides detailed analyses and insights to help develop and deploy generalizable LLM agents, highlighting the need to address cross-environment transfer challenges for real-world deployment.
...
TL;DR: XSkill is a dual-stream framework for continual learning in multimodal agents that extracts and utilizes both experiences (action-level guidance) and skills (task-level guidance) from past trajectories without parameter updates, achieving superior performance across diverse benchmarks.


<details>
  <summary>Details</summary>
Motivation: Multimodal agents suffer from inefficient tool use and inflexible orchestration in open-ended settings, lacking the ability to continually improve without parameter updates by learning from past trajectories. The paper identifies the need for two complementary forms of reusable knowledge: experiences for action-level guidance and skills for task-level guidance.

Method: XSkill is a dual-stream framework with two phases: 1) Accumulation phase: distills and consolidates experiences and skills from multi-path rollouts via visually grounded summarization and cross-rollout critique; 2) Inference phase: retrieves and adapts knowledge to current visual context, with usage history fed back into accumulation to form a continual learning loop. Both knowledge extraction and retrieval are grounded in visual observations.

Result: Evaluated on five benchmarks across diverse domains with four backbone models, XSkill consistently and substantially outperforms both tool-only and learning-based baselines. Further analysis reveals that the two knowledge streams play complementary roles in influencing reasoning behaviors and show superior zero-shot generalization.

Conclusion: XSkill successfully addresses the challenge of continual learning in multimodal agents by extracting and utilizing both experiences and skills from past trajectories, demonstrating that dual-stream knowledge representation grounded in visual observations enables efficient tool use and flexible orchestration without parameter updates.
...
<details>
  <summary>Details</summary>
Motivation: Existing RL approaches for traffic signal control suffer from poor generalization to dynamic traffic flow variations, overfitting to static patterns, and using action spaces incompatible with driver expectations, limiting real-world deployment.

Method: Proposes a MARL framework with three key mechanisms: (1) Turning Ratio Randomization during training to enhance robustness; (2) Exponential Phase Duration Adjustment action space for stability; (3) Neighbor-Based Observation scheme using MAPPO with CTDE architecture.

Result: The framework outperforms standard RL baselines, reducing average waiting time by over 10%, demonstrates superior generalization in unseen traffic scenarios, and maintains high control stability in Vissim simulations.

Conclusion: The proposed robust MARL framework offers a practical solution for adaptive traffic signal control by addressing generalization limitations and stability concerns, making it suitable for real-world deployment.
...
TL;DR: AI agent populations with diverse LLMs and reinforcement learning can cause dangerous system overload under resource scarcity, with tribal formation mitigating but not eliminating risks; system behavior depends critically on capacity-to-population ratio.


<details>
  <summary>Details</summary>
Motivation: As diverse AI agents from different developers increasingly compete for finite shared resources in everyday devices, there is poor understanding of their collective dynamics and risks to users and society, necessitating systematic study of how key variables affect coordination outcomes.

Method: Study AI-agent populations as a system where four key variables can be independently toggled: nature (innate LLM diversity), nurture (individual reinforcement learning), culture (emergent tribe formation), and resource scarcity; use both empirical experiments and mathematical analysis.

Result: Under resource scarcity, AI model diversity and reinforcement learning increase dangerous system overload, though tribe formation lessens this risk while some individuals profit; under abundance, these same factors drive overload near zero, though tribe formation slightly worsens it; behavior depends on whether opposing tribes fit within available capacity.

Conclusion: More sophisticated AI-agent populations are not inherently better - whether sophistication helps or harms depends entirely on the capacity-to-population ratio, which is knowable before deployment; this provides a critical design parameter for managing collective AI risks.
...
TL;DR: TopoBench benchmark reveals LLMs struggle with topological grid puzzles due to difficulty extracting spatial constraints rather than reasoning limitations, with frontier models solving <25% of hard instances.


<details>
  <summary>Details</summary>
Motivation: To study LLMs' abilities in reasoning over global spatial invariants (connectivity, loop closure, region symmetry) under controlled settings, as these topological grid puzzles remain challenging even for powerful models.

Method: Introduced TopoBench benchmark with 6 puzzle families across 3 difficulty levels; evaluated strong reasoning LLMs; annotated 750 chain-of-thought traces with error taxonomy; tested targeted interventions simulating error types; studied mitigation strategies including prompt guidance, cell-aligned grid representations, and tool-based constraint checking.

Result: Frontier models solve fewer than 25% of hard instances, with two families nearly unsolved. Error analysis shows premature commitment and constraint forgetting directly impact solving ability, while repeated reasoning is benign. Mitigation strategies reveal bottleneck lies in extracting constraints from spatial representations, not in reasoning over them.

Conclusion: LLMs' failure in topological grid puzzles stems primarily from difficulty extracting and maintaining spatial constraints from representations rather than limitations in reasoning capabilities, highlighting the need for improved spatial constraint extraction methods.
...
TL;DR: Practical polynomial compilation from PDDL 2.1 temporal planning with durative actions into PDDL+, preserving semantics and plan length up to constant factor.


<details>
  <summary>Details</summary>
Motivation: Despite theoretical knowledge that PDDL 2.1 temporal planning with durative actions could be compiled into PDDL+, no practical compilation had been presented in the literature since PDDL+ was introduced.

Method: Developed a practical polynomial compilation from temporal planning with durative actions (PDDL 2.1) into PDDL+, fully capturing the original semantics while only assuming non-self-overlapping of actions.
...
Conclusion: Provides the first practical compilation from PDDL 2.1 temporal planning to PDDL+, enabling practical application of PDDL+ solvers to temporal planning problems with durative actions.
...
TL;DR: Parallelized CEGAR-SEQ algorithm for 3D printing object arrangement using portfolio of strategies outperforms original sequential version by utilizing multi-core CPUs.


<details>
  <summary>Details</summary>
Motivation: Modern multi-core CPUs in personal computers and mobile devices provide parallel computing power previously only available in supercomputers, which can be leveraged to solve complex combinatorial problems like 3D printing object arrangement and scheduling more efficiently.

Method: Parallelized the existing CEGAR-SEQ algorithm by executing it in parallel with a portfolio of object arrangement strategies (Portfolio-CEGAR-SEQ). Alternative strategies include placing objects towards a corner of the printing plate and scheduling by height, complementing the original center-focused strategy.

Result: Portfolio-CEGAR-SEQ outperforms the original CEGAR-SEQ algorithm. When scheduling batches of objects for multiple printing plates, Portfolio-CEGAR-SEQ often uses fewer printing plates than the original algorithm.

Conclusion: Effective utilization of modern multi-core CPU parallelism through high-level parallelization with portfolio strategies significantly improves performance for sequential 3D printing object arrangement and scheduling problems.
...
TL;DR: Reasoning LLM-judges outperform non-reasoning judges in RL-based LLM alignment by enabling policies to achieve strong performance with gold-standard evaluation, though they learn to generate adversarial outputs that can deceive other LLM-judges on benchmarks.


<details>
  <summary>Details</summary>
Motivation: While reasoning LLM-judges show promise for non-verifiable domains where output correctness cannot be directly checked, their actual effectiveness in policy training hasn't been systematically examined, despite their better performance on static evaluation benchmarks.
...
Result: Non-reasoning judges lead to reward hacking easily, while reasoning judges enable policies to achieve strong performance when evaluated by the gold-standard judge. However, reasoning-judge-trained policies achieve this by learning to generate highly effective adversarial outputs that can also score well on popular benchmarks like Arena-Hard by deceiving other LLM-judges.

Conclusion: The study highlights both important findings and room for improvements in applying reasoning LLM-judges to non-verifiable LLM post-training, revealing their potential benefits but also exposing vulnerabilities in current evaluation benchmarks.
...
### [64] [TinyNav: End-to-End TinyML for Real-Time Autonomous Navigation on Microcontrollers](https://arxiv.org/abs/2603.11071)
*Pooria Roy,Nourhan Jadallah. Tomer Lapid,Shahzaib Ahmad,Armita Afroushe,Mete Bayrak*

Main category: cs.RO

TL;DR: TinyNav is an end-to-end TinyML system for real-time autonomous navigation on ESP32 microcontrollers using a lightweight 23k-parameter 2D CNN that processes depth data to predict steering/throttle commands with 30ms latency.


<details>
  <summary>Details</summary>
Motivation: Autonomous navigation typically requires power-intensive processors, limiting accessibility for low-cost robotics. Microcontrollers offer resource efficiency but impose strict model complexity constraints, creating a need for lightweight navigation solutions that can run directly on edge devices.

Method: Custom-trained, quantized 2D convolutional neural network processes a 20-frame sliding window of depth data to predict steering and throttle commands. The architecture avoids 3D convolutions and recurrent layers to maintain low complexity, resulting in a 23k-parameter model optimized for microcontroller deployment.

Result: The system achieves 30 ms inference latency on ESP32 microcontroller. Correlation analysis and Grad-CAM validation demonstrate consistent spatial awareness and obstacle avoidance behavior. The model successfully enables responsive autonomous control directly on constrained edge devices.

Conclusion: TinyNav demonstrates that responsive autonomous control can be deployed directly on highly constrained edge devices, reducing reliance on external compute resources and enabling more accessible, low-cost robotics applications.
...
### [65] [OA-NBV: Occlusion-Aware Next-Best-View Planning for Human-Centered Active Perception on Mobile Robots](https://arxiv.org/abs/2603.11072)
*Boxun Hu,Chang Chang,Jiawei Ge,Man Namgung,Xiaomin Lin,Axel Krieger,Tinoosh Mohsenin*

Main category: cs.RO

TL;DR: OA-NBV is an occlusion-aware next-best-view planning method for mobile robots that selects optimal viewpoints to obtain complete views of occluded humans in cluttered environments.


<details>
  <summary>Details</summary>
Motivation: Current NBV methods focus on generic exploration or long-horizon coverage rather than specifically addressing the immediate need to obtain usable observations of partially occluded humans under real motion constraints, which is critical for human-centered operations like search, triage, and disaster response.
...
Result: OA-NBV achieves over 90% success rate in both simulation and real-world trials, while baseline NBV methods degrade sharply under occlusion. It increases normalized target area by at least 81% and keypoint visibility by at least 58% compared to the strongest baseline.

Conclusion: OA-NBV provides an effective drop-in view-selection module for diverse human-centered downstream tasks by significantly improving observation quality and success rates in occluded environments.
...
### [66] [DRAFTO: Decoupled Reduced-space and Adaptive Feasibility-repair Trajectory Optimization for Robotic Manipulators](https://arxiv.org/abs/2603.11074)
*Yichang Feng,Xiao Liang,Minghui Zheng*

Main category: cs.RO

TL;DR: DRAFTO is a new trajectory optimization algorithm that decouples optimization into reduced-space Gauss-Newton descent with feasibility repair, achieving high efficiency and reliability across diverse robotic tasks.


<details>
  <summary>Details</summary>
Motivation: The paper aims to develop an efficient trajectory optimization algorithm that handles multiple constraints (smoothness, safety, joint limits, task requirements) while reducing computational overhead from repeated constrained optimizations common in existing methods.

Method: DRAFTO constructs a constrained objective function, parameterizes trajectories using basis functions, and decouples optimization into: 1) reduced-space Gauss-Newton descent for main iterations, 2) constrained quadratic programming for initialization and terminal feasibility repair. It uses a two-phase acceptance rule with non-monotone policy and hinge-squared penalty for inequality constraints to ensure global convergence.

Result: Benchmark tests show DRAFTO outperforms optimization-based planners (CHOMP, TrajOpt, GPMP2, FACTO) and sampling-based planners (RRT-Connect, RRT*, PRM) in efficiency and reliability across diverse scenarios. The drawer-grabbing experiment demonstrates practical applicability to complex manipulation tasks.

Conclusion: DRAFTO provides an effective trajectory optimization framework that balances computational efficiency with constraint satisfaction, making it suitable for real-world robotic applications including complex manipulation tasks.
...
### [67] [TATIC: Task-Aware Temporal Learning for Human Intent Inference from Physical Corrections in Human-Robot Collaboration](https://arxiv.org/abs/2603.11077)
*Jiurun Song,Xiao Liang,Minghui Zheng*

Main category: cs.RO

TL;DR: TATIC is a framework that interprets brief physical corrections in human-robot collaboration to jointly infer task-level intent and motion parameters using torque-based force estimation and task-aware TCNs.


<details>
  <summary>Details</summary>
Motivation: Existing approaches lack mechanisms to interpret physical feedback for task-level semantic intent inference, while traditional pHRI methods struggle to infer task-level semantics from physical corrections.

Method: Uses torque-based contact force estimation and task-aware Temporal Convolutional Network (TCN) with task-aligned feature canonicalization for robust generalization and intent-driven adaptation scheme.

Result: Achieves 0.904 Macro-F1 score in intent recognition and demonstrates successful hardware validation in collaborative disassembly tasks.

Conclusion: TATIC bridges the gap between physical corrections and task-level intent inference, enabling more natural and effective human-robot collaboration through brief physical interactions.
...
### [68] [SELF-VLA: A Skill Enhanced Agentic Vision-Language-Action Framework for Contact-Rich Disassembly](https://arxiv.org/abs/2603.11080)
*Chang Liu,Sibo Tian,Xiao Liang,Minghui Zheng*

Main category: cs.RO

TL;DR: SELF-VLA is an agentic vision-language-action framework that integrates explicit disassembly skills to address limitations of current robotic disassembly systems and foundation models in handling complex, contact-rich, long-horizon disassembly tasks.


<details>
  <summary>Details</summary>
Motivation: Current robotic disassembly systems require extensive data preparation, model training, and system management for each subtask, are task- and component-specific, poorly handle variability of end-of-life products, and rely heavily on human labor. While vision-language-action models show promise for standard manipulation tasks, they remain limited for complex, contact-rich, sequential disassembly operations.

Method: SELF-VLA, an agentic vision-language-action framework that integrates explicit disassembly skills to handle the sequential and precise manipulation requirements of disassembly tasks.

Result: Experimental studies show SELF-VLA significantly outperforms current state-of-the-art end-to-end VLA models on two contact-rich disassembly tasks.

Conclusion: The proposed SELF-VLA framework addresses key limitations in robotic disassembly by combining agentic VLA architecture with explicit disassembly skills, demonstrating superior performance on complex disassembly tasks compared to existing end-to-end VLA models.
...
### [69] [Edge-Assisted Multi-Robot Visual-Inertial SLAM with Efficient Communication](https://arxiv.org/abs/2603.11085)
*Xin Liu,Shuhuan Wen,Jing Zhao,Tony Z. Qiu,Hong Zhang*

Main category: cs.RO

TL;DR: A lightweight multi-robot SLAM system using pyramid IMU-predicted optical flow tracking in a robot-edge-cloud architecture achieves real-time collaborative SLAM with reduced bandwidth usage while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of cloud-edge integration for multi-robot SLAM, specifically the bandwidth constraints and long communication links that degrade system performance, while overcoming limited on-board computing resources and low execution efficiency of single robots.

Method: Proposes a lightweight SLAM method using optical flow tracking based on pyramid IMU prediction, combined with a centralized multi-robot SLAM system in a robot-edge-cloud layered architecture. Features include transmission of only feature points and keyframe descriptors with lossless encoding/compression for bandwidth-efficient communication.

Result: Experimental verification on EuRoC dataset shows the method achieves lower data volume feature transmission compared to state-of-the-art local feature compression methods, and achieves same or better positioning accuracy under low computational load compared to advanced centralized multi-robot SLAM schemes.

Conclusion: The proposed system effectively reduces bandwidth usage while maintaining SLAM accuracy, enabling real-time collaborative multi-robot SLAM through efficient cloud-edge integration with lightweight computation and optimized data transmission.
...
### [70] [Thousand-GPU Large-Scale Training and Optimization Recipe for AI-Native Cloud Embodied Intelligence Infrastructure](https://arxiv.org/abs/2603.11101)
...
<details>
  <summary>Details</summary>
Motivation: Embodied intelligence development faces challenges in data, frameworks, infrastructure, and evaluation systems. The authors aim to address these bottlenecks to advance Artificial General Intelligence (AGI) and accelerate the development of autonomous intelligent robots.

Method: Built a cloud-based distributed training platform on thousand-GPU clusters using LeRobot framework. Implemented data pipeline restructuring, variable-length FlashAttention with Data Packing, π-0.5 attention optimization, FP8 quantization, high-performance storage with 3.2T RDMA network, Ray-driven elastic AI data lake, and end-to-end evaluation system.

Result: Achieved 40-fold speedup reducing GR00T-N1.5 model training from 15 hours to 22 minutes. Model optimizations delivered 188% speed increase from FlashAttention/Data Packing, 165% acceleration from π-0.5 attention, and 140% speedup from FP8 quantization. Successfully validated on thousand-GPU clusters.

Conclusion: The framework provides crucial technical foundation for next-generation autonomous intelligent robots, addresses key bottlenecks in embodied intelligence development, and is expected to accelerate human-machine integration era.
...
### [71] [ResWM: Residual-Action World Model for Visual RL](https://arxiv.org/abs/2603.11110)
*Jseen Zhang,Gabriel Adineera,Jinzhou Tan,Jinoh Kim*

Main category: cs.RO

TL;DR: ResWM reformulates world modeling from absolute to residual actions, improving stability and efficiency in model-based RL for continuous control.


<details>
  <summary>Details</summary>
Motivation: Conventional model-based RL frameworks directly condition future predictions on absolute actions, causing unstable optimization due to task-dependent action distributions that are unknown a priori and lead to oscillatory or inefficient control.

Method: Introduces Residual-Action World Model (ResWM) that reformulates control from absolute to residual actions (incremental adjustments), plus an Observation Difference Encoder to model changes between frames. Integrated into Dreamer-style latent dynamics with minimal modifications.

Result: ResWM achieves consistent improvements in sample efficiency, asymptotic returns, and control smoothness on DeepMind Control Suite, surpassing Dreamer and TD-MPC baselines. Produces more stable and energy-efficient action trajectories.

Conclusion: Residual action modeling provides a simple yet powerful principle for bridging algorithmic advances in RL with practical robotics requirements, enabling smoother exploration, lower control variance, and more reliable planning.
...
### [72] [Robust Co-design Optimisation for Agile Fixed-Wing UAVs](https://arxiv.org/abs/2603.11130)
*Adrian Andrei Buda,Xavier Chen,Nicolò Botteghi,Urban Fasel*

Main category: cs.RO

TL;DR: Robust co-design framework for agile fixed-wing UAVs that integrates parametric uncertainty and wind disturbances into concurrent optimization of physical design and control, outperforming deterministic baselines across three flight missions.


<details>
  <summary>Details</summary>
Motivation: Existing co-design frameworks for autonomous systems often neglect robustness requirements needed for real-world unstructured environments, particularly for agile UAVs operating at flight envelope limits where sensitivity to perturbations and model mismatch is critical.

Method: Bi-level approach with high-level loop optimizing physical design while discovering nominal solutions via constrained trajectory planner and evaluating performance across stochastic Monte Carlo ensemble using feedback LQR control, directly integrating parametric uncertainty and wind disturbances.

Result: Framework consistently outperforms deterministic baselines across three agile flight missions, demonstrating inherent tailoring of aerodynamic features (wing placement, aspect ratio) to achieve optimal trade-off between mission performance and disturbance rejection.

Conclusion: Robust co-design strategy effectively addresses the limitations of deterministic approaches by integrating uncertainty and disturbances directly into the optimization process, yielding UAV designs that are inherently robust to real-world environmental challenges.
...
### [73] [A Causal Approach to Predicting and Improving Human Perceptions of Social Navigation Robots](https://arxiv.org/abs/2603.11290)
*Maximilian Diehl,Nathan Tsoi,Gustavo Chavez,Karinne Ramirez-Amaro,Marynel Vázquez*

Main category: cs.RO

TL;DR: A Causal Bayesian Network approach for predicting human perceptions of robot competence and intent during navigation, with a combinatorial search method to improve perceived competence through interpretable, counterfactual robot motions.


<details>
  <summary>Details</summary>
Motivation: Mobile robots need to predict human perceptions for socially adaptable navigation, requiring models that can learn from limited data while being interpretable for safe interactions, especially when robots are perceived as incompetent.

Method: Proposes a Causal Bayesian Network to predict human perceptions of robot competence and intent, plus a combinatorial search method guided by the causal model to identify better navigation behaviors that improve perceived competence.

Result: Achieved F1-scores of 0.78 for competence and 0.75 for intention prediction on binary scales. Online evaluation showed an 83% statistically significant increase in perceived competence for low-competent robot behaviors on a 5-point Likert scale.

Conclusion: The causal approach enhances interpretability, generates counterfactual motions, and achieves comparable or superior predictive performance to state-of-the-art methods while significantly improving perceived robot competence through targeted behavior modifications.
...
### [74] [Distributed Kalman--Consensus Filtering with Adaptive Uncertainty Weighting for Multi-Object Tracking in Mobile Robot Networks](https://arxiv.org/abs/2603.11328)
*Niusha Khosravi,Rodrigo Ventura,Meysam Basiri*

Main category: cs.RO

TL;DR: Implementation of Distributed Kalman-Consensus Filter for multi-object tracking in mobile robot networks with adaptive weighting to handle heterogeneous localization uncertainty and partial observability.


<details>
  <summary>Details</summary>
Motivation: Address challenges in distributed multi-object tracking where agents have differing localization quality, leading to frame misalignment, inconsistent estimates, track duplication, and ghost tracks during information fusion.

Method: Builds on MOTLEE framework with frame-alignment using dynamic objects as transient landmarks. Proposes uncertainty-aware adaptive consensus weighting that adjusts neighbor influence based on covariance. Local tracking uses Kalman Filter with Constant Velocity Model and Global Nearest Neighbor data association.

Result: Adaptive weighting effectively protects local estimates from inconsistent data, yielding MOTA improvement of 0.09 for agents with localization drift, though system performance remains constrained by communication latency.

Conclusion: The proposed Distributed Kalman-Consensus Filter with adaptive weighting successfully addresses heterogeneous localization uncertainty in distributed multi-object tracking, improving robustness against unreliable data while highlighting communication latency as a remaining constraint.
...
### [75] [ADMM-based Continuous Trajectory Optimization in Graphs of Convex Sets](https://arxiv.org/abs/2603.11335)
*Lukas Pries,Jon Arrizabalaga,Zachary Manchester,Markus Ryll*

Main category: cs.RO

TL;DR: A numerical solver for continuous trajectories in non-convex environments using customized ADMM with polynomial parameterization and spatio-temporal allocation graphs.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for trajectory planning in non-convex environments often use decoupled spatial and temporal optimization, limiting search space and requiring complex warm-starting procedures that hinder reliability.

Method: Customized ADMM implementation with two key components: 1) Polynomial parameterization of trajectories enabling closed-form primal updates as minimum-control-effort problems, 2) Spatio-temporal allocation graph based on mixed-integer formulation with slack updates solved as shortest-path searches.
...
Conclusion: The proposed solver overcomes limitations of existing methods by enabling joint spatio-temporal optimization and eliminating the need for complex warm-starting procedures, making it more effective for trajectory planning in non-convex environments.
...
### [76] [Novelty Adaptation Through Hybrid Large Language Model (LLM)-Symbolic Planning and LLM-guided Reinforcement Learning](https://arxiv.org/abs/2603.11351)
...
<details>
  <summary>Details</summary>
Motivation: Traditional symbolic planners fail in dynamic open-world environments when encountering novel objects due to missing operators in the planning domain, preventing autonomous agents from achieving their goals.

Method: Neuro-symbolic architecture integrating symbolic planning, reinforcement learning, and LLMs. LLM identifies missing operators, symbolic planner generates plans, and LLM writes reward functions for RL agent to learn control policies for newly identified operators.
...
Conclusion: The proposed neuro-symbolic approach effectively addresses novelty handling in dynamic environments by leveraging LLM's common sense reasoning to bridge gaps in symbolic planning domains and enable learning of new interaction capabilities.
...
### [77] [MirrorDrift: Actuated Mirror-Based Attacks on LiDAR SLAM](https://arxiv.org/abs/2603.11364)
*Rokuto Nagata,Kenji Koide,Kazuma Ikeda,Ozora Sako,Shion Horie,Kentaro Yoshioka*

Main category: cs.RO

TL;DR: MirrorDrift: A novel physical attack on LiDAR SLAM using specular reflection from actuated planar mirrors to create ghost points and systematically bias scan matching, achieving significant localization errors even on secure LiDARs with interference mitigation.


<details>
  <summary>Details</summary>
Motivation: Traditional LiDAR spoofing attacks require sensor-specific timing knowledge and are increasingly mitigated by modern defenses. There's a need for injection-free alternatives that can bypass these security measures while still effectively compromising LiDAR SLAM systems.

Method: Uses actuated planar mirrors to exploit specular reflection, creating ghost points in LiDAR scans. The attack optimizes mirror placement, alignment, and actuation to systematically bias scan-matching correspondences without requiring signal injection.

Result: In simulation: 6.1x increase in average pose error (APE) over random placement, degrading three SLAM systems to 2.29-3.31 m mean APE. Real-world: Induces localization errors up to 6.03 m on modern LiDAR with state-of-the-art interference mitigation.

Conclusion: MirrorDrift demonstrates the first successful SLAM-targeted attack against production-grade secure LiDARs, showing that specular reflection provides an effective injection-free alternative to traditional spoofing attacks, highlighting new vulnerabilities in LiDAR SLAM systems.
...
### [78] [D-SLAMSpoof: An Environment-Agnostic LiDAR Spoofing Attack using Dynamic Point Cloud Injection](https://arxiv.org/abs/2603.11365)
*Rokuto Nagata,Kenji Koide,Kazuma Ikeda,Ozora Sako,Kentaro Yoshioka*

Main category: cs.RO

TL;DR: D-SLAMSpoof is a novel LiDAR spoofing attack that compromises SLAM in feature-rich environments using dynamic injection patterns, while ISD-SLAM is a defense method using inertial dead reckoning to detect and mitigate such attacks.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of conventional LiDAR spoofing methods that often fail in feature-rich environments like urban areas and indoor spaces, and to expose inherent vulnerabilities in LiDAR-based SLAM systems used in autonomous vehicles.

Method: D-SLAMSpoof leverages LiDAR spoofing with both spatial injection shapes and temporally coordinated dynamic injection patterns guided by scan-matching principles. ISD-SLAM defense uses inertial dead reckoning signals commonly available in autonomous systems to detect spoofing attacks.

Result: D-SLAMSpoof significantly improves attack success rates in real-world, feature-rich environments where conventional methods fail. ISD-SLAM accurately detects LiDAR spoofing attacks including D-SLAMSpoof and effectively mitigates resulting position drift.

Conclusion: The work exposes inherent vulnerabilities in LiDAR-based SLAM and introduces the first practical defense against LiDAR-based SLAM spoofing using only standard onboard sensors, providing critical insights for improving security and reliability of autonomous systems.
...
### [79] [Vision-Based Hand Shadowing for Robotic Manipulation via Inverse Kinematics](https://arxiv.org/abs/2603.11383)
*Hendrik Chiche,Antoine Jamme,Trevor Rigoberto Martinez*

Main category: cs.RO

TL;DR: Offline hand-shadowing pipeline using single RGB-D camera for teleoperating 6-DOF robot via inverse kinematics, achieving 90% success in structured tasks but only 9.3% in unstructured environments due to occlusion.


<details>
  <summary>Details</summary>
Motivation: Teleoperation of low-cost robotic manipulators is challenging due to complex mapping of human hand articulations to robot joint commands, requiring accessible marker-free solutions.

Method: Offline pipeline using egocentric RGB-D camera on 3D-printed glasses: MediaPipe Hands detects 21 landmarks per hand, depth deprojection to 3D, coordinate transformation to robot frame, damped-least-squares inverse kinematics in PyBullet for 6-DOF SO-ARM101, gripper controller with four-level fallback hierarchy, physics simulation preview, and LeRobot framework for physical replay.

Result: 90% success rate on structured pick-and-place benchmark (5-tile grid, 10 grasps per tile); outperformed four vision-language-action policies (ACT, SmolVLA, pi0.5, GR00T N1.5); only 9.3% success in unstructured real-world environments (grocery store, pharmacy, N=75) due to hand occlusion by objects.

Conclusion: Marker-free analytical retargeting shows promise for structured tasks but faces significant limitations in unstructured environments due to occlusion, highlighting need for improved robustness in real-world applications.
...
### [80] [Deployment-Time Reliability of Learned Robot Policies](https://arxiv.org/abs/2603.11400)
*Christopher Agia*
...
Conclusion: These deployment-time mechanisms collectively advance the foundations for reliable robot policy deployment, and continued progress on such foundations will be essential for enabling trustworthy and scalable robot autonomy in the future.
...
### [81] [Real-time Rendering-based Surgical Instrument Tracking via Evolutionary Optimization](https://arxiv.org/abs/2603.11404)
*Hanyang Hu,Zekai Liang,Florian Richter,Michael C. Yip*

Main category: cs.RO

TL;DR: A CMA-ES-based evolutionary optimization pipeline for efficient and accurate surgical instrument tracking in robot-assisted surgery, using batch rendering to estimate pose and joint configurations with improved speed and robustness.


<details>
  <summary>Details</summary>
Motivation: Reliable surgical instrument tracking remains challenging due to partial visibility and specialized articulation design. Existing methods suffer from unreliable feature detection under degraded visual quality/data scarcity, while rendering-based approaches face computational costs and suboptimal convergence.

Method: Incorporates CMA-ES evolutionary optimization into a versatile tracking pipeline that jointly estimates surgical instrument pose and joint configurations. Uses batch rendering to efficiently evaluate multiple pose candidates in parallel, reducing inference time and improving convergence robustness.

Result: The method significantly outperforms prior approaches in both accuracy and runtime on synthetic and real-world datasets. It generalizes to joint angle-free and bi-manual tracking settings.

Conclusion: The proposed CMA-ES-based tracking framework provides accurate and efficient surgical instrument tracking suitable for vision feedback control and online surgery video calibration, addressing key challenges in robot-assisted minimally invasive surgery.
...
### [82] [Grounding Robot Generalization in Training Data via Retrieval-Augmented VLMs](https://arxiv.org/abs/2603.11426)
*Jensen Gao,Dorsa Sadigh,Sandy Huang,Dhruv Shah*

Main category: cs.RO

TL;DR: RADAR is a framework for precisely evaluating policy generalization in robotics by comparing test tasks to training data using retrieval and vision-language model analysis.


<details>
  <summary>Details</summary>
Motivation: Current robot manipulation research lacks precise characterization of how evaluation settings represent generalization from training distributions, making it difficult to assess what forms of generalization policies actually achieve.

Method: Two-stage pipeline: 1) Retrieval using generalist policy embeddings to identify relevant training examples for evaluation tasks; 2) Vision-language models analyze evaluation tasks against retrieved data along multiple axes and classify required generalization types.

Result: VLMs effectively analyze data for generalization; retrieval step accurately identifies needed examples; RADAR scales to large datasets and agrees with human-defined benchmark conditions from prior work.

Conclusion: RADAR provides a scalable framework for precise evaluation of policy generalization in robotics, enabling interpretable analysis of how test tasks relate to training data and what generalization types are required.
...
### [83] [A Generalized Theory of Load Distribution in Redundantly-actuated Robotic Systems](https://arxiv.org/abs/2603.11431)
*Joshua Flight,Clément Gosselin*

Main category: cs.RO

TL;DR: A generalized theory for load distribution in redundantly-actuated robotic systems that characterizes feasible wrench distributions and provides efficient computational solutions for wrench synthesis and analysis.


<details>
  <summary>Details</summary>
Motivation: To address the need for a comprehensive theory that describes how applied loads are distributed within rigid bodies handled by redundantly-actuated robotic systems, which has important implications for force-control in multifingered grippers, legged robots, cooperating robots, and other overconstrained mechanisms.

Method: Develops a generalized mathematical theory that fully characterizes the feasible set of manipulating wrench distributions for a given resultant wrench applied to a rigid body. Derives explicit solutions to both wrench synthesis (determining required actuator forces for desired resultant wrench) and wrench analysis (computing resultant wrench from given actuator forces) problems.

Result: The theory provides computationally efficient solutions that scale linearly with the number of applied wrenches, requiring neither numerical methods nor inversion of large matrices. The paper identifies significant shortcomings in current state-of-the-art approaches and proposes corrections, supported by illustrative examples demonstrating the advantages of the improved methods.

Conclusion: The presented generalized theory offers a complete characterization of load distribution in redundantly-actuated systems with efficient computational solutions, addressing limitations in existing approaches and providing practical improvements for force-control applications in various robotic systems.
...
### [84] [Enhancing Lightweight Vision Language Models through Group Competitive Learning for Socially Compliant Navigation](https://arxiv.org/abs/2603.11447)
*Xinyu Zhang,Atsushi Konno,Toshihiko Yamasaki,Ling Xiao*

Main category: cs.RO

TL;DR: Group Competitive Learning (GCL) enhances lightweight Vision Language Models for social robot navigation by improving reasoning capabilities while maintaining computational efficiency, enabling smaller models to outperform larger baselines.


<details>
  <summary>Details</summary>
Motivation: There's a trade-off between model size and computational efficiency in Vision Language Models for social robot navigation. Large VLMs have strong reasoning but high computational overhead, while lightweight VLMs are efficient but have weaker reasoning in complex social environments. The challenge is achieving both strong reasoning ability and efficiency for real-time robotic deployment.

Method: Proposes Group Competitive Learning (GCL) with two components: 1) Group Competitive Objective (GCO) harmonizes global semantics with distributional regularization, and 2) Asymmetric Group Optimization (AGO) explores the upper limits of model performance. The approach amplifies capabilities of lightweight VLMs without increasing model size.

Result: GCL significantly elevates VLM performance on social navigation benchmarks. Qwen2.5-VL-3B achieves F1 score of 0.968 (40% improvement over vanilla SFT), and Qwen3-VL-4B achieves 0.914 (12% improvement). Notably, the 3B model with GCL outperforms the 8B baseline model by 28%, demonstrating that smaller models can surpass larger ones with proper training strategy.

Conclusion: Group Competitive Learning provides an effective solution for achieving both high accuracy and computational efficiency in real-world deployment of social robot navigation systems, enabling lightweight VLMs to match or exceed the performance of larger models while maintaining inference efficiency.
...
### [85] [CoViLLM: An Adaptive Human-Robot Collaborative Assembly Framework Using Large Language Models for Manufacturing](https://arxiv.org/abs/2603.11461)
*Jiabao Zhao,Jonghan Lim,Hongliang Li,Ilya Kovalenko*

Main category: cs.RO

TL;DR: CoViLLM is an adaptive human-robot collaborative assembly framework that uses LLMs for task planning to enable assembly of customized and previously unseen products, overcoming limitations of traditional rule-based HRC systems.


<details>
  <summary>Details</summary>
Motivation: Traditional manufacturing robots lack flexibility for mass customization, and existing HRC frameworks are limited by predefined perception-manipulation pipelines that cannot autonomously generate task plans for new product variants.
...
Result: The framework was validated on the NIST Assembly Task Board for known, customized, and new product cases, demonstrating flexible collaborative assembly that extends HRC beyond predefined product and task settings.

Conclusion: CoViLLM enables adaptive human-robot collaboration for customized product assembly by integrating LLM-based task planning with perception systems, overcoming limitations of traditional rule-based approaches.
...
### [86] [NFPO: Stabilized Policy Optimization of Normalizing Flow for Robotic Policy Learning](https://arxiv.org/abs/2603.11470)
...
<details>
  <summary>Details</summary>
Motivation: Current DRL for robotics uses multivariate Gaussian policies with diagonal covariance, which cannot model multi-modal distributions needed for complex robotic tasks.

Method: Proposes NFPO using Normalizing Flows as policy parameterization for multi-modal modeling, with closed-form log probability and low overhead. Addresses training instability in online RL with simple but effective techniques.

Result: Extensive experiments in multiple simulation environments show NFPO obtains robust and strong performance in robotic learning tasks and successfully transfers to real-world robots.

Conclusion: Normalizing Flows provide effective multi-modal policy parameterization for DRL in robotics when training instability is properly addressed, enabling successful real-world deployment.
...
### [87] [SPARK: Skeleton-Parameter Aligned Retargeting on Humanoid Robots with Kinodynamic Trajectory Optimization](https://arxiv.org/abs/2603.11480)
*Hanwen Wang,Qiayuan Liao,Bike Zhang,Kunzhao Ren,Koushil Sreenath,Xiaobin Xiong*

Main category: cs.RO

TL;DR: Two-stage pipeline for generating natural, dynamically feasible motion references from human demonstrations for humanoid robots, using skeleton calibration and progressive kinodynamic trajectory optimization.


<details>
  <summary>Details</summary>
Motivation: Human motion provides rich priors for training humanoid control policies, but raw demonstrations are incompatible with robot kinematics and dynamics, limiting direct use. Need to generate natural and dynamically feasible motion references from task-space human data.

Method: Two-stage pipeline: 1) Convert human motion to URDF-based skeleton representation and calibrate to target humanoid dimensions, aligning skeleton structure to reduce IK error. 2) Refine trajectories through progressive kinodynamic trajectory optimization in three stages: kinematic TO, inverse dynamics, and full kinodynamic TO, each warm-started from previous solution.

Result: Generates dynamically consistent state trajectories and joint torque profiles, providing high-quality references for learning-based controllers. Enables natural, physically consistent motion references across diverse humanoid platforms.

Conclusion: Skeleton calibration and kinodynamic trajectory optimization together enable generation of natural, physically consistent motion references from human demonstrations for various humanoid robots, overcoming incompatibility issues between human and robot kinematics/dynamics.
...
### [88] [MiNI-Q: A Miniature, Wire-Free Quadruped with Unbounded, Independently Actuated Leg Joints](https://arxiv.org/abs/2603.11537)
*Daniel Koh,Suraj Shah,Yufeng Wu,Dennis Hong*

Main category: cs.RO

TL;DR: MiNI-Q² is a miniature wire-free quadruped robot with mechanically unbounded 2-DOF leg joints that enables diverse locomotion capabilities including crawling, stair climbing, inverted motion, jumping, and backflipping.
...
Method: Developed MiNI-Q² with independently actuated, mechanically unbounded 2-DOF leg joints; presented mechanical design, kinematic analysis, and experimental validation; extended wire-free architecture from previous Q8bot design for improved assembly reliability.

Result: Robot achieves speeds up to 0.46 m/s, demonstrates low-clearance crawling, stair climbing, inverted locomotion, jumping, and backflipping, and folds to minimum height of 2.5 cm. All design files released open source.

Conclusion: MiNI-Q² successfully demonstrates the advantages of mechanically unbounded leg joints for miniature quadruped robots, enabling versatile locomotion capabilities while maintaining compact form factor and wire-free operation.
...
### [89] [RoboClaw: An Agentic Framework for Scalable Long-Horizon Robotic Tasks](https://arxiv.org/abs/2603.11558)
...
TL;DR: RoboClaw is a VLA framework that unifies data collection, policy learning, and task execution under a single VLM controller using Entangled Action Pairs for autonomous self-resetting, enabling continuous on-policy learning and robust long-horizon task execution.


<details>
  <summary>Details</summary>
Motivation: Existing VLA systems for robotic manipulation face challenges in scaling to long-horizon tasks due to separated pipelines for data collection, policy learning, and deployment, which require manual environment resets and suffer from brittle multi-policy execution.

Method: RoboClaw introduces Entangled Action Pairs (EAP) that couple forward manipulation behaviors with inverse recovery actions to form self-resetting loops for autonomous data collection. The framework uses a single VLM-driven controller that performs high-level reasoning and dynamically orchestrates learned policy primitives during deployment.

Result: Experiments show 25% improvement in success rate over baseline methods on long-horizon tasks and 53.7% reduction in human time investment. The framework demonstrates improved stability and scalability compared to conventional open-loop pipelines.

Conclusion: RoboClaw successfully addresses the scalability challenges of VLA systems by unifying data collection, policy learning, and execution under a single controller with self-resetting mechanisms, significantly reducing human effort while improving robustness for long-horizon manipulation tasks.
...
### [90] [Unsupervised LiDAR-Based Multi-UAV Detection and Tracking Under Extreme Sparsity](https://arxiv.org/abs/2603.11586)
*Nivand Khosravi,Rodrigo Ventura,Meysam Basiri*

Main category: cs.RO

TL;DR: Unsupervised LiDAR-only pipeline for detecting and tracking airborne UAVs using sparse point clouds from solid-state LiDAR, with range-adaptive DBSCAN clustering and probabilistic data association for multi-target tracking.


<details>
  <summary>Details</summary>
Motivation: Solid-state LiDAR scanning produces extremely sparse measurements for airborne UAV detection (1-2 returns per scan), which is inadequate for existing detection approaches and robust multi-target data association, requiring an unsupervised solution without labeled training data.

Method: Unsupervised LiDAR-only pipeline with range-adaptive DBSCAN clustering and three-stage temporal consistency check for detection. For tracking, compares deterministic Hungarian assignment with joint probabilistic data association (JPDA), both coupled with Interacting Multiple Model filtering. Uses two-environment evaluation combining real-world detection with RTK-GPS ground truth and simulation-based tracking.

Result: Best detection setup achieves 0.891 precision, 0.804 recall, and 0.63 m RMSE. JPDA reduces identity switches by 64% with negligible impact on MOTA compared to Hungarian assignment, demonstrating advantage in ambiguous scenarios with closely approaching UAV trajectories.

Conclusion: The proposed unsupervised pipeline effectively addresses sparse LiDAR measurements for UAV detection and tracking, with probabilistic data association significantly improving identity maintenance in ambiguous multi-target scenarios, overcoming limitations of GNSS-only evaluation at close inter-UAV distances.
...
### [91] [Diversity You Can Actually Measure: A Fast, Model-Free Diversity Metric for Robotics Datasets](https://arxiv.org/abs/2603.11634)
*Sreevardhan Sirigiri,Nathan Samuel de Lara,Christopher Agia,Florian Shkurti,Fabio Ramos*

Main category: cs.RO

TL;DR: FAKTUAL introduces a model-free data curation method for robot imitation learning that uses signature kernel-based entropy to select diverse demonstrations, improving downstream success rates without policy access or training overhead.


<details>
  <summary>Details</summary>
Motivation: Existing robotics datasets for imitation learning contain trajectories of varying lengths with high-dimensional observations, making it difficult to quantify diversity while respecting trajectory structure and geometry. Current methods lack principled ways to measure and leverage dataset diversity for improved imitation learning performance.

Method: Extends Shannon and von Neumann entropy to trajectory data using signature transform-based entropy on Gram matrices of signature kernels over demonstrations. Introduces FAKTUAL (FAst trajectory Kernel enTropy cUration for imitation Learning), a model-free algorithm that selects demonstration subsets maximizing entropy given a budget, requiring no policy access or rollouts.

Result: FAKTUAL consistently improves downstream success rates over random selection across image/state-based RoboMimic and MetaWorld benchmarks and four real-world manipulation tasks. It's substantially more computationally efficient than recent robot data curation methods while adding negligible overhead relative to policy training.

Conclusion: Signature kernel-based entropy provides a practical tool for understanding and improving dataset diversity in robot imitation learning. FAKTUAL demonstrates that diversity-aware curation with model-free entropy maximization can significantly enhance imitation learning performance across diverse tasks and architectures.
...
### [92] [Learn Structure, Adapt on the Fly: Multi-Scale Residual Learning and Online Adaptation for Aerial Manipulators](https://arxiv.org/abs/2603.11638)
*Samaksh Ujjawal,Naveen Sudheer Nair,Shivansh Pratap Singh,Rishabh Dev Yadav,Wei Pan,Spandan Roy*

Main category: cs.RO

TL;DR: FDT+LRA framework for real-time residual modeling in aerial manipulators, using factorized attention for cross-variable coupling and multi-scale dynamics, with latent adaptation for deployment-time distribution shifts.


<details>
  <summary>Details</summary>
Motivation: AAMs have complex nonlinear dynamics with nonstationary residuals during manipulator reconfiguration and payload changes. Conventional analytical models have fixed structures, while static data-driven models assume stationarity and degrade under configuration changes. Existing learning methods don't properly factorize cross-variable coupling and multi-scale temporal effects.

Method: Proposes Factorized Dynamics Transformer (FDT) that treats physical variables as independent tokens, enabling explicit cross-variable attention while separating short-horizon inertial dependencies from long-horizon aerodynamic effects. Uses Latent Residual Adapter (LRA) for rapid linear adaptation via Recursive Least Squares in latent space to handle deployment-time distribution shifts without computational overhead.

Result: Real-world experiments on aerial manipulator with unseen payloads show higher prediction fidelity, accelerated disturbance attenuation, and superior closed-loop tracking precision compared to state-of-the-art learning baselines, while maintaining real-time feasibility.

Conclusion: The FDT+LRA framework effectively addresses the challenges of residual modeling in AAMs by factorizing cross-variable coupling and multi-scale dynamics, with latent adaptation enabling robust performance under distribution shifts, demonstrating practical real-time applicability.
...
### [93] [Chunk-Boundary Artifact in Action-Chunked Generative Policies: A Noise-Sensitive Failure Mechanism](https://arxiv.org/abs/2603.11642)
*Rui Wang*

Main category: cs.RO

TL;DR: Action chunking in visuomotor policies creates execution discontinuities at boundaries that are noise-sensitive failure mechanisms, not just unavoidable nuisances.


<details>
  <summary>Details</summary>
Motivation: To understand and address the execution discontinuities that arise at chunk boundaries in action-chunked generative visuomotor policies, which are currently poorly understood but central to policy design.

Method: Analyzed frozen pretrained action-chunked policies to identify chunk-boundary artifacts as noise-sensitive failure mechanisms. Used statistical tests (p < 1e-4, permutation test), systematic noise modulation, identification of artifact-related directions in noise space, and trajectory-level steering to manipulate artifact magnitude.

Result: Chunk-boundary artifacts are strongly associated with task failure, emerge during rollout (not just post-hoc), are systematically modulated by latent noise, and can be reliably altered via noise-space steering. In hard tasks, success/failure distributions shift accordingly; in near-ceiling tasks, positive gains are compressed by policy saturation while negative causal effects remain visible.

Conclusion: Boundary discontinuity can be recast from an unavoidable execution nuisance into an analyzable, noise-dominated, and intervenable failure mechanism, enabling systematic analysis and intervention in action-chunked policies.
...
### [94] [A Hybrid Neural-Assisted Unscented Kalman Filter for Unmanned Ground Vehicle Navigation](https://arxiv.org/abs/2603.11649)
*Gal Versano,Itzik Klein*

Main category: cs.RO

TL;DR: Hybrid estimation framework combining UKF with deep neural network to predict dynamic noise covariance from raw inertial/GNSS data, trained only on simulation data with sim2real transfer, achieving 12.7% position improvement across diverse vehicle datasets.
...
Method: Hybrid framework that preserves classical unscented Kalman filter equations while adding a deep neural network to predict process and measurement noise uncertainty directly from raw inertial and GNSS measurements. Uses sim2real approach with training exclusively on simulated data to obtain perfect ground truth and avoid extensive real-world data collection.

Result: Achieved 12.7% position improvement compared to adaptive model-based approach across three diverse datasets featuring different vehicle types (off-road vehicle, passenger car, mobile robot), inertial sensors, road surfaces, and environmental conditions over 160 minutes of testing.

Conclusion: The proposed hybrid framework offers a scalable and more robust solution for unmanned ground vehicle navigation by effectively bridging classical state estimation with modern deep learning, demonstrating strong generalization capabilities through sim2real training.
...
### [95] [Concurrent Prehensile and Nonprehensile Manipulation: A Practical Approach to Multi-Stage Dexterous Tasks](https://arxiv.org/abs/2603.11655)
*Hao Jiang,Yue Wu,Yue Wang,Gaurav S. Sukhatme,Daniel Seita*

Main category: cs.RO

TL;DR: DexMulti: A sample-efficient approach for dexterous multi-task manipulation that decomposes demonstrations into object-centric skills and uses a retrieve-align-execute paradigm, achieving 66% success rate with only 3-4 demonstrations per object.
...
Method: Decomposes demonstrations into object-centric skills with well-defined temporal boundaries, retrieves demonstrated skills based on current object geometry, aligns them to observed object state using uncertainty-aware estimator tracking centroid and yaw, and executes via retrieve-align-execute paradigm.

Result: Achieves average 66% success rate on training objects with only 3-4 demonstrations per object, outperforming diffusion policy baselines by 2-3x while requiring far fewer demonstrations. Demonstrates robust generalization to held-out objects and spatial variations up to +/-25 cm across three multi-stage tasks (Grasp + Pull, Grasp + Open, Grasp + Grasp) on two dexterous hands (Allegro and LEAP) in over 1,000 real-world trials.

Conclusion: DexMulti provides a sample-efficient approach for real-world dexterous multi-task manipulation by decomposing demonstrations into reusable skills and using geometry-aware retrieval and alignment, enabling robust generalization with minimal demonstration data.
...
### [96] [Coupling Tensor Trains with Graph of Convex Sets: Effective Compression, Exploration, and Planning in the C-Space](https://arxiv.org/abs/2603.11658)
*Gerhard Reinerth,Riddhiman Laha,Marcello Romano*

Main category: cs.RO

TL;DR: TANGO integrates tensor compression with graph optimization for efficient, scalable motion planning by approximating configuration space with Tensor Train decomposition and embedding regions into GCS-like structures.


<details>
  <summary>Details</summary>
Motivation: Existing optimization-based planners like Graph of Convex Sets (GCS) require predefined convex characterization of high-dimensional configuration space, which is often intractable for general robotic tasks, limiting their practical applicability.

Method: Uses Tensor Train decomposition to approximate feasible configuration space in compressed form, enabling rapid discovery of task-relevant regions, then embeds these regions into a GCS-like structure for geometry-aware motion planning that respects system constraints and environmental complexity.

Result: Rigorous simulation studies on planar and real robots demonstrate effective compression and generation of higher quality trajectories compared to existing approaches.

Conclusion: TANGO enables efficient, geometry-aware motion planning by coupling tensor-based compression with structured graph reasoning, laying groundwork for more expressive and scalable configuration space representations in future robotic systems.
...
### [97] [Energy Prediction on Sloping Ground for Quadruped Robots](https://arxiv.org/abs/2603.11963)
*Mohamed Ounally,Cyrille Pierre,Johann Laconte*

Main category: cs.RO

TL;DR: A simple energy model for quadruped robots that predicts locomotion cost based on terrain slope and heading, using only onboard sensors and validated on natural terrain.


<details>
  <summary>Details</summary>
Motivation: Energy management is critical for legged robots in outdoor environments, affecting mission success and ecological impact. Understanding how terrain slope and heading orientation influence energetic cost is essential for efficient locomotion planning.

Method: Introduces a simple energy model that relies solely on standard onboard sensors without specialized instrumentation. The model is identified from field runs on a commercial quadruped and expressed as a compact function of slope angle and heading.
...
Conclusion: The proposed energy model successfully captures terrain-dependent locomotion costs using minimal sensor requirements, supporting energy-efficient path planning for quadruped robots in previously unexplored outdoor environments.
...
### [98] [Learning Visuomotor Policy for Multi-Robot Laser Tag Game](https://arxiv.org/abs/2603.11980)
*Kai Li,Shiyu Zhao*

Main category: cs.RO

TL;DR: End-to-end visuomotor policy for multi-robot laser tag trained via teacher-student distillation outperforms classic modular methods in accuracy and collision avoidance.


<details>
  <summary>Details</summary>
Motivation: Classic modular approaches for multi-robot laser tag face challenges including limited observability, reliance on depth mapping, and inter-robot communication requirements.

Method: Train high-performing teacher policy with multi-agent reinforcement learning, then distill knowledge into vision-based student policy using permutation-invariant feature extractor and depth heatmap input.

Result: Policy outperforms classic methods by 16.7% in hitting accuracy and 6% in collision avoidance, successfully deployed on real robots.

Conclusion: End-to-end visuomotor policy with teacher-student distillation effectively addresses limitations of classic modular approaches for multi-robot laser tag.
...
### [99] [Sim-to-reality adaptation for Deep Reinforcement Learning applied to an underwater docking application](https://arxiv.org/abs/2603.12020)
*Alaaeddine Chaarani,Narcis Palomeras,Pere Ridao*

Main category: cs.RO

TL;DR: This paper presents a deep reinforcement learning approach for autonomous underwater docking using a Girona AUV, addressing sim-to-real gaps through a high-fidelity digital twin environment with accelerated training and successful real-world validation.


<details>
  <summary>Details</summary>
Motivation: Traditional control methods struggle with unpredictable environmental conditions in autonomous underwater docking. While DRL offers robustness, significant bottlenecks exist in bridging the sim-to-real gap and managing high training latencies for practical deployment.

Method: Adapted Stonefish simulator into a multiprocessing RL framework with realistic AUV dynamics, collision models, and sensor noise. Used PPO algorithm to develop 6-DoF control policy trained in headless environment with randomized starting positions. Reward structure accounted for distance, orientation, action smoothness, and adaptive collision penalties for soft docking.
...
Conclusion: The systematic approach using high-fidelity digital twin environment effectively bridges the sim-to-real gap for autonomous underwater docking, demonstrating that DRL controllers can develop adaptive behaviors for complex underwater docking tasks with practical deployment potential.
...
### [100] [Flight through Narrow Gaps with Morphing-Wing Drones](https://arxiv.org/abs/2603.12059)
...
<details>
  <summary>Details</summary>
Motivation: Fixed-wing drones cannot traverse gaps narrower than their wingspan. Inspired by birds folding wings, the paper aims to enable sub-wingspan gap traversal using wing-sweep morphing, addressing control challenges of lift loss during gap passage and precise wing-sweep timing.

Method: Developed aerodynamic model for wing-sweep morphing drone including low speeds and post-stall angles. Integrated longitudinal dynamics into optimal reference trajectory generation and Nonlinear Model Predictive Control (NMPC) with runtime adaptive costs and constraints. Validated on 130g wing-sweep-morphing drone.

Result: Achieved average altitude error of 5 cm during narrow-gap passage at 5-7 m/s forward speeds, enforcing fully swept wings near gap across variable threshold distances. Drone compensates lift loss by accelerating and pitching up ahead of gap. Also demonstrated accurate gap passage while maintaining constant forward speed and near-constant altitude on hardware.

Conclusion: The wing-sweep morphing drone with optimal trajectory generation and adaptive NMPC successfully enables sub-wingspan gap traversal, addressing control challenges through systematic modeling and control framework that compensates for lift loss during wing-sweep maneuvers.
...
### [101] [Decentralized Cooperative Localization for Multi-Robot Systems with Asynchronous Sensor Fusion](https://arxiv.org/abs/2603.12075)
*Nivand Khosravi,Niusha Khosravi,Mohammad Bozorg,Masoud S. Bahraini*

Main category: cs.RO

TL;DR: A decentralized cooperative localization framework for mobile robots in GPS-denied environments using EKF with asynchronous communication, automatic coordinate alignment, and dual-landmark evaluation for improved robustness.


<details>
  <summary>Details</summary>
Motivation: Address localization challenges for nonholonomic mobile robots in GPS-denied environments with limited communication infrastructure, where conventional methods fail in feature-sparse terrains like enclosed spaces and underwater environments.

Method: Decentralized EKF-based localization with asynchronous communication, automatic coordinate system alignment via transformation matrices, and dual-landmark evaluation using both static environmental features and mobile robots as dynamic landmarks.

Result: DCL outperforms centralized cooperative localization with 34% RMSE reduction; dual-landmark variant achieves 56% improvement in both Gazebo simulation and real-world basement environments.

Conclusion: The proposed DCL framework enables reliable localization in challenging domains where conventional methods are ineffective, demonstrating applicability to enclosed spaces, underwater environments, and feature-sparse terrains.
...
### [102] [CRAFT: A Tendon-Driven Hand with Hybrid Hard-Soft Compliance](https://arxiv.org/abs/2603.12120)
*Leo Lin,Shivansh Patel,Jay Moon,Svetlana Lazebnik,Unnat Jain*

Main category: cs.RO

TL;DR: CRAFT hand is a tendon-driven anthropomorphic hand with hybrid hard-soft compliance that places soft material at joints and rigid links, using rolling-contact joints and 15 motors for compact design, achieving improved strength, endurance, and manipulation capabilities at under $600.


<details>
  <summary>Details</summary>
Motivation: To create an anthropomorphic hand that addresses the non-uniform nature of contact during manipulation, where impacts concentrate at joints while links carry most of the load, enabling better handling of fragile and low-friction items while maintaining compact form factor and affordability.

Method: Uses hybrid hard-soft compliance design with soft material at joints and rigid links, rolling-contact joint surfaces for repeatable motion paths, tendon-driven actuation with 15 motors mounted on fingers for compactness, and lightweight finger construction.

Result: In structural tests: improved strength and endurance while maintaining comparable repeatability. In teleoperation: improved handling of fragile and low-friction items, covers 33/33 grasps in Feix taxonomy. Full design costs under $600 with open-source release including vision-based teleoperation and simulation integration.

Conclusion: CRAFT hand successfully demonstrates a cost-effective, open-source anthropomorphic hand design with hybrid compliance that improves manipulation capabilities while maintaining structural integrity and repeatability, making advanced robotic manipulation more accessible.
...
### [103] [ComFree-Sim: A GPU-Parallelized Analytical Contact Physics Engine for Scalable Contact-Rich Robotics Simulation and Control](https://arxiv.org/abs/2603.12185)
*Chetan Borse,Zhixian Xie,Wei-Cheng Huang,Wanxin Jin*

Main category: cs.RO

TL;DR: ComFree-Sim is a GPU-parallelized contact physics engine using complementarity-free modeling with closed-form contact impulse computation, achieving near-linear runtime scaling with contact density and 2-3× higher throughput than MuJoCo Warp.


<details>
  <summary>Details</summary>
Motivation: Traditional physics engines for contact-rich robotics suffer from superlinear runtime scaling with contact density due to iterative solves for complementarity constraints, creating bottlenecks for real-time applications like MPC and dexterous manipulation.

Method: Uses complementarity-free contact modeling with impedance-style prediction-correction updates in the dual cone of Coulomb friction. Contact computation decouples across contact pairs and cone facets, enabling GPU parallelization. Extends to unified 6D contact model (tangential, torsional, rolling friction) with practical dual-cone impedance heuristic.

Result: Achieves near-linear runtime scaling with number of contacts, 2-3× higher throughput than MuJoCo Warp in dense contact scenes with comparable physical fidelity. Successfully deployed in real-time MPC for in-hand dexterous manipulation on LEAP hand and dynamics-aware motion retargeting.

Conclusion: ComFree-Sim demonstrates that complementarity-free contact modeling with GPU parallelization enables efficient, scalable physics simulation for contact-rich robotics, improving real-time control performance and enabling practical high-frequency applications.
...
### [104] [SaPaVe: Towards Active Perception and Manipulation in Vision-Language-Action Models for Robotics](https://arxiv.org/abs/2603.12193)
*Mengzhen Liu,Enshen Zhou,Cheng Chi,Yi Han,Shanyu Rong,Liming Chen,Pengwei Wang,Zhongyuan Wang,Shanghang Zhang*

Main category: cs.RO

TL;DR: SaPaVe is an end-to-end framework that jointly learns semantic-driven active perception and robust manipulation through decoupled camera/action spaces, bottom-up training, and novel datasets, outperforming existing VLAs in real-world tasks.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle to unify semantic-driven active perception with robust, viewpoint-invariant execution in robot manipulation tasks. Current approaches fail to effectively coordinate camera control and manipulation actions in dynamic environments.

Method: SaPaVe decouples camera and manipulation actions rather than using a shared action space, employs bottom-up training (first semantic camera control on large-scale dataset, then joint optimization with hybrid data), introduces ActiveViewPose-200K dataset for camera movement learning, and incorporates a 3D geometry-aware module for execution robustness under dynamic viewpoints.

Result: Extensive experiments in simulation and real-world environments show SaPaVe outperforms recent vision-language-action models like GR00T N1 and π₀, achieving up to 31.25% higher success rates in real-world tasks. The framework also introduces ActiveManip-Bench, the first benchmark for evaluating active manipulation beyond fixed-view settings.

Conclusion: Tightly coupled perception and execution, when trained with decoupled yet coordinated strategies, enable efficient and generalizable active manipulation. The proposed framework demonstrates that semantic-driven active perception and robust manipulation can be effectively unified through careful architectural design and training methodology.
...
### [105] [HandelBot: Real-World Piano Playing via Fast Adaptation of Dexterous Robot Policies](https://arxiv.org/abs/2603.12243)
*Amber Xie,Haozhi Qi,Dorsa Sadigh*

Main category: cs.RO

TL;DR: HandelBot: A two-stage framework combining simulation-trained policy with rapid physical adaptation for precise bimanual piano playing with multi-fingered robotic hands.
...
Method: Two-stage pipeline: 1) Structured refinement stage corrects spatial alignments by adjusting lateral finger joints based on physical rollouts; 2) Residual reinforcement learning autonomously learns fine-grained corrective actions.
...
Conclusion: HandelBot demonstrates effective combination of simulation training with rapid physical adaptation for high-precision dexterous manipulation tasks, addressing the data collection bottleneck for millimeter-scale precision requirements.
...
### [106] [HumDex:Humanoid Dexterous Manipulation Made Easy](https://arxiv.org/abs/2603.12260)
*Liang Heng,Yihe Tang,Jiajun Xu,Henghui Bao,Di Huang,Yue Wang*

Main category: cs.RO

TL;DR: HumDex is a portable teleoperation system for humanoid whole-body dexterous manipulation that uses IMU-based motion tracking and learning-based hand retargeting, enabling efficient human motion data collection and a two-stage imitation learning framework for improved generalization.


<details>
  <summary>Details</summary>
Motivation: Efficient collection of high-quality demonstration data for humanoid whole-body dexterous manipulation is a bottleneck. Existing teleoperation systems have limitations in portability, occlusion, and precision, hindering their applicability to complex whole-body tasks.

Method: HumDex system uses IMU-based motion tracking for portable full-body tracking and introduces a learning-based retargeting method for smooth hand motions. It enables human motion data collection and proposes a two-stage imitation learning framework: pre-training on diverse human motion data to learn generalizable priors, then fine-tuning on robot data to bridge the embodiment gap.
...
Conclusion: HumDex addresses the portability-precision trade-off in teleoperation systems and enables efficient data collection for humanoid whole-body dexterous manipulation, with a two-stage imitation learning framework that enhances generalization capabilities.
...
### [107] [$Ψ_0$: An Open Foundation Model Towards Universal Humanoid Loco-Manipulation](https://arxiv.org/abs/2603.12263)
...
TL;DR: Psi-Zero (Ψ₀) is an open foundation model for humanoid loco-manipulation that uses a staged training approach with human video pre-training followed by robot data post-training, achieving superior performance with significantly less data than existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing approaches co-train on large human and humanoid datasets but suffer from kinematic disparities between humans and robots, leading to poor data efficiency and suboptimal performance despite large data volumes.

Method: Two-stage training: 1) Autoregressive pre-training of VLM backbone on large-scale egocentric human videos for visual-action representations, 2) Flow-based action expert post-training on high-quality humanoid robot data for precise joint control. Emphasizes data quality over quantity.

Result: Achieves best performance using only ~800 hours of human video and 30 hours of real robot data, outperforming baselines trained on 10× more data by over 40% in overall success rate across multiple tasks.

Conclusion: Decoupled learning with staged training on high-quality human manipulation data followed by domain-specific robot trajectories is more effective than scaling noisy internet clips or heterogeneous cross-embodiment datasets for humanoid loco-manipulation.
