...
TL;DR: AIdentifyAGE ontology provides standardized framework for forensic dental age assessment, addressing methodological heterogeneity and enabling traceable linkage between observations, methods, and outcomes in medico-legal contexts.


<details>
  <summary>Details</summary>
Motivation: Current forensic dental age assessment practices face challenges including methodological heterogeneity, fragmented data representation, limited interoperability between systems, and lack of transparency/reproducibility - problems amplified by increasing AI adoption in this domain.

Method: Development of domain-specific ontology (AIdentifyAGE) that models complete medico-legal workflow, integrates judicial context, individual information, forensic examination data, dental development assessment methods, radiographic imaging, reference studies, and AI-based estimation methods, building on established biomedical, dental, and machine learning ontologies.

Result: A standardized, semantically coherent framework that enables traceable linkage between observations, methods, reference data, and reported outcomes, ensuring interoperability, extensibility, and FAIR principles compliance.

Conclusion: The AIdentifyAGE ontology enhances consistency, transparency, and explainability in forensic dental age assessment, establishing foundation for ontology-driven decision support systems in medico-legal and judicial contexts.
...
TL;DR: Contextuality emerges inevitably from single-state reuse in classical probabilistic systems, imposing irreducible information-theoretic costs that nonclassical frameworks avoid by relaxing global joint probability assumptions.


<details>
  <summary>Details</summary>
Motivation: To understand the fundamental representational consequences of single-state reuse in adaptive systems operating across multiple contexts, which is ubiquitous in natural and artificial intelligence but poorly understood.

Method: Model contexts as interventions acting on a shared internal state; prove that any classical model reproducing contextual outcome statistics must incur irreducible information-theoretic cost; provide minimal constructive example; analyze how nonclassical probabilistic frameworks avoid this obstruction.

Result: Contextuality is not unique to quantum mechanics but inevitable in classical probabilistic representations with single-state reuse; classical models must incur irreducible information-theoretic cost where dependence on context cannot be mediated solely through internal state.

Conclusion: Contextuality represents a general representational constraint on adaptive intelligence independent of physical implementation, arising from single-state reuse rather than quantum dynamics or Hilbert space structure.
...
<details>
  <summary>Details</summary>
Motivation: LLM-based human mobility simulations are computationally expensive, limiting scalability for large-scale applications like urban planning, epidemiology, and transportation analysis.
...
Conclusion: The proposed cache framework enables efficient large-scale human mobility simulations by reducing computational costs while preserving simulation fidelity through reconstructible caches and latent-space reasoning.
...
Conclusion: The study identifies which benchmark design choices extend longevity and provides insights for creating more durable evaluation frameworks that maintain their ability to differentiate between state-of-the-art models over time.
...
<details>
  <summary>Details</summary>
Motivation: To critically evaluate code evolution techniques by comparing them against simple baselines, as many proposed pipelines show impressive performance but lack comparison to simpler approaches.

Method: Testing two simple baselines across three domains: mathematical bounds, agentic scaffolds, and machine learning competitions, with analysis of search spaces, domain knowledge, and evaluation methods.
...
Conclusion: Code evolution research needs better evaluation methods to reduce stochasticity while maintaining economic feasibility, with emphasis on rigorous search space design and domain expertise rather than complex evolution pipelines.
...
TL;DR: The paper improves the upper bound for slicing all edges of an n-dimensional hypercube with hyperplanes, showing S(n) ≤ ⌈4n/5⌉ (with a small adjustment for odd multiples of 5), improving on the previous bound of ⌈5n/6⌉ from 1971. The proof uses a construction of 8 hyperplanes slicing Q₁₀ aided by an automated tool called CPro1.


<details>
  <summary>Details</summary>
Motivation: The problem of determining the minimum number of hyperplanes needed to slice all edges of an n-dimensional hypercube has been studied since at least 1971, with Paterson's bound of S(n) ≤ ⌈5n/6⌉ standing for decades. The authors aim to improve this long-standing upper bound using modern computational tools.

Method: The authors construct 8 hyperplanes that slice all edges of the 10-dimensional hypercube Q₁₀. This construction is aided by CPro1, an automatic tool that uses reasoning large language models coupled with automated hyperparameter tuning to create search algorithms for discovering mathematical constructions. The 10-dimensional construction is then used to derive the general bound for all n.

Result: The main result is S(n) ≤ ⌈4n/5⌉, except when n is an odd multiple of 5, in which case S(n) ≤ 4n/5 + 1. This improves upon Paterson's 1971 bound of S(n) ≤ ⌈5n/6⌉. The paper also obtains new lower bounds on the maximum number of edges in Q_n that can be sliced using k < n hyperplanes.

Conclusion: The paper demonstrates that modern computational tools like CPro1 can help improve long-standing combinatorial bounds. The improved bound S(n) ≤ ⌈4n/5⌉ represents significant progress on a problem that had seen no improvement for over 50 years, showing the potential of AI-assisted mathematical discovery.
...
TL;DR: NeuDiff Agent is an AI workflow system for neutron crystallography that automates data reduction, integration, refinement, and validation to produce validated crystal structures and publication-ready CIF files, achieving 4.6-5.0x speedup over manual processing.


<details>
  <summary>Details</summary>
Motivation: Large-scale neutron facilities face analysis latency as the bottleneck in scientific throughput, especially for complex samples requiring iterative processing. Manual workflows are time-consuming and inefficient for structurally and magnetically complex samples.

Method: NeuDiff Agent is a governed, tool-using AI workflow that automates the established crystallography pipeline (reduction, integration, refinement, validation) for TOPAZ at the Spallation Neutron Source. It uses explicit governance through allowlisted tools, fail-closed verification gates at workflow boundaries, and complete provenance capture.

Result: In benchmark testing, NeuDiff Agent reduced wall time from 435 minutes (manual) to 86.5-94.4 minutes (4.6-5.0x faster) while producing validated CIF files with no checkCIF level A or B alerts. The system was tested with two different large language model backends using fixed prompt protocols.
...
TL;DR: Node Learning is a decentralized AI paradigm where intelligence resides at individual edge nodes and expands through selective peer interactions, enabling continuous local learning and opportunistic knowledge exchange without centralized coordination.
...
Conclusion: Node Learning unifies autonomous and cooperative behavior within a single abstraction, accommodates heterogeneity in data, hardware, objectives, and connectivity, and places existing paradigms within a broader decentralized perspective without discarding them.
...
TL;DR: The paper proposes an order-oriented scoring framework for hesitant fuzzy sets, proves classical orders don't form lattices, introduces dominance functions for ranking with control sets, and demonstrates applications in decision-making.


<details>
  <summary>Details</summary>
Motivation: Traditional scoring approaches for hesitant fuzzy sets lack formal order-theoretic foundations, leading to less flexible and coherent scoring mechanisms. The paper aims to establish a unified framework where scores are explicitly defined with respect to given orders.

Method: 1) Analyzes classical orders on hesitant fuzzy elements (nonempty subsets of [0,1]) and proves they don't induce lattice structures. 2) Shows scores defined with symmetric order satisfy normative criteria. 3) Introduces dominance functions for ranking relative to control sets with minimum acceptability thresholds. 4) Provides two concrete examples: discrete dominance function and relative dominance function for finite sets.

Result: 1) Contrary to prior claims, classical orders on hesitant fuzzy elements do not induce lattice structures. 2) Scores defined with symmetric order satisfy strong monotonicity with respect to unions and the Gärdenfors condition. 3) Dominance functions can construct fuzzy preference relations on hesitant fuzzy sets and support group decision-making.

Conclusion: The order-oriented framework provides a more rigorous foundation for scoring hesitant fuzzy sets. Dominance functions offer practical tools for ranking relative to control sets, enabling flexible applications in decision-making contexts where minimum acceptability thresholds are relevant.
...
TL;DR: GUI-Owl-1.5 is a state-of-the-art native GUI agent model with multiple size variants that achieves SOTA results across 20+ GUI benchmarks for automation, grounding, tool-calling, and knowledge tasks on desktop, mobile, and browser platforms.


<details>
  <summary>Details</summary>
Motivation: To develop a comprehensive GUI agent capable of handling diverse GUI automation tasks across multiple platforms (desktop, mobile, browser) with cloud-edge collaboration and real-time interaction capabilities.

Method: Three key innovations: 1) Hybrid Data Flywheel combining simulated and cloud-based sandbox environments for UI understanding and trajectory generation; 2) Unified thought-synthesis pipeline to enhance reasoning capabilities with focus on tool use, memory, and multi-agent adaptation; 3) Multi-platform Environment RL Scaling using MRPO algorithm to address platform conflicts and improve training efficiency for long-horizon tasks.
...
Conclusion: GUI-Owl-1.5 represents a significant advancement in GUI agent technology with multiple size variants supporting diverse platforms, achieving SOTA performance through innovative data collection, reasoning enhancement, and multi-platform RL scaling approaches, with models open-sourced for community use.
...
TL;DR: OpenSage is an agent development kit that enables LLMs to automatically create agents with self-generated topology and toolsets, featuring hierarchical memory and specialized software engineering toolkits, outperforming existing ADKs across benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current agent development kits either lack sufficient functional support or require manual human design of agent topology, tools, and memory, limiting agent generalizability and performance. There's a need for an ADK that enables automatic agent creation with comprehensive support.

Method: OpenSage enables LLMs to automatically create agents with self-generated topology and toolsets. It provides functionality for agents to create/manage sub-agents and toolkits, features a hierarchical graph-based memory system for efficient management, and includes specialized toolkits for software engineering tasks.

Result: Extensive experiments across three state-of-the-art benchmarks with various backbone models demonstrate OpenSage's advantages over existing ADKs. Rigorous ablation studies confirm the effectiveness of each component design.

Conclusion: OpenSage represents a paradigm shift from human-centered to AI-centered agent development, paving the way for next-generation agent development kits by enabling automatic agent creation with comprehensive topology, toolset, and memory support.
...
TL;DR: AgentLAB is the first benchmark for evaluating LLM agent vulnerabilities to adaptive, long-horizon attacks across 28 environments with 644 security test cases, revealing current agents remain highly susceptible despite single-turn defenses.


<details>
  <summary>Details</summary>
Motivation: LLM agents are increasingly deployed in complex, long-horizon environments, exposing them to multi-turn attacks that exploit user-agent-environment interactions in ways infeasible in single-turn settings, necessitating systematic evaluation of these vulnerabilities.

Method: Developed AgentLAB benchmark with five novel attack types (intent hijacking, tool chaining, task injection, objective drifting, memory poisoning) across 28 realistic agentic environments and 644 security test cases to systematically evaluate LLM agent susceptibility.
...
Conclusion: AgentLAB serves as a valuable benchmark for tracking progress in securing LLM agents against long-horizon attacks in practical settings, highlighting the need for new defense mechanisms beyond single-turn protections.
...
TL;DR: LLM-Wikirace is a benchmark testing LLMs' planning, reasoning, and world knowledge through Wikipedia navigation tasks, revealing superhuman performance on easy levels but significant limitations on hard tasks where planning and long-horizon reasoning become critical.


<details>
  <summary>Details</summary>
Motivation: To create a benchmark that evaluates planning, reasoning, and world knowledge in LLMs through Wikipedia navigation tasks, addressing the need for assessing how models connect concepts and plan multi-step paths in real-world knowledge graphs.

Method: Developed LLM-Wikirace benchmark where models must navigate Wikipedia hyperlinks step-by-step from source to target pages, requiring look-ahead planning. Evaluated diverse open- and closed-source models including Gemini-3, GPT-5, and Claude Opus 4.5 across easy and hard difficulty levels, with trajectory-level analysis of planning failures.

Result: Frontier models achieve superhuman performance on easy tasks but performance drops sharply on hard difficulty (Gemini-3 succeeds in only 23% of hard games). World knowledge is necessary but insufficient beyond a threshold where planning and long-horizon reasoning become dominant. Models struggle with replanning after failure, frequently entering loops.

Conclusion: LLM-Wikirace reveals clear limitations in current reasoning systems, showing that while frontier models excel on simple planning tasks, they face substantial challenges with complex, long-horizon reasoning and recovery from failures, highlighting an open arena where planning-capable LLMs still need significant improvement.
...
TL;DR: Fine-tuning aligned vision-language models on narrow harmful datasets causes severe emergent misalignment that generalizes across tasks and modalities, with multimodal evaluation revealing higher degradation than text-only benchmarks.


<details>
  <summary>Details</summary>
Motivation: Lifelong multimodal agents need continuous adaptation through post-training, but this creates tension between acquiring capabilities and preserving safety alignment. The paper investigates how fine-tuning on harmful data affects alignment in vision-language models.

Method: Experiments on Gemma3-4B with LoRA fine-tuning on harmful datasets, evaluating misalignment scaling with LoRA rank. Used multimodal evaluation (vs text-only), analyzed geometric properties of harmful behaviors via PCA, and tested mitigation strategies (benign narrow fine-tuning and activation-based steering).

Result: Misalignment scales monotonically with LoRA rank, with multimodal evaluation showing substantially higher misalignment (70.71±1.22 at r=128) than text-only (41.19±2.51). Even 10% harmful data induces substantial degradation. Harmful behaviors occupy low-dimensional subspace (10 principal components capture majority). Mitigation strategies reduce but don't completely remove harmful behaviors.

Conclusion: Current post-training paradigms may not sufficiently preserve alignment in post-deployment settings, highlighting need for robust continual learning frameworks. Unimodal safety benchmarks underestimate alignment degradation in vision-language models.
...
TL;DR: DeepContext is a stateful RNN-based framework that tracks temporal intent evolution across multi-turn dialogues to detect incremental jailbreak attempts that bypass stateless safety filters.
...
Method: DeepContext uses a Recurrent Neural Network (RNN) architecture that ingests sequences of fine-tuned turn-level embeddings and propagates a hidden state across conversations to capture incremental risk accumulation that stateless models overlook.

Result: DeepContext achieves state-of-the-art F1 score of 0.84 for multi-turn jailbreak detection, significantly outperforming hyperscaler cloud-provider guardrails and leading open-weight models like Llama-Prompt-Guard-2 (0.67) and Granite-Guardian (0.67), while maintaining sub-20ms inference overhead on T4 GPU.

Conclusion: Modeling the sequential evolution of intent is a more effective and computationally efficient alternative to deploying massive, stateless models for LLM safety monitoring.
...
TL;DR: SourceBench is a benchmark for evaluating the quality of web sources cited by LLMs across 100 real-world queries, using an eight-metric framework covering content quality and page-level signals, with human-labeled data and LLM-based evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations of LLMs focus primarily on answer correctness while neglecting the quality of cited web sources, creating a gap in assessing evidence quality in LLM-generated responses.

Method: Developed SourceBench with 100 real-world queries spanning five intents (informational, factual, argumentative, social, shopping). Created an eight-metric framework covering content quality (relevance, accuracy, objectivity) and page-level signals (freshness, authority/accountability, clarity). Built a human-labeled dataset and calibrated LLM-based evaluator matching expert judgments. Evaluated eight LLMs, Google Search, and three AI search tools across 3996 cited sources.

Result: The benchmark provides systematic evaluation of source quality, with LLM-based evaluator closely matching expert judgments. Evaluation of multiple systems reveals four key new insights about GenAI and web search that can guide future research.

Conclusion: SourceBench addresses the gap in evaluating evidence quality in LLM responses, providing a comprehensive benchmark for assessing web source quality across diverse query types, with findings offering important guidance for future GenAI and web search research.
...
TL;DR: Text-level safety evaluations don't transfer to tool-call safety in LLM agents; models can refuse harmful requests in text while simultaneously executing forbidden actions via tool calls.


<details>
  <summary>Details</summary>
Motivation: Current safety evaluations focus on text-level refusal behavior but don't assess whether alignment that suppresses harmful text also suppresses harmful actions through tool calls, creating a critical gap in safety assessment for LLM agents interacting with external systems.

Method: Introduced GAP benchmark with systematic evaluation across six frontier models, six regulated domains, seven jailbreak scenarios per domain, three system prompt conditions, and two prompt variants, producing 17,420 analysis-ready datapoints to measure divergence between text-level and tool-call-level safety.

Result: Text safety doesn't transfer to tool-call safety; models exhibit GAP cases where text refuses harmful requests while tool calls execute forbidden actions (219 cases persist even with safety-reinforced prompts). System prompt wording significantly influences tool-call behavior (TC-safe rates vary 21-57 percentage points). Runtime governance contracts reduce information leakage but don't deter forbidden tool-call attempts.
...
TL;DR: LLM4Cov: An offline agent-learning framework for hardware verification that uses execution-validated data curation, policy-aware agentic data synthesis, and worst-state-prioritized sampling to enable scalable learning under execution constraints.


<details>
  <summary>Details</summary>
Motivation: Execution-aware LLM agents for learning from tool feedback face challenges with expensive and slow feedback, making online reinforcement learning impractical, especially in hardware verification which relies on industrial simulators and non-differentiable execution signals.

Method: Models verification as memoryless state transitions guided by deterministic evaluators; introduces execution-validated data curation, policy-aware agentic data synthesis, and worst-state-prioritized sampling to enable scalable offline learning.
...
Conclusion: LLM4Cov provides an effective offline framework for hardware verification that addresses execution constraints through novel data curation and sampling techniques, enabling efficient learning with smaller models.
...
<details>
  <summary>Details</summary>
Motivation: Current agent hijacking attacks rely on manual prompt manipulation with low success rates and poor transferability to closed-source commercial models, necessitating a more systematic approach targeting fundamental architectural mechanisms.

Method: Phantom uses Structured Template Injection to exploit LLM agent chat template tokens, causing role confusion. It employs multi-level template augmentation, Template Autoencoder (TAE) for continuous latent space embedding, and Bayesian optimization to search for optimal adversarial templates.

Result: The framework significantly outperforms existing baselines in Attack Success Rate (ASR) and query efficiency across Qwen, GPT, and Gemini models, and identified over 70 vulnerabilities in real-world commercial products confirmed by vendors.

Conclusion: Structured template-based hijacking poses severe practical threats to LLM agents, requiring systematic defenses for next-generation agentic systems, with Phantom providing an empirical foundation for security improvements.
...
Method: HQFS uses a three-stage hybrid approach: 1) Learns next-step return and volatility proxy using variational quantum circuit (VQC) with small classical head, 2) Converts risk-return objective and constraints into QUBO solved via quantum annealing (with classical fallback), 3) Signs each rebalance output with post-quantum signature for verifiable audit trail.

Result: On market dataset: 7.8% reduction in return prediction error, 6.1% reduction in volatility prediction error vs classical baseline. Decision layer improves out-of-sample Sharpe by 9.4%, lowers maximum drawdown by 11.7%. QUBO solve stage cuts average solve time by 28% vs mixed-integer baseline while producing traceable signed allocation records.

Conclusion: HQFS demonstrates a practical hybrid quantum-classical pipeline that addresses key limitations of traditional financial risk systems by integrating forecasting, discrete optimization, and auditability, showing measurable improvements in prediction accuracy, portfolio performance, and computational efficiency while meeting regulatory requirements.
...
TL;DR: Black-box safety evaluation fails for models with latent context-conditioned policies, as no evaluator can reliably estimate deployment risk due to fundamental statistical and computational limits.


<details>
  <summary>Details</summary>
Motivation: To challenge the assumption that model behavior on test distributions reliably predicts deployment performance, particularly for models with unobserved internal variables that are rare during evaluation but prevalent during deployment.

Method: Formal analysis using latent context-conditioned policies, statistical minimax lower bounds via Le Cam's method, adaptive evaluation analysis with hash-based triggers and Yao's minimax principle, computational separation under trapdoor one-way function assumptions, and white-box probing with bias correction.

Result: Fundamental limits show: (1) passive evaluation incurs expected absolute error ≥ 0.208δL; (2) adaptive evaluation worst-case error ≥ δL/16; (3) computational separation prevents polynomial-time evaluators from detecting unsafe behaviors; white-box probing requires O(1/(γ²ε_R²)) samples.
...
Method: Created a conversational and longitudinal benchmark using real market data and human decision trajectories. Includes onboarding interviews, step-wise market context, and advisory dialogues where models generate rankings over fixed investment horizons. Provides multi-view references distinguishing descriptive behavior from normative utility grounded in investor-specific risk preferences.
...
Conclusion: Conv-FinRe enables diagnosis of whether LLMs follow rational analysis, mimic user noise, or are driven by market momentum. The benchmark addresses limitations of behavior-only evaluation in financial advisory contexts and is publicly available on Hugging Face and GitHub.
...
TL;DR: Sonar-TS is a neuro-symbolic framework for natural language querying of time series databases using a Search-Then-Verify pipeline, with NLQTSBench as the first large-scale benchmark for evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing Text-to-SQL methods cannot handle continuous morphological intents (shapes/anomalies) in time series, while time series models struggle with ultra-long histories, creating a gap for natural language querying of time series databases.

Method: Sonar-TS uses a neuro-symbolic Search-Then-Verify pipeline: first uses a feature index to ping candidate windows via SQL, then generates Python programs to lock on and verify candidates against raw signals, analogous to active sonar.

Result: Experiments demonstrate Sonar-TS effectively navigates complex temporal queries where traditional methods fail, highlighting unique challenges in the NLQ4TSDB domain.
...
TL;DR: Cinder is a two-stage matchmaking system for multiplayer games that uses Ruzicka similarity for initial filtering and Kantorovich distance on non-linear skill buckets for precise fairness evaluation.


<details>
  <summary>Details</summary>
Motivation: Traditional matchmaking based on average team skill metrics often results in unbalanced games, especially when dealing with lobbies (pre-made teams) of heterogeneous skill levels with wide or skewed distributions. This negatively impacts player retention and satisfaction in multiplayer online games.

Method: Two-stage system: 1) Rapid preliminary filter using Ruzicka similarity index on "non-outlier" skill ranges of lobbies; 2) Precise fairness evaluation by mapping player ranks to non-linear skill buckets (generated from inverted normal distribution for higher granularity at average skill levels) and calculating Kantorovich distance on sorted bucket indices to produce a "Sanction Score."

Result: System viability demonstrated by analyzing distribution of Sanction Scores from 140 million simulated lobby pairings, providing robust foundation for establishing fair matchmaking thresholds.

Conclusion: Cinder offers an effective solution for fair and fast matchmaking by combining efficient filtering with precise fairness quantification, addressing limitations of traditional average-based approaches for heterogeneous skill lobbies.
...
TL;DR: M2F is an agentic framework for end-to-end project-scale autoformalization in Lean that converts textbook mathematics into verified Lean code through two-stage compilation with verifier-in-the-loop feedback.


<details>
  <summary>Details</summary>
Motivation: Current automated formalization methods are limited to isolated theorems and short snippets, unable to handle textbook-scale mathematics which requires managing cross-file dependencies, resolving imports, and ensuring end-to-end compilation of entire projects.

Method: Two-stage framework: (1) Statement compilation stage splits documents into atomic blocks, orders via dependencies, repairs declaration skeletons until project compiles (allowing proof placeholders); (2) Proof repair stage closes proof holes using goal-conditioned local edits under fixed signatures. Both stages maintain verifier-in-the-loop feedback, committing edits only when toolchain confirms improvement.

Result: In ~3 weeks, converted 479 pages of real analysis and convex analysis textbooks into 153,853 lines of Lean code, achieving 96% proof success on FATE-H benchmark (vs. 80% baseline). Demonstrates textbook-scale formalization at pace typically requiring months/years of expert effort.

Conclusion: Practical, large-scale automated formalization of mathematical literature is within reach, as demonstrated by M2F's ability to handle project-scale autoformalization with high success rates and significant efficiency gains over manual expert effort.
...
TL;DR: Sales Research Agent for CRM data outperforms leading AI models on enterprise sales analytics benchmarks.


<details>
  <summary>Details</summary>
Motivation: Enterprises need AI systems that can answer sales-leader questions over live CRM data with transparent, repeatable evidence of quality, which most available models lack.

Method: Developed Sales Research Agent in Microsoft Dynamics 365 Sales that connects to live CRM data, reasons over complex schemas, and produces insights through text and charts. Created Sales Research Bench benchmark scoring systems on eight customer-weighted dimensions including groundedness, relevance, explainability, schema accuracy, and chart quality.
...
Conclusion: The Sales Research Agent provides enterprises with a high-performing AI solution for CRM analytics that offers transparent, repeatable quality assessment through the Sales Research Bench benchmark, enabling customers to compare AI solutions effectively.
...
TL;DR: PA-MoE addresses simplicity bias in RL by using phase-aware routing instead of token-level routing in MoE architectures, enabling experts to specialize in temporally consistent task phases.


<details>
  <summary>Details</summary>
Motivation: Existing RL methods with single policy networks suffer from simplicity bias where simple tasks dominate parameter updates, leaving insufficient capacity for complex tasks. Traditional MoE's token-level routing fragments phase-consistent patterns, undermining expert specialization.

Method: Proposes Phase-Aware Mixture of Experts (PA-MoE) with a lightweight phase router that learns latent phase boundaries directly from RL objectives without pre-defined categories, allocating temporally consistent assignments to the same expert.

Result: Experimental results demonstrate the effectiveness of PA-MoE in addressing simplicity bias and improving expert specialization for complex tasks.

Conclusion: PA-MoE successfully overcomes limitations of traditional MoE by introducing phase-aware routing that preserves phase-specific expertise, providing a more effective architecture for RL agents handling diverse task complexities.
...
TL;DR: ITR (Instruction-Tool Retrieval) reduces LLM agent costs by 95% per-step context tokens and 70% end-to-end via dynamic retrieval of only necessary prompt fragments and tools per step.


<details>
  <summary>Details</summary>
Motivation: LLM agents waste resources by re-ingesting long system instructions and large tool catalogs each turn, increasing costs, latency, tool-selection errors, and derailment probability.

Method: Instruction-Tool Retrieval (ITR), a RAG variant that retrieves per step only minimal system-prompt fragments and smallest necessary tool subsets, composing dynamic runtime prompts with narrowed toolsets and confidence-gated fallbacks.
...
Conclusion: ITR is particularly valuable for long-running autonomous agents as savings compound with agent steps, offering practical deployment guidance for cost-effective LLM agent operation.
...
TL;DR: IntentCUA is a multi-agent framework for computer-use automation that uses intent-aligned plan memory to stabilize long-horizon execution, achieving 74.83% task success rate with 0.91 step efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing computer-use automation approaches (RL-based planners, trajectory retrieval) suffer from drift from user intent, repeated solving of routine subproblems, error accumulation, and inefficiency in long-horizon tasks with noisy perception and evolving environment states.
...
TL;DR: RFEval benchmark reveals 49.7% unfaithful reasoning in Large Reasoning Models, showing accuracy is insufficient proxy for faithfulness; RL-style objectives can reduce faithfulness even when maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Large Reasoning Models often produce plausible-sounding rationales that don't reflect their true decision process, undermining reliability and trust in AI systems. Current evaluation focuses on accuracy but neglects the structural integrity of reasoning processes.

Method: Introduces formal framework with two testable conditions: stance consistency (coherent stance linking reasoning to answer) and causal influence (reasoning causally drives answer under output-level interventions). Presents RFEval benchmark with 7,186 instances across seven tasks using controlled, output-level counterfactual interventions to probe faithfulness.

Result: Evaluating twelve open-source LRMs reveals unfaithfulness in 49.7% of outputs, predominantly from stance inconsistency. Failures concentrate in brittle, convergent domains (math, code). Post-training regimes correlate more with faithfulness than scale: RL-style objectives on top of supervised fine-tuning can reduce faithfulness even when accuracy maintained. Accuracy is neither sufficient nor reliable proxy for faithfulness.

Conclusion: Establishes rigorous methodology for auditing LRM reliability; trustworthy AI requires optimizing not only for correct outcomes but also for structural integrity of reasoning process. RFEval provides benchmark for evaluating reasoning faithfulness.
...
TL;DR: S2Q learns multiple sub-value functions to retain alternative high-value actions, enabling better adaptation to changing value functions and improved performance in cooperative MARL.
...
Method: Proposes Successive Sub-value Q-learning (S2Q) which learns multiple sub-value functions to retain alternative high-value actions, incorporating them into a Softmax-based behavior policy to encourage persistent exploration and enable quick adjustment to changing optima.
...
Conclusion: S2Q addresses limitations of existing value decomposition methods by learning multiple sub-value functions, enabling better exploration and adaptation to changing value functions in cooperative MARL.
...
<details>
  <summary>Details</summary>
Motivation: Existing curriculum learning approaches require predefined difficulty metrics, while hard example mining methods need expensive per-sample loss tracking. There's a need for an efficient way to prioritize difficult samples during training without significant computational cost.

Method: Predictive Batch Scheduling (PBS) uses a lightweight linear predictor trained online to estimate sample difficulty from static token-level features: token frequency, sequence length, vocabulary diversity, and rare token ratio. The predictor is updated during training to improve correlation with actual loss.

Result: PBS achieves 6-13% faster convergence measured by evaluation loss across training checkpoints. The predictor achieves 0.44 correlation with actual loss using only four features, improving from 0.14 to 0.44 over 10,000 training steps. Experiments were conducted on a 130M parameter transformer.

Conclusion: Token frequency statistics encode meaningful information about sample difficulty, enabling effective curriculum learning with negligible computational overhead. PBS provides an efficient alternative to traditional curriculum learning and hard example mining methods.
...
TL;DR: Empirical analysis of five AI coding agents' pull requests reveals distinct PR description characteristics that influence human reviewer engagement, response timing, and merge outcomes in collaborative software development.
...
Method: Conducted an empirical analysis of pull requests created by five AI coding agents using the AIDev dataset, analyzing agent differences in PR description characteristics (including structural features) and examining human reviewer response in terms of review activity, response timing, sentiment, and merge outcomes.

Result: AI coding agents exhibit distinct PR description styles associated with differences in reviewer engagement, response time, and merge outcomes. Notable variation across agents was observed in both reviewer interaction metrics and merge rates.

Conclusion: Findings highlight the role of pull request presentation and reviewer interaction dynamics in human-AI collaborative software development, suggesting that how AI agents present their work significantly impacts human collaboration outcomes.
...
<details>
  <summary>Details</summary>
Motivation: 6G wireless systems face growing functional complexity and diverse service demands that require moving beyond rule-based control to intent-driven autonomous intelligence. User requirements now involve multi-dimensional objectives (latency sensitivity, energy preference, computational constraints, service-level requirements) that change dynamically due to environmental factors and user-network interactions, necessitating accurate understanding of both communication environment and user intent.

Method: The paper investigates agentic AI for 6G physical layer through a closed-loop pipeline of intent perception, autonomous decision making, and network execution. It reviews physical-layer tasks, identifies application scenarios for agentic AI, discusses challenges in multimodal perception, cross-layer decision making, and sustainable optimization, and presents AgenCom - an intent-driven link decision agent that adaptively constructs communication links based on user preferences and channel conditions.

Result: The paper presents AgenCom as a case study demonstrating how LLM-based agents can translate natural-language intents into executable control decisions, adaptively constructing communication links under diverse user preferences and channel conditions. This shows the feasibility of intent-driven autonomous control in 6G physical layer communications.

Conclusion: LLM-based agentic AI provides a promising foundation for intent-aware network agents in 6G systems, enabling the translation of natural-language intents into executable control decisions and supporting autonomous, sustainable evolution of wireless communications through multimodal perception and cross-layer decision making.
...
Result: The framework enables scalable and comparable assessment of sustainability rating methodologies, creating trustworthy benchmark datasets for evaluating rating approaches.

Conclusion: Calls on the AI community to adopt AI-powered approaches to strengthen sustainability rating methodologies that support urgent sustainability agendas through harmonized, comparable rating assessments.
...
<details>
  <summary>Details</summary>
Motivation: Standard Shapley value methods assume feature independence, which breaks down in vision tasks where pixels have spatial and semantic dependencies. While Owen value (hierarchical Shapley) addresses this, its effectiveness depends on feature group definitions, and current segmentation approaches violate key consistency properties.

Method: Proposes a new segmentation approach that satisfies the T-property to ensure semantic alignment across hierarchy levels. This enables computational pruning while maintaining attribution accuracy. The method leverages hierarchical group structures for Owen value calculations in vision tasks.

Result: O-Shap outperforms baseline SHAP variants in attribution precision, semantic coherence, and runtime efficiency on image and tabular datasets, particularly when structure matters. The T-property compliant segmentation improves consistency and interpretability.

Conclusion: Proper hierarchical segmentation satisfying the T-property is crucial for effective Owen value-based feature attribution. O-Shap demonstrates that semantically aligned hierarchies enable both computational efficiency and improved attribution quality in vision tasks with dependent features.
...
TL;DR: InstructKG is a framework that automatically constructs instructor-aligned knowledge graphs from lecture materials to capture learning dependencies and progressions for personalized education.


<details>
  <summary>Details</summary>
Motivation: Current approaches for capturing conceptual dependencies in education are either too surface-level (focusing on course-level concepts) or ignore pedagogical signals in instructional materials. There's a need to automatically identify learning progressions and knowledge gaps at scale, especially in large courses where individual diagnosis is infeasible.

Method: InstructKG extracts significant concepts as nodes and infers learning dependencies as directed edges (e.g., "part-of" or "depends-on" relationships) from lecture materials (slides, notes). It synergizes temporal and semantic signals from educational materials (teaching order, concept definitions) with the generalizability of large language models.
...
Conclusion: InstructKG provides an effective framework for automatically constructing knowledge graphs that capture intended learning progressions, enabling better identification of knowledge gaps and targeted interventions for personalized learning at scale.
...
<details>
  <summary>Details</summary>
Motivation: Generative AI operates through opaque mechanisms that challenge traditional understandings of knowledge production. Current engineering approaches lack the necessary epistemic foundation for responsible integration into science, education, and institutions, requiring a paradigmatic break from the Turing-Shannon-von Neumann tradition.
...
Result: Proposes "navigational knowledge" as a third mode of knowledge production that emerges from navigating high-dimensional semantic spaces, distinct from both symbolic reasoning and statistical recombination. This framework provides the missing epistemic account needed for understanding generative AI's mechanisms.

Conclusion: Generative AI requires a fundamental epistemological shift from the symbolic paradigm to a geometric-semantic paradigm based on high-dimensional spaces. The proposed Indexical Epistemology provides the principled basis needed for responsible integration of generative AI into knowledge-producing institutions.
...
TL;DR: A parallel algorithm for decomposing hard CircuitSAT instances using specialized constraints to partition SAT instances into weakened formulas, with parameter tuning for efficient decomposition guided by parallel hardness estimation.


<details>
  <summary>Details</summary>
Motivation: Hard CircuitSAT instances are computationally challenging to solve directly. There's a need for efficient decomposition techniques that can break down these difficult problems into more manageable subproblems while maintaining solution quality.

Method: A parameterized parallel algorithm that uses specialized constraints to partition original SAT instances into a family of weakened formulas. The approach adjusts parameters to efficiently identify high-quality decompositions, guided by hardness estimations computed in parallel.
...
Conclusion: The proposed parallel decomposition technique provides an effective approach for tackling hard CircuitSAT problems through intelligent partitioning and parallel hardness estimation, showing promise for practical applications in circuit verification and cryptography.
...
<details>
  <summary>Details</summary>
Motivation: As CNNs grow larger for better accuracy, they face increasing issues with size, execution time, memory footprint, and power consumption. Existing pruning solutions lack common implementation standards, making them difficult to implement and compare.

Method: Introduces Combine, a criterion-based pruning framework that enables iterative pruning with various criteria functions. The framework creates a standard language for comparing pruning criteria and includes novel criterion functions for weight removal decisions.

Result: The framework successfully prunes VGG-inspired models, removing up to 79% of filters while retaining or improving accuracy. It reduces network computations by up to 68%, demonstrating that different criteria have varying effects on different models.

Conclusion: Combine provides an effective, fast framework for CNN pruning with standardized criteria comparison, enabling significant model compression (up to 79% filter reduction) while maintaining or improving performance, addressing key efficiency challenges in modern CNNs.
...
<details>
  <summary>Details</summary>
Motivation: Current Genomic Foundation Models (GFMs) using Masked Language Modeling or Next Token Prediction excel at capturing local genomic syntax and motif patterns but fail to capture broader functional context, resulting in representations lacking global biological perspective.
...
Result: Evaluations across diverse genomic benchmarks demonstrate JEPA-DNA consistently yields superior performance in both supervised and zero-shot tasks compared to generative-only baselines.

Conclusion: JEPA-DNA provides more robust and biologically grounded representations, offering a scalable path toward foundation models that understand both genomic alphabet and underlying functional logic of sequences.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to create a highly efficient formula recognition model that maintains state-of-the-art performance while being significantly smaller and faster, enabling practical deployment on consumer-grade hardware and in-browser applications.

Method: Texo uses attentive design, distillation techniques, and transfer of vocabulary and tokenizer from larger models to create a minimalist architecture with only 20 million parameters while preserving recognition capabilities.
...
Conclusion: Texo demonstrates that minimalist formula recognition models can achieve competitive performance with significantly reduced computational requirements, making advanced mathematical formula recognition accessible for real-time applications and browser-based deployment.
...
TL;DR: A framework for constructing symbolic causal world models online using Meta-Interpretive Learning and predicate invention, achieving high sample efficiency and scalability in complex relational domains.


<details>
  <summary>Details</summary>
Motivation: Standard world modelling methods suffer from sample inefficiency, lack of transparency, and poor scalability, making it difficult for agents to internalize the underlying logic of complex environments.

Method: Integrates continuous model learning and repair into the agent's decision loop using Meta-Interpretive Learning and predicate invention to find semantically meaningful, reusable abstractions, constructing a hierarchy of disentangled concepts from observations.

Result: The lifted inference approach scales to domains with complex relational dynamics where propositional methods suffer combinatorial explosion, achieving sample-efficiency orders of magnitude higher than PPO neural-network baselines.

Conclusion: The proposed framework enables agents to construct symbolic causal world models entirely online, addressing key limitations of existing methods through semantic abstraction and relational reasoning.
...
TL;DR: Proposes an AI Agent-based collaborative workflow for humanities/social sciences research, validated using Taiwan's Claude.ai usage data, identifying three human-AI collaboration modes while emphasizing human judgment's irreplaceability.


<details>
  <summary>Details</summary>
Motivation: Addresses the research gap where generative AI studies focus predominantly on software engineering and natural sciences, with limited methodological exploration for humanities and social sciences, necessitating a tailored collaborative framework.
...
Result: Proposes a replicable AI collaboration framework, identifies three operational modes of human-AI collaboration (direct execution, iterative refinement, human-led), and demonstrates workflow feasibility through empirical analysis of Taiwan AI usage patterns.
...
TL;DR: LBM (Large Behavioral Model) is a behavioral foundation model fine-tuned to predict individual strategic choices by mapping structured psychological trait profiles to actions, outperforming prompting-based LLMs in high-stakes decision prediction.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with consistent, individual-specific behavior prediction in high-stakes environments, particularly when complex interactions between psychological traits and situational constraints are involved. Prompting-based approaches suffer from identity drift and limited ability to leverage detailed persona descriptions.
...
Result: LBM fine-tuning improves behavioral prediction relative to unadapted Llama-3.1-8B-Instruct backbone and performs comparably to frontier baselines when conditioned on Big Five traits. Unlike prompting-based baselines that exhibit complexity ceilings, LBM continues to benefit from increasingly dense trait profiles.

Conclusion: LBM establishes a scalable approach for high-fidelity behavioral simulation, enabling applications in strategic foresight, negotiation analysis, cognitive security, and decision support by effectively mapping psychological profiles to discrete actions across diverse strategic dilemmas.
...
<details>
  <summary>Details</summary>
Motivation: To develop evaluation frameworks that go beyond surface-level metrics by investigating how LLMs internally represent cognitive complexity, addressing the black-box nature of these models.

Method: Analyzed high-dimensional activation vectors from different LLMs using Bloom's Taxonomy as a hierarchical lens, probing whether different cognitive levels (Remember to Create) are linearly separable within residual streams using linear classifiers.

Result: Linear classifiers achieved approximately 95% mean accuracy across all Bloom levels, demonstrating that cognitive level is encoded in a linearly accessible subspace of model representations, with representations becoming increasingly separable across layers.

Conclusion: LLMs resolve cognitive difficulty early in the forward pass, with cognitive complexity levels encoded in linearly separable neural representations, providing insights into the internal organization of cognitive processing in these models.
...
TL;DR: The paper introduces Shapley-DCLR, a metric to quantify temporal knowledge leakage in LLMs during backtesting, and TimeSPEC, a method to filter leaked information by verifying claims against pre-cutoff sources.


<details>
  <summary>Details</summary>
Motivation: LLMs may inadvertently use post-cutoff knowledge encoded during training when predicting past events, undermining the validity of retrospective evaluation (backtesting). Current methods lack interpretable ways to detect and quantify this temporal knowledge leakage.

Method: 1) Claim-level framework: Decompose model rationales into atomic claims and categorize by temporal verifiability. 2) Shapley-DCLR metric: Apply Shapley values to measure each claim's contribution to prediction, quantifying decision-critical leakage rate. 3) TimeSPEC method: Interleave generation with claim verification and regeneration to filter temporal contamination, ensuring all supporting claims trace to pre-cutoff sources.

Result: Experiments on 350 instances across three domains (U.S. Supreme Court cases, NBA salary estimation, stock return ranking) show substantial leakage in standard prompting baselines. TimeSPEC reduces Shapley-DCLR while preserving task performance, outperforming prompt-based temporal constraints.

Conclusion: Explicit, interpretable claim-level verification via TimeSPEC effectively mitigates temporal knowledge leakage for reliable backtesting, demonstrating superiority over simple prompt-based approaches for ensuring temporally valid predictions.
...
TL;DR: Web Verbs proposes a semantic layer for web actions using typed, documented functions that unify API-based and browser-based paradigms, enabling LLMs to synthesize reliable, efficient, and verifiable workflows.


<details>
  <summary>Details</summary>
Motivation: The web is evolving from human browsing to agentic environments, but current web agents operate on low-level primitives (clicks, keystrokes) that are brittle, inefficient, and difficult to verify. There's a need for a semantic layer for web actions to complement content-oriented semantic layers.

Method: Proposes Web Verbs - a web-scale set of typed, semantically documented functions that expose site capabilities through a uniform interface. These verbs serve as stable, composable units that agents can discover, select, and synthesize into concise programs, unifying API-based and browser-based paradigms.

Result: The approach improves reliability through stable interfaces, efficiency by reducing dozens of steps into few function calls, and verifiability through typed contracts and checkable traces. A proof-of-concept implementation and case studies demonstrate concise and robust execution compared to existing agents.

Conclusion: Web Verbs provides a semantic abstraction layer for web actions that enables LLMs to synthesize reliable, auditable workflows with explicit control and data flow. The paper outlines a roadmap for standardization to make verbs deployable and trustworthy at web scale.
...
<details>
  <summary>Details</summary>
Motivation: Despite strong reasoning capabilities in frontier LLMs, the practical process of training domain-specialized scientific language models from raw sources remains under-documented, particularly for researchers with moderate compute budgets.

Method: End-to-end pipeline covering metadata filtering, archive validation, LaTeX extraction, text normalization, domain-aware tokenization, and dense transformer training on 2xA100 GPUs. Conducted 24 experimental runs to analyze training stability, scaling behavior, and infrastructure bottlenecks.
...
Conclusion: Provides an engineering-grounded, transparent account of training a small scientific language model from scratch, offering insights to support researchers operating under moderate compute budgets seeking to build domain-specialized models.
...
TL;DR: MedClarify is an AI agent that improves medical LLM diagnostic performance by generating uncertainty-aware follow-up questions to reduce diagnostic errors by ~27 percentage points compared to single-shot LLM baselines.


<details>
  <summary>Details</summary>
Motivation: Current medical LLMs struggle with diagnostic reasoning when patient cases are incomplete, often yielding multiple similarly likely diagnoses without systematic follow-up questioning, unlike real clinicians who iteratively gather information through differential diagnosis reasoning.

Method: MedClarify computes candidate diagnoses (differential diagnosis), proactively generates follow-up questions aimed at reducing diagnostic uncertainty, and selects questions with highest expected information gain for targeted, uncertainty-aware reasoning.

Result: The information-theoretic reasoning approach reduces diagnostic errors by ~27 percentage points compared to standard single-shot LLM baselines, demonstrating effective follow-up questioning generation.
...
TL;DR: A dataless regularization method using curvature matrix approximation (Kronecker-Factored Approximate Curvature) to mitigate representation drift in task arithmetic without requiring external task data.
...
Conclusion: Proposed dataless curvature-based regularization effectively mitigates representation drift in task arithmetic while maintaining modularity and addressing data availability constraints.
...
TL;DR: A framework integrating formal verification with deep learning for image retrieval that supports complex natural language queries while providing transparent, verifiable results.


<details>
  <summary>Details</summary>
Motivation: Current embedding-based retrieval systems struggle with complex queries involving relationships, object compositions, and precise constraints (identities, counts, proportions), lacking reliability and transparency.

Method: Integrates formal verification into deep learning through graph-based verification methods and neural code generation, grounding retrieval results in formal reasoning to verify each atomic truth in user queries.

Result: Produces trustworthy, verifiable results that identify which specific constraints are satisfied/unmet, moving beyond vector representation ambiguity while boosting performance of existing embedding-based approaches.

Conclusion: The framework offers a more transparent and accountable retrieval process by combining formal verification with neural methods, addressing limitations of current approaches for complex natural language queries.
...
TL;DR: MCVAE: A multimodal contrastive variational autoencoder for NSCLC survival prediction that handles missing modalities through stochastic masking, learned gating, and multi-task learning with cross-modal alignment.


<details>
  <summary>Details</summary>
Motivation: NSCLC survival prediction is challenging due to individual variability and benefits from multimodal integration (WSI, transcriptomics, methylation). Real-world clinical datasets often have missing modalities, and current models lack robustness to severe missingness.

Method: Multimodal Contrastive Variational AutoEncoder (MCVAE) with modality-specific variational encoders, fusion bottleneck with learned gating mechanisms, multi-task objective (survival loss + reconstruction loss), cross-modal contrastive loss for latent space alignment, and stochastic modality masking during training.

Result: Extensive evaluations on TCGA-LUAD (n=475) and TCGA-LUSC (n=446) demonstrate efficacy in predicting disease-specific survival (DSS) and robustness to severe missingness compared to two state-of-the-art models. Found that multimodal integration is not always beneficial across all modality subsets.

Conclusion: MCVAE effectively addresses missing modality challenges in NSCLC survival prediction through robust multimodal integration with uncertainty modeling, learned gating, and cross-modal alignment. The study provides insights on when multimodal integration is beneficial.
...
TL;DR: A Privacy-by-Design framework integrating multiple privacy regulations (GDPR, PIPEDA, COPPA) and child rights principles (UNCRC, AADC) to guide development of AI applications for children, with LLM-based case study demonstration.


<details>
  <summary>Details</summary>
Motivation: Children increasingly use AI technologies, raising privacy concerns. Existing privacy regulations require protections but implementation is challenging in practice, necessitating a structured framework for developers.

Method: Proposed Privacy-by-Design framework maps privacy principles from GDPR, PIPEDA, and COPPA to LLM application stages (data collection, model training, operational monitoring, validation). Incorporates design guidelines from UNCRC, AADC, and academic research, demonstrated through case study of LLM-based educational tutor for children under 13.

Result: Framework shows how technical/organizational controls and age-appropriate design decisions throughout LLM lifecycle can reduce privacy risks while meeting legal standards, supporting development of compliant AI applications for children.

Conclusion: Privacy-by-Design framework enables proactive, risk-averse approach to AI development for children, integrating legal requirements with practical implementation strategies to balance innovation with privacy protection.
...
TL;DR: WarpRec is a high-performance recommender systems framework that bridges academia-industry gaps with backend-agnostic architecture, enabling seamless local-to-distributed transitions while integrating sustainability metrics and preparing for agentic AI evolution.
...
Method: WarpRec uses a novel backend-agnostic architecture that includes 50+ state-of-the-art algorithms, 40 metrics, and 19 filtering/splitting strategies, with seamless transition from local execution to distributed training. It integrates CodeCarbon for real-time energy tracking and anticipates agentic AI evolution.

Result: The framework eliminates the trade-off between experimentation ease and industrial scalability, demonstrates that scalability doesn't compromise scientific integrity or sustainability, and provides architectural foundation for next-generation sustainable, agent-ready recommender systems.

Conclusion: WarpRec bridges academia-industry gaps and serves as architectural backbone for sustainable, agent-ready recommender systems, enabling innovation while addressing ecological responsibility and preparing for the shift toward agentic AI within the Generative AI ecosystem.
...
<details>
  <summary>Details</summary>
Motivation: To provide a systematic approach for evaluating AI model performance on ARM Cortex processors in embedded systems, addressing the need for energy-efficient AI deployment while balancing computational demands and accuracy requirements.

Method: Design of an automated test bench for systematic evaluation across key performance indicators (KPIs), using Pareto analysis to balance trade-offs between energy consumption and model accuracy, and analyzing correlation between FLOPs and inference time.

Result: Identified near-linear correlation between FLOPs and inference time; M7 processor ideal for short inference cycles; M4 offers better energy efficiency for longer inference tasks; M0+ suitable for simpler tasks despite lower efficiency for complex models.

Conclusion: Provides practical guidance for developers to design energy-efficient AI systems on ARM Cortex processors, with processor-specific recommendations for balancing performance, accuracy, and energy consumption in real-world embedded applications.
...
TL;DR: KG-RAG integrates knowledge graphs with retrieval-augmented generation to enhance LLMs for telecom-specific tasks, reducing hallucinations and improving factual accuracy by 14.3% over standard RAG and 21.6% over LLM-only models.
...
Method: KG-RAG framework integrates knowledge graphs (structured representation of telecom domain knowledge from standards/technical documents) with retrieval-augmented generation (dynamic retrieval of relevant facts to ground model outputs).
...
Result: Results show that reusability and verifiability do not correlate with standard accuracy, revealing a blind spot in current accuracy-based leaderboards for reasoning capability. Surprisingly, CoTs from specialized reasoning models are not consistently more reusable or verifiable than those from general-purpose LLMs like Llama and Gemma.

Conclusion: The paper demonstrates the importance of evaluating reasoning quality beyond just accuracy, introducing reusability and verifiability as complementary metrics that reveal limitations in current CoT evaluation approaches and challenge assumptions about specialized reasoning models.
...
<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of training LLM agents for extremely long-horizon tasks, which require maintaining context and reasoning over extended sequences beyond typical LLM capabilities.

Method: Two-stage approach: 1) Cold-start via trajectory-splitting SFT that preserves early context, truncates later context, and maintains overlap between sub-trajectories; 2) Progressive RL training with multiple stages of progressively extended timeouts. Uses Research-Factory pipeline for automated high-quality training data generation from research papers with evaluation rubrics.

Result: KLong (106B) surpasses Kimi K2 Thinking (1T) by 11.28% on PaperBench, with performance improvements generalizing to SWE-bench Verified and MLE-bench coding benchmarks.

Conclusion: The proposed trajectory-splitting SFT and progressive RL training enable effective learning of extremely long-horizon tasks, demonstrating superior performance and generalization capabilities compared to larger models.
...
Method: Proposes ODE-based theoretical framework where activation addition is first-order ODE approximation; identifies steering directions via barrier functions defined as log-density ratio between positive/negative activations; implements ODESteer for multi-step adaptive steering.

Result: ODESteer achieves consistent improvements over state-of-the-art methods: 5.7% improvement on TruthfulQA, 2.5% on UltraFeedback, and 2.4% on RealToxicityPrompts across diverse LLM alignment benchmarks.

Conclusion: Establishes principled ODE-based theoretical foundation for activation steering, validated empirically by ODESteer's superior performance, providing unified framework for designing more effective LLM alignment methods.
...
TL;DR: A hybrid federated learning approach combining SWIN Transformer and CNN models (DenseNet201, Inception V3, VGG19) for COVID-19 and pneumonia diagnosis from X-ray images, enabling secure distributed medical data processing.


<details>
  <summary>Details</summary>
Motivation: To leverage computational advancements and AI for healthcare applications while addressing data privacy concerns through federated learning, creating a secure distributed system for medical data processing that helps physicians with reliable disease diagnosis.

Method: Hybrid ensemble approach combining SWIN Transformer (Microsoft's Vision Transformer) with CNN models (DenseNet201, Inception V3, VGG19) using TensorFlow/Keras, implemented within a federated learning framework for secure distributed training on medical X-ray data.

Result: The proposed system enables detection of COVID-19 and pneumonia from X-ray reports with improved accuracy through real-time continual learning while ensuring data security and authenticity via federated learning integration.

Conclusion: The federated learning-based hybrid AI model provides a reliable, secure solution for medical diagnosis that enhances disease detection accuracy while preserving patient data privacy, serving as a valuable tool for physicians in pandemic response.
...
TL;DR: The paper proposes evaluating AI general intelligence through performance on all conceivable human games, introduces the AI GameStore platform for scalable game generation, and shows current VLMs perform poorly (under 10% of human average) on most games.


<details>
  <summary>Details</summary>
Motivation: Current AI benchmarks are too narrow and quickly saturate; there's a need for better evaluation of human-like general intelligence across the full spectrum of human capabilities.

Method: Proposes evaluating AI through performance on all human games (Multiverse of Human Games). Introduces AI GameStore platform using LLMs with humans-in-the-loop to synthesize new games by sourcing/adapting variants from popular gaming platforms (Apple App Store, Steam). Generated 100 games as proof of concept.

Result: Evaluated 7 frontier vision-language models on short gameplay episodes. Best models achieved less than 10% of human average score on majority of games. Models particularly struggled with games requiring world-model learning, memory, and planning.

Conclusion: The AI GameStore provides a practical way to measure and drive progress toward human-like general intelligence. Current AI systems are far from human-level performance across the full spectrum of human games, highlighting gaps in world-model learning and planning capabilities.
...
TL;DR: MolHIT introduces a hierarchical discrete diffusion model for molecular graph generation that achieves near-perfect chemical validity and SOTA performance on MOSES dataset, surpassing 1D baselines.
...
Method: Hierarchical Discrete Diffusion Model that generalizes discrete diffusion to additional categories encoding chemical priors, with decoupled atom encoding that splits atom types according to their chemical roles.

Result: Achieves new state-of-the-art performance on MOSES dataset with near-perfect validity for the first time in graph diffusion, surpassing strong 1D baselines across multiple metrics. Also demonstrates strong performance in downstream tasks including multi-property guided generation and scaffold extension.

Conclusion: MolHIT overcomes long-standing performance limitations in existing molecular graph generation methods and represents a powerful framework for AI-driven drug discovery and materials science.
...
<details>
  <summary>Details</summary>
Motivation: Traditional PDE solver design requires substantial mathematical expertise and manual tuning, while recent neural network approaches offer flexibility but suffer from high computational costs and limited interpretability. There's a need for accessible, automated PDE solving that maintains transparency.

Method: Multi-agent framework with coarse-to-fine execution strategy and residual-based self-verification mechanism. The system autonomously designs, implements, debugs, and verifies numerical solvers for general PDEs from natural language descriptions, generating transparent solvers based on classical numerical analysis principles.

Result: Experiments on 24 canonical and real-world PDE problems demonstrate competitive or superior accuracy compared to existing neural and LLM-based baselines. The framework correctly selects numerical schemes based on PDE structural properties.

Conclusion: AutoNumerics suggests viability as an accessible paradigm for automated PDE solving, offering transparent solver generation grounded in classical numerical analysis rather than black-box approaches.
...
TL;DR: HIPE-2026 is a CLEF evaluation lab focused on extracting person-place relations from historical texts, extending previous campaigns to include semantic relation extraction across languages and time periods.


<details>
  <summary>Details</summary>
Motivation: To advance historical text processing by moving beyond named entity recognition to semantic relation extraction, enabling downstream applications in knowledge-graph construction, historical biography reconstruction, and spatial analysis in digital humanities.

Method: Introduces a three-fold evaluation profile assessing accuracy, computational efficiency, and domain generalization for systems classifying two relation types: $at$ (historical presence) and $isAt$ (temporal location around publication time).

Result: The lab extends the HIPE series toward semantic relation extraction, requiring systems to reason over temporal and geographical cues in multilingual historical texts across different time periods.

Conclusion: HIPE-2026 aims to bridge relation extraction with large-scale historical data processing, supporting digital humanities applications through standardized evaluation of person-place association extraction from noisy historical sources.
...
<details>
  <summary>Details</summary>
Motivation: To enable autonomous forklifts to unload pallets on inclined surfaces without dragging the pallets during fork withdrawal, which is a common problem in real-world logistics scenarios like truck unloading.

Method: Uses Iterative Closest Point (ICP) algorithm on point clouds from the upper region of the pallet to track relative position and attitude angle difference between pallet and fork in real-time. The fork is aligned parallel to the target surface based on tracking results, then withdrawn along the tilt direction.

Result: Effectiveness verified through dynamic simulations and experiments using a real forklift replicating unloading operations onto inclined truck beds, demonstrating successful drag-free unloading.

Conclusion: The proposed ICP-based control method enables autonomous forklifts to successfully unload pallets on inclined surfaces without dragging, addressing a practical challenge in logistics automation.
...
<details>
  <summary>Details</summary>
Motivation: To develop an efficient and accurate trajectory generation method for parallel kinematic milling robots that handles decoupled position and orientation data while ensuring smooth motion and computational efficiency suitable for low-cost microcontrollers.

Method: Integrates B-spline and Quaternion interpolation for decoupled position/orientation data. Uses smooth piece-wise Bezier curves to synchronize orientation with arc-length-parameterized position via sequential quadratic programming. Employs unit quaternions for orientation interpolation (avoiding gimbal lock) and modifier polynomials for position interpolation. Optimizes temporal trajectories using minimum jerk, time-optimal piece-wise Bezier curves in two stages: task space followed by joint space.
...
Conclusion: The proposed method provides an effective solution for smooth trajectory generation in parallel kinematic milling robots, offering improved performance and computational efficiency suitable for implementation on low-cost hardware.
...
<details>
  <summary>Details</summary>
Motivation: Traditional sampling-based planners with STL specifications rely on min-max robustness measures that create non-smooth optimization landscapes with sharp decision boundaries, hindering efficient tree exploration and performance in multi-constraint scenarios with limited guidance signals.

Method: Proposes RRT$^η$ framework with three key contributions: (1) AGM robustness interval semantics for partial trajectory reasoning, (2) efficient incremental monitoring algorithm for computing these intervals, and (3) enhanced Direction of Increasing Satisfaction vectors using Fulfillment Priority Logic for principled objective composition.
...
Conclusion: RRT$^η$ effectively addresses limitations of traditional min-max robustness measures by providing smoother optimization landscapes through AGM integration, enabling more efficient exploration and better performance in complex multi-constraint robotic planning scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Distributed tactile sensing is difficult to scale over large areas due to wiring complexity, cost, fragility of dense sensor arrays, and limited coverage/response time of alternatives.

Method: Uses vibrating tensioned strings as sensing elements with continuous electromagnetic excitation and contact microphones to observe spectral changes from contact. Includes physics-based string-vibration simulator to predict contact effects and real-time inference pipeline mapping vibrations to contact state.

Result: Demonstrates millimeter-scale localization, reliable force estimation, and real-time slip detection. Provides scalable hardware concept, physics-grounded simulation tool, and real-time inference pipeline.

Conclusion: Sound of Touch offers a lightweight, scalable string-based tactile sensing approach for instrumenting extended robot surfaces with accurate contact state estimation capabilities.
...
TL;DR: The paper proposes a "Walk-Along with Robots" (WawR) methodology for studying autonomous robots in public spaces, drawing from public realm ethnography to address limitations of controlled experiments and structured observations.


<details>
  <summary>Details</summary>
Motivation: As autonomous robots increasingly operate in public spaces (campus walkways, neighborhood sidewalks), traditional HRI field studies are limited to controlled experiments with prototype robots or structured methods like Wizard of Oz. Current autonomous mobile robots (particularly delivery robots) operate beyond researcher control, navigating dynamic routes and unpredictable environments, requiring a more deliberate research approach.

Method: Proposes the Walk-Along with Robots (WawR) methodology, drawing inspiration from public realm ethnography in urban studies, geography, and sociology. Outlines key features of the method, steps applied in their study, unique insights it offers, and ways it can be evaluated.

Result: The paper presents the WawR methodology as a solution for studying autonomous robots in public spaces, offering a framework for researchers to adapt ethnographic approaches to robotics research in uncontrolled, dynamic environments.

Conclusion: The WawR methodology addresses the challenge of studying autonomous robots operating beyond researcher control in public spaces, and the paper aims to stimulate further discussion on research methodologies for studying autonomous robots in public realms.
...
TL;DR: SimToolReal: A sim-to-real RL approach for general dexterous tool manipulation using procedurally generated tool-like objects and universal goal training, achieving strong zero-shot performance across diverse real-world tools.
...
Method: Procedurally generate a large variety of tool-like object primitives in simulation and train a single RL policy with the universal goal of manipulating each object to random goal poses. This enables generalization without object or task-specific training.

Result: SimToolReal outperforms prior retargeting and fixed-grasp methods by 37% while matching specialist RL policies trained on specific target objects and tasks. Achieves strong zero-shot performance over 120 real-world rollouts spanning 24 tasks, 12 object instances, and 6 tool categories.

Conclusion: The approach demonstrates effective generalization of sim-to-real RL policies for tool manipulation, enabling zero-shot performance across diverse real-world tools without task-specific engineering.
...
TL;DR: Boreas-RT extends the Boreas dataset with 643 km of diverse driving data across 9 routes, featuring multi-modal sensors and centimeter-level ground truth for evaluating odometry and localization algorithms in challenging real-world conditions.


<details>
  <summary>Details</summary>
Motivation: To address the limitation that many state-of-the-art autonomous driving algorithms overfit to simple driving environments and degrade in more challenging conditions, by providing a diverse, multi-season dataset with varying traffic and weather conditions for robust evaluation.

Method: Collection of 60 sequences over 9 real-world routes totaling 643 km using a comprehensive sensor suite including 5MP camera, 360° Doppler radar, 128-channel lidar, FMCW Doppler lidar, IMU, and wheel encoders, with centimeter-level ground truth from post-processed GNSS-INS data.

Result: Benchmark results demonstrate that many state-of-the-art odometry and localization algorithms significantly degrade on the more challenging Boreas-RT routes, highlighting the dataset's value for evaluating algorithm robustness in diverse conditions.

Conclusion: Boreas-RT provides a unified, publicly available dataset with development kit and leaderboard that enables comprehensive evaluation of multi-modal autonomous driving algorithms across diverse and challenging real-world road conditions.
...
<details>
  <summary>Details</summary>
Motivation: Prior LLM-based task planning approaches are fragile in dynamic settings due to open-loop operation without robust environmental feedback, reliance on specialized models/fine-tuning, and lack of adaptive error recovery mechanisms.
...
Result: Experiments in simulation and real-world settings demonstrate improved generalization and increased success rates in zero-shot manipulation tasks compared to prior approaches.

Conclusion: Iterative closed-loop multi-agent coordination with specialized agents and targeted error recovery enables more robust robotic manipulation in dynamic environments without requiring model fine-tuning.
...
TL;DR: A graph-based approach for learning long-horizon manipulation tasks by representing scene states as object relationships, segmenting demonstrations into phases, and learning minimal task models from multiple demonstrations.


<details>
  <summary>Details</summary>
Motivation: Learning long-horizon manipulation tasks efficiently from demonstrations is challenging. Existing methods focus on learning actions directly, but this paper focuses on inferring what the robot should achieve rather than how, by representing evolving scene states through object relationships.

Method: Proposes demonstration segmentation and pooling to extract manipulation graphs representing object relationships across task phases. Uses pre-trained visual features for object matching across demonstrations. Captures complete object interactions from control onset to manipulation completion, unlike prior partial or short-window approaches.

Result: Evaluated demonstration segmentation accuracy and utility of learning from multiple demonstrations for finding minimal task models. Deployed fitted models in simulation and on real robot, showing task representations support reliable execution across environments.

Conclusion: The graph-based approach successfully learns long-horizon manipulation tasks by focusing on what to achieve rather than how, with robust multi-demonstration learning and reliable cross-environment execution.
...
TL;DR: A physics-based benchmark evaluates 6D pose estimators and 3D mesh models for robotic grasping, showing reconstruction artifacts reduce grasp candidates but have minimal impact on grasping performance with accurate pose estimation.


<details>
  <summary>Details</summary>
Motivation: Standard geometric evaluations of 3D reconstruction methods don't reflect how reconstruction quality affects downstream robotic manipulation tasks like grasping, creating a gap between perception system evaluation and functional performance.

Method: Introduces a large-scale, physics-based benchmark that evaluates 6D pose estimators and 3D mesh models by generating grasps on reconstructed meshes and executing them on ground-truth models, simulating real-world interaction with imperfect perception.

Result: Reconstruction artifacts significantly decrease the number of grasp pose candidates but have negligible effect on grasping performance when pose is accurately estimated. Spatial error dominates grasp success relationship, and simple translation error provides insight for symmetric objects.

Conclusion: The benchmark provides insights into how perception systems relate to object manipulation in robotics, revealing that accurate pose estimation is more critical than perfect geometric reconstruction for successful grasping.
...
TL;DR: A Conditional Flow Matching framework maps rigid gripper grasp poses to soft Fin-ray gripper poses, achieving significantly higher success rates than baseline rigid poses.


<details>
  <summary>Details</summary>
Motivation: There's a representation gap between grasp synthesis for rigid and soft grippers. Existing methods like Anygrasp are designed for rigid parallel grippers and fail to capture soft grippers' compliant behaviors, resulting in data-intensive and inaccurate models.

Method: Uses Conditional Flow Matching (CFM) to learn transformation from rigid to soft gripper poses. Includes data collection pipeline for paired rigid-soft grasp poses. A U-Net autoencoder conditions CFM on object geometry from depth images to learn continuous mapping from Anygrasp poses to stable Fin-ray gripper poses.

Result: CFM-generated poses achieve 34% success rate for seen objects and 46% for unseen objects, compared to baseline rigid poses (6% and 25%). Significant improvements for cylindrical (50% and 100%) and spherical objects (25% and 31%). Successfully generalizes to unseen objects.
...
TL;DR: AR-based teleoperation framework for hybrid rigid-soft robots using simulation overlay and real-to-sim parameter identification for consistent virtual-physical behavior.


<details>
  <summary>Details</summary>
Motivation: Hybrid rigid-soft robots offer precision and compliance for unstructured environments, but coordination challenges arise from modeling difficulties, perception issues, and cross-domain kinematics.

Method: AR-based physical human-robot interaction framework using AR headset to overlay simulated robot model on real system, with real-to-simulation parameter identification pipeline leveraging soft robot's geometric properties for accurate modeling.

Result: Enables direct teleoperation of hybrid rigid-soft robot for simple reaching and grasping tasks with consistent behavior between virtual and physical systems.

Conclusion: The AR framework with parameter identification addresses coordination challenges in hybrid rigid-soft robots, facilitating intuitive teleoperation for grasping tasks in unstructured environments.
...
TL;DR: Coordinate-free inverse flight dynamics formulation for fixed-wing aircraft on SO(3) with closed-form trajectory-to-input mapping, applied to tethered flight with analytic bank angle expressions and minimal-thrust solutions.


<details>
  <summary>Details</summary>
Motivation: Bridge inverse simulation in aeronautics with geometric modeling in robotics by developing a rigorous coordinate-free formulation that avoids local attitude coordinates and provides analytical tools for trajectory design and feasibility analysis.
...
Result: Closed-form trajectory-to-input mapping; analytic expressions for required bank angle in tethered flight on spherical parallels; identification of zero-bank locus where tether tension balances centrifugal effects; closed-form minimal-thrust angle of attack under simple lift/drag law; pointwise quasi-steady inversion solutions that become steady-flight trim under time-invariant conditions.

Conclusion: The framework successfully bridges aeronautical inverse simulation with robotic geometric modeling, providing rigorous analytical building blocks for trajectory design, feasibility checking, and understanding the decoupling between aerodynamic coordination and apparent gravity in coordinated flight scenarios.
...
TL;DR: Framework for UAV cable manipulation combining high-fidelity PDE modeling with reduced-order representation for real-time control, using model predictive control to stabilize oscillations and handle hybrid transitions.


<details>
  <summary>Details</summary>
Motivation: Enable real-time, dynamics-aware control of UAVs carrying suspended flexible cables by addressing the computational complexity of high-fidelity models while maintaining accuracy for effective manipulation.

Method: Discretize cable PDEs using finite-difference method, apply proper orthogonal decomposition to extract reduced-order model (ROM), and formulate nonlinear model predictive control scheme based on ROM for stabilization and hybrid transition handling.

Result: Simulations confirm ROM stability, efficiency, and robustness, with controller effectively regulating cable dynamics under various conditions and enabling trajectory planning in constrained environments.

Conclusion: The framework successfully enables real-time control of UAV cable manipulation by balancing model fidelity with computational efficiency through reduced-order modeling and predictive control.
...
TL;DR: A multi-session framework for autonomous systems that uses map-based localization with topology-informed decision-making to selectively trigger mapping and loop closing, improving global consistency in repeated environment visits.
...
Conclusion: The proposed multi-session framework with topology-informed decision-making effectively addresses challenges of repeated environment exposure, improving mapping and localization consistency for autonomous systems operating in previously visited areas.
...
TL;DR: FRAPPE introduces a two-stage fine-tuning method for VLA models to improve world modeling by predicting future latent representations and aligning them with multiple visual foundation models, addressing pixel-level over-emphasis and error accumulation issues.


<details>
  <summary>Details</summary>
Motivation: Current VLA models for world modeling have two main issues: 1) training objectives force over-emphasis on pixel-level reconstruction, constraining semantic learning and generalization; 2) reliance on predicted future observations during inference leads to error accumulation.
...
Conclusion: FRAPPE provides a scalable and data-efficient pathway to enhance world-awareness in generalist robotic policies by addressing key limitations in current world modeling approaches for VLA models.
...
TL;DR: A proprioceptive state estimator for legged robots using only IMU and motor measurements, leveraging contacting legs as kinematic anchors with footfall position constraints to suppress drift, achieving accurate pose estimation without cameras or LiDAR.
...
Method: Treats contacting legs as kinematic anchors: uses joint-torque-based foot wrench estimation to select reliable contacts, with footfall positions providing world-frame constraints. Introduces height clustering and time-decay correction to prevent elevation drift. Applies inverse-kinematics cubature Kalman filter to improve foot velocity observations. Mitigates yaw drift through multi-contact geometric consistency and kinematics-derived heading reference.

Result: Evaluated on four quadruped platforms (three Astrall robots and Unitree Go2 EDU). Achieved 0.1638m horizontal and 0.219m vertical error on Astrall point-foot robot for ~200m/15m loops; 0.2264m/0.199m on wheel-legged robot B; 7.68m/0.540m on wheel-legged robot C for ~700m/20m loops; 2.2138m horizontal and <0.1m vertical error on Unitree Go2 EDU for ~120m/8m loops.

Conclusion: The proposed proprioceptive state estimator effectively suppresses long-term drift using only IMU and motor measurements, with a unified formulation applicable to various legged robot types. The method demonstrates practical accuracy for real-world deployment without external sensors.
...
TL;DR: Decentralized VMC framework enables safe human-robot collaboration using virtual springs/dampers instead of trajectory planning, with stall detection and negotiation to eliminate deadlocks.


<details>
  <summary>Details</summary>
Motivation: Need for safe, scalable human-robot collaboration systems that can handle multiple agents without explicit trajectory planning, prevent deadlocks, and maintain safety through intuitive control.

Method: Virtual Model Control (VMC) with both humans and robots in virtual-component-shaped workspace using springs/dampers; decentralized force-based stall detector identifies deadlocks resolved through negotiation; distributed implementation for scalability.

Result: Reduced deadlock probability from up to 61.2% to zero in block placement tasks; demonstrated safe collaboration with up to 2 robots + 2 humans (experiments) and 4 robots (simulation) while maintaining ~20 cm inter-agent separation; deadlock-free operation across team sizes.

Conclusion: The VMC-based framework provides effective, scalable, and safe human-robot collaboration through intuitive virtual force control, eliminating deadlocks via decentralized stall detection and negotiation while maintaining safety distances.
...
TL;DR: SOLen: A 3D printed soft optical sensing approach using a printed lens within a Y-shaped waveguide that converts deformation into differential optical signals for motion detection in soft robots.


<details>
  <summary>Details</summary>
Motivation: Additive manufacturing enables complex soft robot geometries, creating demand for sensing solutions compatible with single-material, one-step fabrication. Optical soft sensors are attractive but suffer from uncontrolled light propagation issues, while mitigation strategies typically require multimaterial interfaces.

Method: Developed SOLen approach with printed lens placed in front of an emitter within Y-shaped waveguide. Modified acrylate polyurethane resin with lauryl acrylate for improved compliance and optical transmittance. Used single-layer optical characterization to derive wavelength-dependent refractive index and transmittance. Designed lens profile via simulations using measured refractive index for target focal distance. Demonstrated rotational tests for branch-selective signal switching.

Result: Successfully printed lens with sub-millimeter fidelity. Rotational tests demonstrated reproducible branch-selective signal switching over multiple cycles. Established transferable material-to-optics workflow for soft optical sensors with lenses.

Conclusion: SOLen approach enables new functionalities for next-generation soft robots by providing a single-material, one-step fabrication compatible optical sensing solution that encodes both motion direction and amplitude through deformation-induced lens rotation and focal-spot translation.
...
TL;DR: A cost-effective hardware solution for rainy conditions that improves perception for automated vehicles, increasing pedestrian detection accuracy from 8.3% to 41.6% while supporting sustainability goals.


<details>
  <summary>Details</summary>
Motivation: Limited research on physical hardware solutions for adverse weather perception in automated vehicles, especially for critical applications like vehicle platooning. Existing approaches (hydrophilic/hydrophobic lenses, sprays) provide only partial mitigation, while industrial protection systems are costly and not scalable for automotive deployment.

Method: A cost-effective hardware solution designed for rainy conditions, compatible with multiple cameras simultaneously. The system works with existing camera-based sensing platforms without requiring additional high-cost sensors or hardware replacements.

Result: The proposed system increased pedestrian detection accuracy of a Deep Learning model from 8.3% to 41.6%. It extends operational reliability of automated vehicles in challenging weather conditions.

Conclusion: The hardware solution addresses the gap in physical protection systems for automated vehicle perception in rain, offering cost-effectiveness, scalability, and sustainability benefits by reducing resource consumption, supporting modular upgrades, and enabling more efficient deployment of automated vehicle technologies.
...
TL;DR: OS-ERA introduces optical waveguide sensors to Electro-Ribbon Actuators, enabling high-fidelity bending state classification and solving sensing limitations for closed-loop control.


<details>
  <summary>Details</summary>
Motivation: Electro-Ribbon Actuators (ERAs) have excellent displacement and speed but suffer from limited sensing precision using capacitive sensors, hindering accurate control. There's a need for reliable proprioceptive sensing without affecting actuation performance.

Method: Design and embed two soft optical waveguide sensors into ERA to analyze complex curvature. Train a classifier to map sensing signals to distinguish eight bending states. Validate model on held-out trials and compare signal trajectories against training manifold.

Result: OS-ERA achieves high-fidelity bending state classification with preserved signal trajectories despite actuation speed mismatches. The system demonstrates voltage- and speed-invariance, maintains repeatability, and follows training manifold across all tests.

Conclusion: OS-ERA solves the longstanding sensing bottleneck of ERAs by providing reliable proprioceptive information through optical sensing, enabling steps toward closed-loop control while maintaining the actuator's fast and repeatable performance.
...
TL;DR: Above-knee powertrain placement for powered prosthetic knees shows functional feasibility with improved walking speed and cadence compared to below-knee placement, suggesting optimized mass distribution can preserve powered assistance benefits while mitigating added weight effects.


<details>
  <summary>Details</summary>
Motivation: Lower limb amputation impairs mobility, and while powered prosthetic knees can restore some function, added mass from powered components may diminish benefits by negatively affecting gait mechanics and increasing metabolic cost. Optimizing mass distribution rather than simply minimizing total mass may provide more effective solutions.

Method: Exploratory study evaluating feasibility of above-knee powertrain placement for powered prosthetic knee in a small cohort. Compared above-knee vs below-knee configurations, assessing walking speed, cadence, gait symmetry, knee range of motion, peak velocity, and testing on ramps and stairs to evaluate control strategy robustness across locomotion tasks.

Result: Above-knee configuration demonstrated improved walking speed (+9.2% for one participant) and cadence (+3.6%) compared to below-knee placement, with mixed effects on gait symmetry. Kinematic measures showed similar knee range of motion and peak velocity across configurations. Control strategy proved robust across multiple locomotion tasks including ramps and stairs.

Conclusion: Above-knee placement is functionally feasible for powered prosthetic knees, and careful mass distribution can preserve the benefits of powered assistance while mitigating adverse effects of added weight. Further studies are needed to confirm these trends and guide design and clinical recommendations.
...
TL;DR: RA-Nav is a risk-aware navigation framework for aerial robots that uses semantic segmentation to classify obstacles and predict risks, enabling safe navigation when static obstacles suddenly move.


<details>
  <summary>Details</summary>
Motivation: Existing aerial navigation systems fail to adapt when static obstacles suddenly move, creating safety risks. There's a need for systems that can anticipate and respond to such state transitions through environmental semantic awareness.
...
Conclusion: The proposed risk-aware navigation framework successfully addresses the limitation of existing systems by incorporating semantic awareness and risk prediction, enabling aerial robots to navigate safely when obstacles unexpectedly change state.
...
<details>
  <summary>Details</summary>
Motivation: Industrial robotic camera systems are expensive and complex, limiting adoption. There's a need for affordable, accessible systems that can autonomously perform cinematic camera motions without requiring explicit programming.

Method: IRIS combines a lightweight, fully 3D-printed 6-DOF manipulator with a goal-conditioned visuomotor imitation learning framework based on Action Chunking with Transformers (ACT). The system learns object-aware and perceptually smooth camera trajectories directly from human demonstrations.
...
Conclusion: IRIS provides an accessible, low-cost solution for autonomous cinematic camera control that eliminates the need for explicit geometric programming through learning from demonstration, enabling broader adoption of robotic camera systems.
...
TL;DR: FR-GESTURE dataset for gesture-based UGV control by First Responders, containing 12 commands, 3312 RGBD pairs from 2 viewpoints and 7 distances, with baseline experiments and public availability.
...
Result: First dataset specifically for gesture-based UGV guidance by First Responders (FR-GESTURE), publicly available at https://doi.org/10.5281/zenodo.18131333, with baseline experiments provided for improvement.

Conclusion: FR-GESTURE dataset addresses the need for gesture-based UGV control in disaster response, providing a foundation for future research in AI and robotics to assist First Responders.
...
TL;DR: Proposes hybrid zonotopes with ADMM heuristic for efficient embedded motion planning of hybrid systems, reducing computational complexity and improving convergence.


<details>
  <summary>Details</summary>
Motivation: Embedded optimization-based planning for hybrid systems is computationally intensive due to mixed-integer programming, requiring more efficient formulations and solution methods.

Method: Uses hybrid zonotopes for set representation and reachability analysis, paired with a new ADMM mixed-integer programming heuristic to formulate and solve optimal planning problems.

Result: Hybrid zonotopes produce sets with lower memory complexity and tighter convex relaxations; ADMM heuristic achieves improved convergence rates compared to state-of-the-art MIP heuristics.

Conclusion: The framework enables efficient embedded hybrid system planning, demonstrated in autonomous driving behavior and motion planning scenarios.
...
TL;DR: Deep-Flow: Unsupervised safety-critical anomaly detection for AVs using Optimal Transport Conditional Flow Matching on a low-rank spectral manifold, achieving 0.766 AUC-ROC on Waymo dataset and revealing distinction between kinematic danger and semantic non-compliance.


<details>
  <summary>Details</summary>
Motivation: Safety validation for Level 4 AVs is bottlenecked by inability to scale detection of rare, high-risk long-tail scenarios using traditional rule-based heuristics. Need for data-driven approach to identify safety-critical anomalies beyond conventional safety filters.
...
Result: Achieves AUC-ROC of 0.766 on Waymo Open Motion Dataset against heuristic golden set of safety-critical events. Reveals fundamental distinction between kinematic danger and semantic non-compliance. Identifies predictability gap by surfacing out-of-distribution behaviors (lane-boundary violations, non-normative junction maneuvers) overlooked by traditional safety filters.

Conclusion: Provides mathematically rigorous foundation for defining statistical safety gates, enabling objective, data-driven validation for safe deployment of autonomous fleets. Framework offers unsupervised approach to detect safety-critical anomalies beyond conventional rule-based methods.
...
TL;DR: GNN-based dynamics modeling combined with structure-exploiting MPC enables real-time control of high-dimensional systems like soft robots, achieving 100Hz closed-loop control for 1000-node systems with sub-centimeter accuracy.
...
Method: Integrates Graph Neural Network (GNN)-based dynamics models with structure-exploiting Model Predictive Control (MPC). Uses graph representation with localized interactions to preserve sparsity, and employs a tailored condensing algorithm that eliminates state variables and scales linearly with system nodes, leveraging GPU parallelization.
...
Conclusion: The proposed framework successfully enables real-time control of high-dimensional systems by combining GNN-based modeling with structure-exploiting MPC, achieving computational efficiency through sparsity preservation and GPU acceleration.
